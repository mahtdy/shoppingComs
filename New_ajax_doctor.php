...
if($action=='del_doctor_reated_doctor_representation'&&$can_delete==1){
    $temp_field_nmae=explode(",",$field_nmae_brand);
    $temp=explode(",",$id_brand);
    $dif_arr=array_diff($temp_field_nmae ,$temp );
//    print_r($dif_arr);exit;
    foreach ($temp as $val){
        $query1="delete from new_doctor_brand_related where id='$val' and type=19 and page_id='$value_brand'";
//        echo $query1;exit;
        $coms_conect->query($query1);
    }
...
if($action=="show_related_doctor_representation_show"||$action=="del_doctor_reated_doctor_representation"){
//      echo 'chap table';
//	  echo '<div class="uploadbts">
//			<a   data-toggle="modal" data-target="#show_doctor" data-placement="top" rel="tooltip"><button><span class="flaticon-add133"></span><span>'.$view_add_doctor.'</span></button></a>
//			</div>';
    echo '<div class="toolbox">
			<div class="head"><span class="flaticon-copy23"></span><span style="color: rgb(82, 82, 82);">'.$view_related_doctor_list_representation.'</span>
			</div>
			<div class="tools"><a id="dropdelete_representation" data-toggle="modal" data-target="#del_reated_doctor_representation" data-placement="top"><span style="color:red;" class="flaticon-delete84"></span></a>
			</div>
			</div>';
    echo '<table cellpadding="0" id="new_page_table" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						<thead>
							...
							<th class="center">
								...
								<th>'.$view_title_doctor_representation.'</th>
...
//                        echo 'val=',$value.'=val';
        if($temp_arr[$j]>""){
            ...
            $sql12 = "SELECT  id,name,la,site  FROM new_doctor_cat_brand   WHERE id='$value_brand'";
            ...
            <tr>
                ...
                <td class="center" style="border-right: none;border-left: none;">
                    <label class="position-relative">
                        <input id='<?=$id?>' type="checkbox"   class="conchkNumber_doctor_representation_form"/>
                        <span class="lbl"></span>
                    </label> 
                </td>
                ...
                <td>
                    <?if($_SESSION["can_delete"]==1){?>
                        <a href="#" id="<?=$id?>" class="del_reated_doctor_representation red" data-title="delete" data-toggle="modal" data-target="#del_reated_doctor_representation" data-placement="top" rel="tooltip" style="font-size: 18px !important;margin: 0 5px 0 5px;">
    ...
    <script type="text/javascript">
        $(function () {
            $('.conchkSelectAll_representation_form').click( function() {
                $('.conchkNumber_doctor_representation_form').prop('checked', $(this).is(':checked'));
            });
            $('.conchkNumber_doctor_representation_form').click( function() {
                if($('.conchkNumber_doctor_representation_form:checked').length == $('.conchkNumber_doctor_representation_form').length) {
...
if($action=='show_related_doctor_representation'){
    echo '
          <table  cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered">
            <thead>
              <tr>
            
                <th class="center"><label class="position-relative"><input type="checkbox" class="conchkSelectAll_doctor_representation" /><span class="lbl"></span></label></th>
                <th>ID</th>
                <th>'.$view_subject.' </th> 
...
//    echo 'chkid brnd=='.$checkid_brand;
    ...
    $query2="select id,name from new_doctor_cat_brand WHERE name LIKE '%$id_brand%'" ;
//  echo $query2;
    $result2 = $coms_conect->query($query2);
    while($RS2 = $result2->fetch_assoc()) {
       ?>
        <input type="hidden" value="<?=$checkid_brand?>" name="conchkNumber_doctor_representation_checked" class="conchkNumber_doctor_representation_checked">
        <tr>
        <th class="center"><label class="position-relative">
        <input type="checkbox" value="<?=$RS2['id']?>"  <? $j=0; foreach($temp_arr as $value_brand){
            if($temp_arr[$j]>"") { if($RS2['id']==$value_brand){ echo 'checked '; }}$j++;}?> class="conchkNumber_doctor_representation"/><span class="lbl"></span></label></th>
        <td><?=$RS2["id"]?></td>
        <td><?=$RS2["name"]?></td>
...
if ($action=='del_doctor_expert'&&$can_edit==1){
    $sql="delete from new_doctor_expertise where id=$id_ctlg";
    //echo $sql;
  ...
  $id_semat=injection_replace($_POST['id_semat']);

if ($action=='del_doctor_semat'&&$can_edit==1){
    $sql="delete from new_doctor_phone where id=$id_semat";
    //echo $sql;
    $coms_conect->query($sql);
}


if ($action=='del_doctor_adres'&&$can_edit==1){
    $sql="delete from new_doctor_adres where id=$id_adres";
    //echo $sql;
    $coms_conect->query($sql);
}
...
if ($action=='del_doctor_tozie'&&$can_edit==1){
    $sql="delete from new_doctor_tozie where id=$id_tozie";
    //echo $sql;
    $coms_conect->query($sql);
}
...
if($action=='edit_page_com_type'&&$can_edit==1){
    $sql="select name,id,la,meta_keyword,meta_desciption,seo_title from new_doctor_cat_type where id=$id";
...
if($action=='edit_page_com_activity'&&$can_edit==1){
    $sql="select name,id,la,meta_keyword,meta_desciption,seo_title from new_doctor_cat_activity where id=$id";
    $result = $coms_conect->query($sql);
    $row = $result->fetch_assoc();
    if(check_lang_title($row['la'],$_SESSION["manager_title_lang"])==1)
        echo $row['id']."^".$row['name']."^".$row['meta_keyword']."^".$row['meta_desciption']."^".$row['seo_title'];
...
if($action=='edit_page_com_brand'&&$can_edit==1){
    $sql="select name,id,la,meta_keyword,meta_desciption,seo_title from new_doctor_cat_brand where id=$id";
    $result = $coms_conect->query($sql);
    $row = $result->fetch_assoc();
    if(check_lang_title($row['la'],$_SESSION["manager_title_lang"])==1)
        echo $row['id']."^".$row['name']."^".$row['meta_keyword']."^".$row['meta_desciption']."^".$row['seo_title'];
...
if($action=='edit_com_type_status'&&$can_edit==1){
    ...
    $cat_la=get_result("select la from new_doctor_cat_type where id = '$id'",$coms_conect);
    $parent_id=get_result("select parent_id from new_doctor_cat_type where id = '$id'",$coms_conect);
    ...
    foreach($temp as $val){
        $query1="update new_doctor_cat_type set status=$value where id=$val";
        $coms_conect->query($query1);
    }
...
if($action=='edit_com_activity_status'&&$can_edit==1){
    $_SESSION["father_car_arr"]='';
    $cat_la=get_result("select la from new_doctor_cat_activity where id = '$id'",$coms_conect);
    $parent_id=get_result("select parent_id from new_doctor_cat_activity where id = '$id'",$coms_conect);
    get_cat_child ($id,$coms_conect,$cat_la);
    get_cat_father ($parent_id,$coms_conect,$cat_la);
    $_SESSION["father_car_arr"] .=$id;
    $temp=explode('#',$_SESSION["father_car_arr"]);
    foreach($temp as $val){
        $query1="update new_doctor_cat_activity set status=$value where id=$val";
        $coms_conect->query($query1);
    }
    echo $_SESSION["father_car_arr"];
}
...
if($action=='edit_com_brand_status'&&$can_edit==1){
    $_SESSION["father_car_arr"]='';
    $cat_la=get_result("select la from new_doctor_cat_brand where id = '$id'",$coms_conect);
    $parent_id=get_result("select parent_id from new_doctor_cat_brand where id = '$id'",$coms_conect);
    get_cat_child ($id,$coms_conect,$cat_la);
    get_cat_father ($parent_id,$coms_conect,$cat_la);
    $_SESSION["father_car_arr"] .=$id;
    $temp=explode('#',$_SESSION["father_car_arr"]);
    foreach($temp as $val){
        $query1="update new_doctor_cat_brand set status=$value where id=$val";
        $coms_conect->query($query1);
    }
    echo $_SESSION["father_car_arr"];
}

if($action=='can_comment_doctor'){
    $sql="select user_id,la,site from new_doctor where id=$id";
    $result = $coms_conect->query($sql);
//     echo $id;
//     print_r($result);
    $row = $result->fetch_assoc();
    ...
    if(!in_array($row["user_id"],$_SESSION["manager_user_permisson"])||$can_edit!=1)
        ...
        $query1="update new_doctor set can_comment=$value where id=$id";
        $coms_conect->query($query1);
//        echo $query1;
    }
}
...
if($action=='change_doctor_status'){
    $sql="select user_id from new_doctor a,new_madules_comment b  where a.id=b.madul_id and  b.id=$id";
    $result = $coms_conect->query($sql);
...
if($action=='del_doctor_comments'){
    $temp=explode(",",$id);
    foreach ($temp as $value){
        $sql="select user_id from new_doctor a,new_madules_comment b  where a.id=b.madul_id and  b.id=$value";
        $result = $coms_conect->query($sql);
        $row = $result->fetch_assoc();
...
if($action=='change_lock_doctor'){
    ...
    if(!in_array($row["user_id"],$_SESSION["manager_user_permisson"])||$can_edit!=1||$_SESSION["edit_item"]!=$action)
        ...
        $query1="update new_doctor set mudoal_lock=$value where id=$id";
        $coms_conect->query($query1);
        //echo $query1;
    }
}

if($action=='del_doctor'){
    $temp=explode(",",$id);

    foreach ($temp as $value){
        $query="select user_id,site,la from new_doctor where id='$value'";
        $result = $coms_conect->query($query);
...
//            echo 'salam';
		...
		(select c.label_id  from new_doctor a ,new_mudoal_label c where   a.id=c.id and c.type=19 and c.id=$value) ";
            $coms_conect->query($query1);
            $condition1="id=$value  and type=19";
            delete_from_data_base('new_mudoal_label',$condition1,$coms_conect);
            $condition="id=$value";
            delete_from_data_base('new_doctor',$condition,$coms_conect);
        }
    }
}
...
if($action=='del_com_type'){
    ...
    if(!in_array($id,$_SESSION["manager_page_cat"]))
        header('Location: new_manager_signout.php');
    $query="select user_id,la from new_doctor_cat_type where id='$id'";
    $result = $coms_conect->query($query);
    $RS1 = $result->fetch_assoc();
    check_lang_title($RS1["la"],$_SESSION["manager_title_lang"]);
    ...
    if(!in_array($RS1["user_id"],$_SESSION["manager_user_permisson"])||$can_delete!=1)
        ...
        #########??? ÏÓÊå ÈäÏí
        $condition1="id=$id";
        delete_from_data_base('new_doctor_cat_type',$condition1,$coms_conect);
    }
}
...
if($action=='del_com_activity'){
    ...
    if(!in_array($id,$_SESSION["manager_page_cat"]))
        header('Location: new_manager_signout.php');
    $query="select user_id,la from new_doctor_cat_activity where id='$id'";
    $result = $coms_conect->query($query);
    $RS1 = $result->fetch_assoc();
    check_lang_title($RS1["la"],$_SESSION["manager_title_lang"]);
    ...
    if(!in_array($RS1["user_id"],$_SESSION["manager_user_permisson"])||$can_delete!=1)
        ...
        #########??? ÏÓÊå ÈäÏí
        $condition1="id=$id";
        delete_from_data_base('new_doctor_cat_activity',$condition1,$coms_conect);
    }
}
...
if($action=='del_com_brand'){
    ...
    if(!in_array($id,$_SESSION["manager_page_cat"]))
        header('Location: new_manager_signout.php');
    $query="select user_id,la from new_doctor_cat_brand where id='$id'";
    $result = $coms_conect->query($query);
    $RS1 = $result->fetch_assoc();
    check_lang_title($RS1["la"],$_SESSION["manager_title_lang"]);
    ...
    if(!in_array($RS1["user_id"],$_SESSION["manager_user_permisson"])||$can_delete!=1)
        ...
        #########??? ÏÓÊå ÈäÏí
        $condition1="id=$id";
        delete_from_data_base('new_doctor_cat_brand',$condition1,$coms_conect);
    }
}
...
if($action=='show_doctor_videos'){
    if($secend_value>0){
        $pic_id=get_result("select id from new_file_path where type=19 and name='doctor_video' and adress='$id'",$coms_conect);
        $pic=get_result("select adress from new_file_path where type=19 and name='doctor_video_pic' and id='$pic_id'",$coms_conect);
        $duration=get_result("select duration from new_doctor where id =$secend_value",$coms_conect);
    }else {
        $duration=substr(get_durition($ffmpeg,$id),11,9);
        $pic=take_pic_video($ffmpeg,$id);
    }
    ...
    echo "<input type='hidden' value='$duration' name='duration'>";
    echo "<input type='hidden' value='$pic' name='doctor_video_pic'>";
    echo '</div>
											</div>
										</div>
									</div>
...
if($action=='del_doctor_videos'){
    if($value>""){
        $query1="UPDATE   new_file_path SET   adress='' where id='$value' and type=19 and name='doctor_video'";
        $coms_conect->query($query1);
    }
}


if($action=='del_doctor_sound'){
    if($value>""){
        $query1="UPDATE   new_file_path SET   adress='' where id='$value' and type=19 and name='doctor_sound'";
        $coms_conect->query($query1);
    }
}
...
if($action=='copy_site_module'){
    ...
    foreach ($temp as $val){
      ...
      sign_out();
        ...
        $sql="select id from new_doctor_brand_related where page_id=$val and type={$_SESSION['modual_type']}";
        $result = $coms_conect->query($sql);
        while($row = $result->fetch_assoc()) {
        $id=$row['id'];
          $query1="insert into new_doctor_brand_related(id,page_id,type) 
          values ('$id',{$newid},{$_SESSION['modual_type']})";
          $coms_conect->query($query1);
      }
        #######################˜áãÇÊ ˜áíÏí#########################
