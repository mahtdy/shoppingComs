...
if ($action == 'show_contactus_status' && $_SESSION['can_edit'] == 1) {
    ...
    <div class="message-header clearfix">
        ...
        <div class="pull-right action-buttons">
            ...
            if ($RS1['type'] == 0) {
                ?>
                <a href="#" id="<?= $RS1['id'] ?>" class="reply_email" data-email="<?= $RS1['email'] ?>"
                   data-toggle="modal" data-target="#repeat">
            ...
            if ($RS1['type'] == 2) {
                ...
                <a href="#" id="<?= $RS1['id'] ?>" class="send_drft" data-email="<?= $RS1['email'] ?>"
                   data-title="<?= $RS1['name'] ?>" data-text="<?= $RS1['text'] ?>" data-toggle="modal"
                   data-target="#repeat">
        ...
        <div class="panel panel-success" id="field<?= $i ?>">
            <div class="panel-body">
                <div class="form-group">
                    ...
                    </a><label class="col-sm-2 control-label" style="text-align: center;"> <?= $RS['name'] ?></label>

                    <div class="form-group col-md-3">
                        <input type="hidden" name="val_id_cat<?= $i ?>" id="val_id_cat<?= $i ?>"
                               value="<?= $RS['cat_id'] ?>">
...
//                            echo $query1;exit;
                            ...
                            <select data-name="<?= $i ?>" id="option_field<?= $i ?>" name="option_field<?= $i ?>"
                                    data-option="option_field<?= $i ?>"
                                    data-div="div_<?= $i ?>" class="form-control select<?= $i ?> slct_slct">
                                <? while ($RS1 = $result1->fetch_assoc()) {
                                    ?>
                                    <option type="hidden" value="<?= $RS1['id']; ?>"><?= $RS1['value']; ?>
...
//    echo 'salam' . $id;
    $query = "select * from new_product_delicated_cat where   cat_id=$id and status=1";
    $query_edit = "select *,c.id as id_val_values from new_product_delicated_cat a,new_product_delicated_values c where   c.product_id='$edit_id' AND c.delicated_id_cat_val=a.id";
//    $query_edit1 = "select * from new_product_delicated_values where   product_id='$edit_id' and delicated_id='$id'";
//echo $query.'**'.$query_edit;exit;
                ...
                <div class="form-group">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="hidden" name="id_val_values<?= $i ?>" id="id_val_values<?= $i ?>"
                               value="<?= $set2[$i - 1] ?>">
                        <? if ($RS['type'] == 0) { ?>
                        <input placeholder="" type="text" name="filed_title<?= $i ?>"
                               id="filed_title<?= $i ?>" class="form-control" value="<?= ($set[$i - 1]) ?>">
                        <? } elseif ($RS['type'] == 1) {
                        $set_set = $set1[$i - 1];
                        $delicated_id = $RS['id'];
                        $query1 = "select * from new_product_delicated_cat_val where   cat_id=$id and delicated_id=$delicated_id";
                        ...
                        ?>
                            <select data-name="<?= $i ?>" id="option_field<?= $i ?>" name="option_field<?= $i ?>[]"
                                    multiple
                                    data-option="option_field<?= $i ?>"
                                    data-div="div_<?= $i ?>" class="form-control select2 slct<?= $i ?> slct_slct">
                                <? while ($RS1 = $result1->fetch_assoc()) {
                                    ?>
                                    <option value="<?= $RS1['id']; ?>" <? if (in_array($RS1['id'], $set_set)) echo 'selected '; ?> ><?= $RS1['value']; ?>
...
//       print_r($RS_edit[$i]);
        ...
        <div class="panel panel-success" id="field_vijen<?= $i ?>">
            <div class="panel-body">
                <div class="form-group">
                    <a data-filedid="<?= $RS['id'] ?>" class="col-md-1 control-label del_add_field_vijen"
                       id="<?= $i ?>"></a>
                    <input type="checkbox" name="chckbox_vijen<?= $i ?>"
                           id="chckbox_vijen<?= $i ?>"
                           class="col-sm-2 control-label" <? if ($set3[$i - 1] == 1) echo 'checked' ?>
                           value="<?= $set3[$i - 1] ?>" style="left: -50px;">
                    <label class="col-sm-2 control-label" style="text-align: center;"> <?= $RS['name'] ?></label>
                    <script>
                        var i =<?= $i ?>;
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="hidden" name="val_id_delicated_cat_vijen<?= $i ?>"
                               id="val_id_delicated_cat_vijen<?= $i ?>"
                               value="<?= $RS['id'] ?>">
                        ...
                        <input type="hidden" name="id_val_values_vijen<?= $i ?>" id="id_val_values_vijen<?= $i ?>"
                               value="<?= $set2[$i - 1] ?>">
                        <? if ($RS['type'] == 0) { ?>
                        <input placeholder="" type="text" name="filed_title_vijen<?= $i ?>"
                               id="filed_title_vijen<?= $i ?>" class="form-control" value="<?= ($set[$i - 1]) ?>">
                            <!--                        <input type="text" name="input_vijen--><? //= $i ?><!--" placeholder="خلاصه ویژگی را وارد کنید"-->
                            <!--                               id="input_vijen--><? //= $i ?><!--" class="col-sm-2 form-control" value="--><? //= $RS['text_vijen'] ?><!--" style="left: -170px;top: -34px;">-->
                        <? } elseif ($RS['type'] == 1) {
                        $set_set = $set1[$i - 1];
                        $delicated_id = $RS['id'];
                        $query1 = "select * from new_product_delicated_cat_val where   cat_id=$id and delicated_id=$delicated_id";
                        ...
                        $j = 0;
                        ?>
                            <select data-name="<?= $i ?>" id="option_field_vijen<?= $i ?>"
                                    name="option_field_vijen<?= $i ?>[]" multiple
                                    data-option="option_field_vijen<?= $i ?>"
                                    data-div="div_<?= $i ?>"
                                    class="form-control select2 slct_vijen<?= $i ?> slct_slct_vijen">
                                <? while ($RS1 = $result1->fetch_assoc()) {
                                    ?>
                                    <option value="<?= $RS1['id']; ?>" <? if (in_array($RS1['id'], $set_set)) echo 'selected '; ?> ><?= $RS1['value']; ?></option>
                                    <?
                                    $j++;
                                } ?>
                            </select>
                        ...
                        <? } ?>
                        <input type="text" name="input_vijen<?= $i ?>"
                               id="input_vijen<?= $i ?>" <? if ($RS['type'] == 0) echo 'style="display: none"' ?>
...
//       print_r($RS_edit[$i]);
        ...
        <div class="panel panel-success" id="field_barrasi<?= $i ?>">
            <div class="panel-body">
                <div class="form-group">
                    <a data-filedid="<?= $RS['id'] ?>" class="col-md-1 control-label del_add_field_barrasi"
                       id="<?= $i ?>"></a>
                    <input type="checkbox" name="chckbox_barrasi<?= $i ?>"
                           id="chckbox_barrasi<?= $i ?>"
                           class="col-sm-2 control-label" <? if ($set3[$i - 1] == 1) echo 'checked' ?>
                           value="<?= $set3[$i - 1] ?>" style="left: -50px;">
                    <label class="col-sm-2 control-label" style="text-align: center;"> <?= $RS['onvan_barr'] ?></label>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <input type="hidden" name="val_id_delicated_cat_barrasi<?= $i ?>"
                               id="val_id_delicated_cat_barrasi<?= $i ?>"
                               value="<?= $RS['id'] ?>">
                        ...
                        <div class="form-group  ">
                            <div class="form-group col-sm-12 div_editor<?= $i ?>" id="div_editor<?= $i ?>"
                                    ...
                                    <div class="form-group col-md-12">
                                        <div class="col-md-12 input-group">
                                            ...
                                            <div class="col-md-5 input-group">
                                                <input type="text"
                                                       id="box1_header_link_barrasi<?= $i ?>"
                                                       value="<?= $set1[$i - 1] ?>"
                                                       class="form-control"
                                                       placeholder=" تصویر"
                                                       name="box1_header_link_barrasi<?= $i ?>"
                                                       style="direction: ltr;">
                                            ...
                                            <div class="ui-sortable red box"
                                                 id="upload_type_box1_header_link_barrasi<?= $i ?>"
                                                 style="float:right;">
                                            </div>
                                ...
                                <!--                        --><?// if ($RS['type'] == 0) { ?>
                                <!--                        <input placeholder="-->
                                <?//= $RS['name'] ?><!--" type="text" name="filed_title_barrasi--><?//= $i ?><!--"-->
                                <!--                               id="filed_title_barrasi-->
                                <?//= $i ?><!--" class="form-control" value="--><?//= ($set[$i - 1]) ?><!--">-->
                                <!--                        <input type="text" name="input_barrasi-->
                                <?//= $i ?><!--" placeholder="خلاصه ویژگی را وارد کنید"-->
                                <!--                               id="input_barrasi-->
                                <?//= $i ?><!--" class="col-sm-2 form-control" value="-->
                                <?//= $RS['text_barrasi'] ?><!--" style="left: -170px;top: -34px;">-->
                                <!--                        --><?// } elseif ($RS['type'] == 1) {
                                $set_set = $set1[$i - 1];
                                $delicated_id = $RS['id'];
                                $query1 = "select * from new_product_delicated_cat_val where   cat_id=$id and delicated_id=$delicated_id";
                                ...
                                ?>
                                <!--                            <select data-name="-->
                                <?//= $i ?><!--" id="option_field_barrasi-->
                                <?//= $i ?><!--" name="option_field_barrasi--><?//= $i ?><!--[]" multiple-->
                                <!--                                    data-option="option_field_barrasi-->
                                <?//= $i ?><!--"-->
                                <!--                                    data-div="div_-->
                                <?//= $i ?><!--" class="form-control select2 slct_barrasi-->
                                <?//= $i ?><!-- slct_slct_barrasi">-->
                                <!--                                --><?// while ($RS1 = $result1->fetch_assoc()) {
                                //                                    ?>
                                <!--                                    <option value="--><?//= $RS1['id']; ?><!--" -->
                        ...
                        <!--                            --><?// } ?>
                        <input type="hidden" id="option_field_barrasi_val<?= $i ?>"
                               name="option_field_barrasi_val<?= $i ?>"
                               value="<?= $RS1['value']; ?>">
                    </div>
                </div>
            </div>
...
if ($action == 'zir_ok') {
    ...
    if ($temp_find > '') {
        ...
        $sql_find = "select id,cat_id_valid from new_product_cat_to_cat where cat_id_cat=$Option_cat1 AND cat_id_pro=$Option_cat";
//    echo $sql_find;exit;
...
//                    show_madules_cat($row['id'],$site_id,$manage_lang, $modual_type ,$coms_conect,$modul_name);
                            ...
                            <script type="text/javascript"
                                    src="/yep_theme/default/rtl/assets/js/jquery.nestable.min.js"></script>
...
//            print_r($sqlfild);
            ...
            while ($RS1 = $result->fetch_assoc()) {
              ?>
<!--                <option class="colse_close" selected value="--><?//= $RS1['id'] ?><!--">--><?//= $RS1['mazaya'] ?><!--</option>-->

                <li id="<?= $RS1['id'] ?>">
                    <div  class="form-group" id="<?= $RS1['id'] ?>"><?= $RS1['mazaya'] ?>
                <i id="<?= $RS1['id'] ?>" class="select2-search-choice-close close_close" style="top: -18px;left: 18px; position: relative!important; background: none!important;">
                </i></div></li>
            <? } ?>
            </ul>
<!--        </select>-->
...
//    $sql_chkd = "select id,cat_id_valid,nes_tik from new_product_cat_to_cat where  cat_id_pro=$Option_cat AND cat_id_cat=$Option_cat1";
//    $result_chkd = $coms_conect->query($sql_chkd);
////    while ($RS1_chkd=$result_chkd->fetch_assoc()){
//    $RS1_chkd = $result_chkd->fetch_assoc();
//
...
//    $idid = $RS1_chkd['id'];
//    $label_chkd = explode(',', $label_chkd);
//    if ((!in_array($id, $label_chkd)) && $chkd == 1) {
//        array_push($label_chkd, $id);
//        $label_chkd = array_filter($label_chkd);
//        $chkdchkd = implode(',', $label_chkd);
//        $sql_up = "UPDATE new_product_cat_to_cat SET nes_tik='$chkdchkd' WHERE id=$idid";
//        $coms_conect->query($sql_up);
//    } elseif ($chkd == 0) {
////        echo $id.'<br>';
//        $label_chkd = array_diff($label_chkd, array($id));
//        $label_chkd = array_filter($label_chkd);
//        $chkdchkd = implode(',', $label_chkd);
//        $sql_up = "UPDATE new_product_cat_to_cat SET nes_tik='$chkdchkd' WHERE id=$idid";
//        $coms_conect->query($sql_up);
...
//echo 'tef='.$temp_find;exit;
    if ($temp_find > '') {
        $sql_up = "UPDATE new_product_cat_to_cat SET cat_id_cat='$option_field1', cat_id_pro='$option_field2', cat_id_valid='$meta_label', cat_rang='' WHERE id=$temp_find";
//    echo $sql_up;exit;
        $coms_conect->query($sql_up);
    } else {
        $sql = "INSERT INTO new_product_cat_to_cat(cat_id_cat, cat_id_pro, cat_id_valid, cat_rang) VALUES ('$option_field1','$option_field2','$meta_label','$cat_rang')";
        $coms_conect->query($sql);
...
if ($action=='ok_config_sms'){
    $sql = "select * from new_members where id=$id AND  sms_code='$value'";
    echo $sql;
//    $coms_conect->query($sql);
//    $sql_save = "UPDATE new_product_color_size SET id_color='$arrcolor1',id_size='$arrsize1' WHERE id='$idpro'";
//    $coms_conect->query($sql_save);
            ...
            <tr>
                ...
                <td>
                    ...
                    if ($_SESSION["can_delete"] == 1) {
                        ?>
                        <a href="#" id="<?= $id ?>" class="del_reated_host_representation red" data-title="delete"
                           data-toggle="modal" data-target="#del_reated_host_representation" data-placement="top"
                           rel="tooltip" style="font-size: 18px !important;margin: 0 5px 0 5px;">
...
//  echo $query2;
    ...
    while ($RS2 = $result2->fetch_assoc()) {
        ...
        <th class="center"><label class="position-relative">
                <input type="checkbox" value="<?= $RS2['id'] ?>" <? $j = 0;
                foreach ($temp_arr as $value_brand) {
                    if ($temp_arr[$j] > "") {
...
if ($action == 'del_com_brand') {
    ...
    if (!in_array($RS1["user_id"], $_SESSION["manager_user_permisson"]) || $can_delete != 1)
        ...
        #########??? ÏÓÊå ÈäÏí
        $condition1 = "id=$id";
        delete_from_data_base('new_host_cat_brand', $condition1, $coms_conect);
    }
}
...
//			</div>';
    ...
    echo '<table cellpadding="0" id="new_page_table" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						<tbody>';
    $totla_related = $id;
    ?><input type="hidden" class="check_checked" id="check_checked" name="check_checked"
             value="<?= $totla_related ?>"><?
    $temp_arr = explode(",", $id);
    $i = 1;
    $j = 0;
...
//                        echo 'val=',$value.'=val';
        if ($temp_arr[$j] > "") {
            ...
            <tr>
                ...
                <?
                if ($row['site'] == 'main') $domain = '/' . $domain_name; else $domain = '/' . $row['site'] . '.' . $domain_name; ?>
                <td><a href="/<?= "$domain/host/{$row['la']}/{$row['id']}/" . insert_dash($row['question']) ?>"
                       target="_blank"><?= $row['question'] ?></a></td>
                <td><?= $row['answer'] ?></td>
...
//    $checkid=$totla_related;
    ...
    while ($RS2 = $result2->fetch_assoc()) {
        ...
        <!--        <input type="text" value="--><?// print_r($temp_arr);?><!--" name="conchkNumber_host_checked" class="conchkNumber_host_checked">-->
    ...
    foreach ($temp_arr as $value) {
        if ($temp_arr[$j] > "") {
            ...
            <tr>
                ...
                <?
                if ($row['site'] == 'main') $domain = '/' . $domain_name; else $domain = '/' . $row['site'] . '.' . $domain_name; ?>
                <td><a href="/<?= "$domain/news/{$row['la']}/{$row['id']}/" . insert_dash($row['name']) ?>"
                       target="_blank"><?= $row['name'] ?></a></td>
