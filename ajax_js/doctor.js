/*    گالری   */
 	$("#add_videos").click(function () {
		 ...
		 if($("#doctor_video").val()){
			 ...
			 data:"action=show_doctor_videos&id="+$("#doctor_video").val(),
	...
	//   واتر مارک
 	...
 	$(".show_content_gallery").click(function () {
		...
		$('.conchkNumber_doctor_form').prop('checked', $(this).is(':checked'));
		var doctors = $('input:checkbox:checked.conchkNumber_doctor_form').map(function () {
			return $(this).attr('id');
		}).get();
		 
		$("#dropdelete").val(doctors);
    });
	$(document).on('click', '.conchkNumber_doctor_form', function(event) {
		var doctors = $('input:checkbox:checked.conchkNumber_doctor_form').map(function () {
			return $(this).attr('id');
		}).get();
		$("#dropdelete").val(doctors);
...
$(function () {	
	$('.conchkNumber_4').click( function() {
		...
		$.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=change_lock_doctor&doctor="+a+"&id="+$(this).attr('id'),
				success: function(result){
					alert();
				}
			});
		 ...
		 if($("#doctor_video").val()){
			 $("#show_waiting_video").show();
				 $.ajax({
					 type:'POST',
					 url:'New_ajax_doctor.php',
					 data:"action=del_doctor_videos&id="+$("#doctor_video").val()+"&value="+$("#edit_mode").val(),
					 	success: function(result){
							$("#show_waiting_video").hide();
							$("#show_video").empty();	
							$("#doctor_video").val('');
	...
	$("#btn_ok").val($("#check_value").val());
		...
		$("#btn_ok").click(function () {
			...
			$.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=del_doctor&id="+id,
				success: function(result){
				// console.log(result);
					window.location.href = "newcoms.php?yep=new_doctor_text";
					// alert(result);
				}
			});		
		});
...
$(document).on('click', '.del_reated_doctor', function(event) {
		$("#btn_del_related_doctor").val($(this).attr('id'));

});

$(document).on('click', '#btn_del_related_doctor', function() {
			$.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=del_doctor_reated_doctor&id="+$(this).val()+"&value="+$("#edit_mode").val()+"&field_nmae="+$("#totla_related").val(),
				success: function(result){
                    // alert(id);
					$("#relateddoctor").html(result);

					$("#btn_del_related_doctor").val('');
				}
			});	
		});	
	...
	$(document).on('click', '.conchkSelectAll_form', function(event) {
		$('.conchkNumber_doctor_form').prop('checked', $(this).is(':checked'));
		var values = $('input:checkbox:checked.conchkNumber_doctor_form').map(function () {
			return $(this).attr('id');
		}).get();
		$("#btn_del_related_doctor").val(values);
    });
	$(document).on('click', '.conchkNumber_doctor_form', function(event) {
		var values = $('input:checkbox:checked.conchkNumber_doctor_form').map(function () {
			return $(this).attr('id');
	...
	$(document).on('click', '.conchkSelectAll_doctor', function(event) {
		$('.conchkNumber_doctor').prop('checked', $(this).is(':checked'));
		var values = $('input:checkbox:checked.conchkNumber_doctor').map(function () {
			return $(this).attr('id');
		}).get();
		$("#btn_del_related_doctor").val(values);
    });
	$(document).on('click', '.conchkNumber_doctor', function(event) {
		var values = $('input:checkbox:checked.conchkNumber_doctor').map(function () {
        return $(this).val();
		}).get();
		$("#btn_del_related_doctor").val(values);
		...
		$(document).on('click', '#btn_ok_doctor00', function(event){
			//var checkid=$(".conchkNumber_doctor_checked").val();
			var values = $('input:checkbox:checked.conchkNumber_doctor').map(function () {
				return $(this).val();
			}).get();

            // var values1 = $('input:checkbox:checked.conchkNumber_doctor1').map(function () {
            //     return $(this).val();
            // }).get();
			if($(values>"")) {
                $("#show_waiting_related").show();
                var chkId = $(".conchkNumber_doctor_checked").val();
               // alert('comjs='+chkId);
                ...
                values=chkId;
		 			$.ajax({
						type:'POST',
						url:'New_ajax.php',
						data:"action=show_related_doctor_show&id="+values+"&checkid="+chkId,
						success: function(result){
...
$(document).on('click', '.del_reated_doctor_representation', function(event) {
    $("#btn_del_related_doctor_representation").val($(this).attr('id'));

});

$(document).on('click', '#btn_del_related_doctor_representation', function() {
    // var rr=$(this).val();
    ...
    $.ajax({
        type:'POST',
        url:'New_ajax_doctor.php',
        data:"action=del_doctor_reated_doctor_representation&id_brand="+$(this).val()+"&value_brand="+$("#edit_mode").val()+"&field_nmae_brand="+$("#totla_related_brand").val(),
        success: function(result){
        	$("#relateddoctor_representation").html(result);
            $("#btn_del_related_doctor_representation").val('');
        }
    });
});
...
$(document).on('click', '.conchkSelectAll_representation_form', function(event) {
    ...
    var values = $('input:checkbox:checked.conchkNumber_doctor_representation_form').map(function () {
    	// alert('qwqw'+$(this).attr('id'));
        return $(this).attr('id');
    }).get();
    $("#btn_del_related_doctor").val(values);
});
$(document).on('click', '.conchkNumber_doctor_representation_form', function(event) {
    var values = $('input:checkbox:checked.conchkNumber_doctor_representation_form').map(function () {
        return $(this).attr('id');
    }).get();
    $("#btn_del_related_doctor_representation").val(values);
});


$(document).on('click', '.conchkSelectAll_doctor_representation', function(event) {
    $('.conchkNumber_doctor_representation').prop('checked', $(this).is(':checked'));
    var values = $('input:checkbox:checked.conchkNumber_doctor_representation').map(function () {
        return $(this).attr('id');
    }).get();
    $("#btn_del_related_doctor_representation").val(values);
});
$(document).on('click', '.conchkNumber_doctor_representation', function(event) {
    var values = $('input:checkbox:checked.conchkNumber_doctor_representation').map(function () {
        return $(this).val();
    }).get();
    $("#btn_del_related_doctor_representation").val(values);
...
$(document).on('click', '.conchkSelectAll_doctor_representation_form', function(event) {
    $('.conchkNumber_doctor_representation_form').prop('checked', $(this).is(':checked'));
    var doctors = $('input:checkbox:checked.conchkNumber_doctor_representation_form').map(function () {
        return $(this).attr('id');
    }).get();

    $("#dropdelete_representation").val(doctors);
});
$(document).on('click', '.conchkNumber_doctor_representation_form', function(event) {
    var doctors = $('input:checkbox:checked.conchkNumber_doctor_representation_form').map(function () {
        return $(this).attr('id');
...
$(document).on('click', '#btn_ok_doctor_representation00', function(event){
			var values = $('input:checkbox:checked.conchkNumber_doctor_representation').map(function () {
                return $(this).val();
				}).get();
			if($(values>"")) {
				$("#show_waiting_related").show();
				// alert('val=='+values);
				var chkId_brand = $(".conchkNumber_doctor_representation_checked").val();
               ...
               // alert('chkid akhar='+chkId_brand);
                values=chkId_brand;
		 			$.ajax({
						type:'POST',
						url:'New_ajax_doctor.php',
						data:"action=show_related_doctor_representation_show&id_brand="+values+"&checkid_brand="+chkId_brand,
						success: function(result){
							$("#show_waiting_related").hide();
							$("#relateddoctor_representation").html(result);
						}
...
$(function () {	
	$('.can_comment').click( function() {
        ...
        $.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=can_comment_doctor&value="+a+"&id="+id,
				success: function(result){
				 // alert(id);
				 // alert(a);
				}
