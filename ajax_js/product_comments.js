...
$(".del_all").click(function () {
	...
	$("#btn_ok_page").click(function () {
			$.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=del_product_comments&id="+$(this).val(),
				success: function(result){
				//alert(result);
				if(result==0)
				alert('اين عمليات غير مجاز است');
				else
				window.location.href = "newcoms.php?yep=new_product_comment";
				}
			});	
		});
...
$(function () {	
	$('.conchkNumber_4').click( function() {
		...
		$.ajax({
				type:'POST',
				url:'New_ajax.php',
				data:"action=change_product_status&value="+a+"&id="+$(this).attr('id'),
				success: function(result){
				 //alert(result);
				}
			});
