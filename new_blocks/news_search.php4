...
<div class="block ">
	...
	<div class="header">
		<h5 class="H_block_h3 H_center"><span><?=$view_serach_news?></span></h5>
	</div>
	<div class="content filter-block H_p0 H_bg H_mt05">
		...
		<select name="modual_cat" id='modual_cat' class="cat-select" multiple="multiple" style="width: 100%;">
			<?$sql121="select id,name from new_modules_cat where type=1 and status=1";	
		...
		<div class="row H_p0" style="margin-top: 15px; padding-right: 15px; padding-left: 15px;">
		...
		$("#faqq").keypress(function (e) {
			if(e.which==13){
			   $("#faq_search").attr("action", "/news/<?=$defult_lang?>/search/" + $("#faqq").val()+'/'+$("#modual_cat").val()+"/1");
			   $("#faq_search").submit()
			}
		});
