<div class="block hidden-xs">
	<div class="header">
		<h3><span><?=$view_related_cat_download?></span></h3>
	</div>
	<div class="content post-block">
		...
		<?if($cat_id==0||get_result("select count(id) from new_modules_cat where id=$cat_id and type=6",$coms_conect)==0)
			...
			$queryw1="select  a.la,a.title,a.id from new_download a ,new_modules_catogory b  
			where  
			site='$site' and la='$la' and status=1 and b.type=6 and a.id=b.page_id  
			...
			<li class="row">
				<div class="col-md-5 pull-right pl0">
					...
					<img src="<?=get_result("select adress from new_file_path where id={$rows3s['id']} and name='download_pic'",$coms_conect);?>"></a>
				</div>
