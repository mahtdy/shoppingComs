<div class="block hidden-xs">
	<div class="header">
		<h3><span><?=$view_visit_content_special?></span></h3>
	</div>
	<div class="content title-block">
		<ul>
		<?$time=time();
		$query1="select  a.name,a.title ,a.la,a.id from new_content a
		...
		$result1 = $coms_conect->query($query1);
			while($rowe1 = $result1->fetch_assoc()){?>
				<li><a href="/content/<?="{$rowe1['la']}/{$rowe1['id']}/".insert_dash($rowe1['name'])?>"><?=$rowe1['title']?></a></li>
			<?}?>
		 </ul>
	</div>
</div>