...
<div class="block ">
	...
	<div class="header">
		<h3><span><?=$view_search_content?></span></h3>
	</div>
	<div class="content filter-block">
		...
		<select name="modual_cat" id='modual_cat' class="cat-select" multiple="multiple" style="width: 100%;">
			<?$sql121="select id,name from new_modules_cat where type=11 and status=1";	
		...
		$("#faqq").keypress(function (e) {
			if(e.which==13){
			   $("#faq_search").attr("action", "/content/<?=$defult_lang?>/search/" + $("#faqq").val()+'/'+$("#modual_cat").val()+"/1");
			   $("#faq_search").submit()
			}
		});  
