...
<div class="block ">
	...
	<div class="header">
		<h5 class="H_block_h3 H_center"><span>جستجوی مقاله</span></h5>
	</div>
	<div class="content filter-block H_p0 H_bg H_mt05">
		...
		<select name="modual_cat" id='modual_cat' class="cat-select" multiple="multiple" style="width: 100%;">
			<?$sql121="select id,name from new_modules_cat where type=7 and status=1";	
		...
		$("#faqq").keypress(function (e) {
			if(e.which==13){
			   $("#faq_search").attr("action", "/article/<?=$defult_lang?>/search/" + $("#faqq").val()+'/'+$("#modual_cat").val()+"/1");
			   $("#faq_search").submit()
			}
		});
