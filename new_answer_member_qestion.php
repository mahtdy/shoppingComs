...
if($answer>""){
	...
	$row = $result->fetch_assoc();
 	if($row['answer']==$answer){
		//$_SESSION['change_password']=$_SESSION['answer_member_qestion'];
		
		$temp= create_member_reset_password($_SESSION['answer_member_qestion']);
		$condition="user_name='{$_SESSION['answer_member_qestion']}'";
		$arr_slide=array("email_link"=>$temp);
		update_data_base($arr_slide,'new_members',$condition,$coms_conect);
		
		
		echo "<script>window.location='new_member_change_password.php?la=$la&val=$temp'</script>";
	}
