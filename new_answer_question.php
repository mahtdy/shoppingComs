...
<?@session_start();
if(!isset($_SESSION['answer_qestion']))
...
if($answer>""){
	...
	$query="SELECT answer from new_managers where user_name='{$_SESSION['answer_qestion']}'";
	 
	$result = $coms_conect->query($query);
	$row = $result->fetch_assoc();
 
	if($row['answer']==$answer){
		$_SESSION['change_password']=$_SESSION['answer_qestion'];
		echo "<script>window.location='new_change_password.php'</script>";
}
		show_msg_warninig($view_incorrect_reply2);
 
}
	$query="SELECT question from new_managers where user_name='{$_SESSION['answer_qestion']}'";
	$result = $coms_conect->query($query);
	$row = $result->fetch_assoc();
