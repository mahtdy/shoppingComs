jQuery(function($) {
	...
	var get_section_url = function(section_name) {
		...
		return this.settings.base + '/new_help/sections/' + url;
	}
	var get_img_url = function(src) {
		return this.settings.base + '/new_help/' +src;
	}
	...
	*/
	...
	$(window).on('hashchange.start_help', function(e) {
		...
		if(help == null && window.location.hash == '') {
			startHelp();
			...
			$(document).on('click.start_help', '.sidebar .nav-list a', function() {
				...
				if( !href.match(/\#help$/) ) $(this).attr('href');
			});
		}
