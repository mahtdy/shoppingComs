            ...
            <tr>
                ...
                <td>
                    <?if($_SESSION["can_delete"]==1){?>
                        <a href="#" id="<?=$id?>" class="del_reated_host_representation red" data-title="delete" data-toggle="modal" data-target="#del_reated_host_representation" data-placement="top" rel="tooltip" style="font-size: 18px !important;margin: 0 5px 0 5px;">
...
//  echo $query2;
    ...
    while($RS2 = $result2->fetch_assoc()) {
       ?>
        <input type="hidden" value="<?=$checkid_brand?>" name="conchkNumber_host_representation_checked" class="conchkNumber_host_representation_checked">
        <tr>
        <th class="center"><label class="position-relative">
        <input type="checkbox" value="<?=$RS2['id']?>"  <? $j=0; foreach($temp_arr as $value_brand){
            if($temp_arr[$j]>"") { if($RS2['id']==$value_brand){ echo 'checked '; }}$j++;}?> class="conchkNumber_host_representation"/><span class="lbl"></span></label></th>
        <td><?=$RS2["id"]?></td>
        <td><?=$RS2["name"]?></td>
...
echo $sql12;exit;
            ...
            <tr>
                ...
                <?if($row['site']=='main') $domain= '/'.$domain_name; else $domain='/'. $row['site'].'.'.$domain_name;?>
                <td><a href="/<?="$domain/host/{$row['la']}/{$row['id']}/".insert_dash($row['question'])?>" target="_blank"><?=$row['question']?></a></td>
                <td><?=$row['answer']?></td>
                ...
                <td>
                    <?if($_SESSION["can_delete"]==1){?>
                        <a href="#" id="<?=$id?>" class="del_reated_host red" data-title="delete" data-toggle="modal" data-target="#del_reated_host" data-placement="top" rel="tooltip" style="font-size: 18px !important;margin: 0 5px 0 5px;">
                            <i class="ace-icon fa fa-trash-o bigger-120" title="<?=$view_delete?>"></i>
                        </a>
                    <?}?>
                </td>
...
//    $checkid=$totla_related;
  ...
  echo $query2;
    ...
    while($RS2 = $result2->fetch_assoc()) {
        ...
        <tr>
        <th class="center"><label class="position-relative">
                <input type="checkbox" value="<?=$RS2['id']?>"  <? $j=0; foreach($temp_arr as $value){
                    if($temp_arr[$j]>"") { if($RS2['id']==$value){ echo 'checked '; }}$j++;}?>  class="conchkNumber_host" ><span class="lbl"></span></label></th>
        <td><?=$RS2["id"]?></td>
        <td><?=$RS2["question"]?></td>
        <td><?=$RS2["answer"]?></td>
