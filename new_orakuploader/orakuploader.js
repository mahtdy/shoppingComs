(function ( $ ) {
    ...
    $.fn.orakuploader = function( options ) {
		...
		var settings = $.extend({
			...
			orakuploader_upload_to_ftp       : false,
		...
		$(holdername).on("click", ".picture_delete", function() {
			var ftp = 0;
			if(settings.orakuploader_upload_to_ftp == true) {
				ftp = 1;
			}
		...
		$(holdername).on("click", ".rotate_picture", function() {	
			...
			$.ajax({
				url: settings.orakuploader_path+"new_orakuploader.php?rotate="+encodeURIComponent($(this).parent().attr('filename'))+"&degree_lvl="+$(this).closest('.rotate_picture').attr('degree-lvl')+"&path="+settings.orakuploader_path+"&resize_to="+settings.orakuploader_resize_to+"&thumbnail_size="+settings.orakuploader_thumbnail_size+"&main_path="+settings.orakuploader_main_path+"&thumbnail_path="+settings.orakuploader_thumbnail_path+"&use_ftp="+ftp
	...
	{	var ftp = 0;
		...
		xhr.open("POST", settings.orakuploader_path+"new_orakuploader.php?filename="+encodeURIComponent(file.name)+"&path="+settings.orakuploader_path+"&resize_to="+settings.orakuploader_resize_to+"&thumbnail_size="+settings.orakuploader_thumbnail_size+"&main_path="+settings.orakuploader_main_path+"&thumbnail_path="+settings.orakuploader_thumbnail_path+"&watermark="+settings.orakuploader_watermark+"&orakuploader_crop_to_width="+settings.orakuploader_crop_to_width+"&orakuploader_crop_to_height="+settings.orakuploader_crop_to_height+"&orakuploader_crop_thumb_to_width="+settings.orakuploader_crop_thumb_to_width+"&orakuploader_crop_thumb_to_height="+settings.orakuploader_crop_thumb_to_height+"&use_ftp="+ftp, true);
		xhr.send(file);
		xhr.onreadystatechange = function() 
		{
			...
			if (xhr.readyState == 4) 
			{
				...
				if(settings.orakuploader_upload_to_ftp) {
					setTimeout(function() {
						$.ajax({
							url: settings.orakuploader_path+"new_orakuploader.php?delete="+encodeURIComponent( xhr.responseText)+"&path="+settings.orakuploader_path+"&resize_to="+settings.orakuploader_resize_to+"&thumbnail_size="+settings.orakuploader_thumbnail_size+"&main_path="+settings.orakuploader_main_path+"&thumbnail_path="+settings.orakuploader_thumbnail_path
						});
					}, 5000);
				}
			}
