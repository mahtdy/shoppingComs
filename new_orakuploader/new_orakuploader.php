...
if(isset($_REQUEST['use_ftp']) && $_REQUEST['use_ftp'] == 1) {
	$ftp_server = "94.182.110.170";
	$ftp_username   = "simg2.abadis.ir";
	$ftp_password   =  "SSporiya500Aa400";
	
	$ftp_path = "uploads/".date("Y")."/".date("m")."/".date("d")."/";
	$ftp_tn_path = "uploads/".date("Y")."/".date("m")."/".date("d")."/tn/";
	...
	@ftp_mkdir($conn_id, "uploads/".date("Y").'/');
	@ftp_mkdir($conn_id, "uploads/".date("Y")."/".date("m")."/");
	@ftp_mkdir($conn_id, "uploads/".date("Y")."/".date("m")."/".date("d").'/');
	@ftp_mkdir($conn_id, "uploads/".date("Y")."/".date("m")."/".date("d")."/tn/");
//	ftp_mkdir($conn_id, $ftp_tn_path);
}
...
if(isset($_GET['delete']))
{
	...
	if(isset($_REQUEST['use_ftp']) && $_REQUEST['use_ftp'] == 1) {
		ftp_delete($conn_id, $ftp_path.$_GET['delete']);
		ftp_delete($conn_id, $ftp_tn_path.$_GET['delete']);
		ftp_close($conn_id);
	}
	exit;
}
...
if(isset($_REQUEST['use_ftp']) && $_REQUEST['use_ftp'] == 1) {
	$file = $main_path.'/'.$filename;
	$remote_file_path = $ftp_path.$filename;
	ftp_put($conn_id, $remote_file_path, $file, FTP_BINARY);
	$file = $thumbnail_path.'/'.$filename;
	$remote_file_path = $ftp_tn_path.$filename;
	ftp_put($conn_id, $remote_file_path, $file, FTP_BINARY);
	ftp_close($conn_id);
