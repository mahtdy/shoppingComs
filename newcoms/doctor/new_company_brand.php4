...
if($onvan>""&&$edit_mode==0&&$_SESSION["can_add"]==1){
    ...
    $query1="insert into new_company_cat_brand(seo_title,status,meta_keyword,meta_desciption,name,rang,type,date,user_id,ip,la) 
...
//	echo $query1;
    ...
    if(!in_array($RS121["la"],$_SESSION["manager_title_lang"]))
        header('Location: new_manager_signout.php');
    $query1="update new_company_cat_brand set seo_title='$seo_title',meta_keyword='$meta_keyword',meta_desciption='$meta_desciption',name='$onvan',edit_date=NOW(),edit_user_id='$manager_id',ip='$custom_ip' where id='$edit_mode'";
    $coms_conect->query($query1);
}
