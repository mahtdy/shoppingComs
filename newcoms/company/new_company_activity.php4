...
if($onvan>""&&$edit_mode==0&&$_SESSION["can_add"]==1){
    ...
    $sql="SELECT id FROM new_company_cat_activity ORDER BY id DESC LIMIT 1";
    ...
    $query1="insert into new_company_cat_activity(seo_title,status,meta_keyword,meta_desciption,name,rang,type,date,user_id,ip,la) 
	values ('$seo_title',1,'$meta_keyword','$meta_desciption','$onvan',$id,'$modual_type',NOW(),$manager_id,'$custom_ip','$manage_lang')";
...
//	echo $query1;
    ...
    check_token( $_REQUEST[ 'user_token' ], $_SESSION[ 'session_token' ], '/coms' );
    $query12="select la from new_company_cat_activity a  where id=$edit_mode";
    $result12 = $coms_conect->query($query12);
    $RS121 = $result12->fetch_assoc();
    if(!in_array($RS121["la"],$_SESSION["manager_title_lang"]))
        header('Location: new_manager_signout.php');
    $query1="update new_company_cat_activity set seo_title='$seo_title',meta_keyword='$meta_keyword',meta_desciption='$meta_desciption',name='$onvan',edit_date=NOW(),edit_user_id='$manager_id',ip='$custom_ip' where id='$edit_mode'";
    $coms_conect->query($query1);
}
...
<div class="tabbable">
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <?//
                    $sql = "SELECT * FROM new_company_cat_activity a,new_cat_permission b  WHERE    b.permission=1 and a.id=b.menu_id and a.parent_id='0' and b.group_id={$_SESSION["manager_group_id"]} and a.type='$modual_type' and a.la='$manage_lang' GROUP BY  a.id ORDER BY a.rang";
                    //echo $sql;
                    $result = $coms_conect->query($sql);
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<div class='dd-handle'><a target='_blank' href="."/$modul_name/$manage_lang/company/$name/1"."> {$row['name']}</a>";
    ...
    <script src="ajax_js/page_com_activity.js"></script>
    <script>
