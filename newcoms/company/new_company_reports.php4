<?$modual_type=11;
$report_url='content_reports';
$report_table='new_content';
$report_title=$new_sysmenu[412];
include 'newcoms/main/new_modual_report.php4';
?>
			<div class="toptenrep">
					<h3><span class="flaticon-view32"></span><span class="title">محتوای پربازدید</span></h3>
					...
					<table cellpadding="0" id="new_page_table" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						<tbody>
						 ...
						 <?
						################################################################
							$sql1 = "SELECT count(id) as cnt from new_content a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=11 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id";
							$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
							$page=injection_replace($_GET["page"]);
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=content_reports$label_page$is_importand_page$cat_page$expire_page$status_page");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
							$sql12 = "SELECT a.user_id,a.site,a.id,a.la,a.title,a.name,a.view,a.publish_date,a.status
							from new_content a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=11 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id
				...
				<tr>
							<td class="center"><?=$RS1["id"]?></td>
								...
								<td><a target="_blank" href="/<?="$domain/content/{$RS1['la']}/{$RS1['id']}"?>"><?=$RS1["name"]?></a></td>
								...
								<td>
									<span class="status_content editable editable-click" data-pk="<?=$RS1["id"]?>" ><?if($RS1["status"]==1){echo 'منتشر شده';}else{echo 'پيش نويس';}?></span>
								</td>
								<td>
									<?if($_SESSION["can_edit"]==1){?>
									<a id="<?=$id?>" class="edit_menu blue"  href="newcoms.php?yep=new_contenttext&edit_id=<?=$id?>" style="font-size: 15px !important;">
									...
									<a href="/<?="$domain/content/{$RS1['la']}/{$RS1['id']}"?>" target="_blank"  class="del_menu blue"   style="font-size: 18px !important;margin: 0 5px 0 5px;">
										<i class="ace-icon fa fa-vimeo-square bigger-120" title="نمایش"></i>
									</a>
									<?}?>
									<?/*?>
									<a href="/newcoms.php?yep=new_content_comment" target="_blank" id="<?=$id?>" class="green new_scomments">
										<i class="ace-icon fa fa-comments bigger-120" title="نظرات"></i>
									</a>	
									<a href="/newcoms.php?yep=new_content_comment" target="_blank" id="<?=$id?>" class=" new_movie" data-title="show_video" data-toggle="modal" data-target="#show_video" data-placement="top" rel="tooltip" style="font-size: 15px !important;">
										<i class="ace-icon fa fa-film bigger-120" title="فیلم"></i>
									</a>
									<a href="/newcoms.php?yep=new_content_comment" target="_blank" id="<?=$id?>" class="green new_movie">
										<i class="ace-icon fa fa-picture-o bigger-120" title="البوم عکس"></i>
									</a>
									<a href="/newcoms.php?yep=new_content_comment" target="_blank" id="<?=$id?>" class="blue new_voice">
										<i class="ace-icon fa fa-volume-up bigger-120" title="صدا"></i>
									</a>
									<a href="/newcoms.php?yep=new_content_comment" target="_blank" id="<?=$id?>" class="red new_files">
