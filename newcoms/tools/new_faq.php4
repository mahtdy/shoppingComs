...
<input type="hidden" value="new_faq" name='yep'>
	<fieldset>
						...
						<div class="clearfix">
							...
							<div class="form-group">
								...
								<?$cat_filter=injection_replace($_GET['cat_filter']);
								 $sql = "SELECT name,id FROM new_modules_cat where type=100";
										//echo $sql;
							...
							<div class="form-group">
								<label class="col-md-3 control-label" for="edit_name">وضعیت</label>
								<div class="col-md-4">
								<?$search_status=injection_replace($_GET['search_status'])?>
									<select name='search_status' class="input-sm form-control"> 
										<option></option>
										<option <?if($search_status==0) echo 'selected';?> value="0"> پیش نویس</option>
...
if($edit_mode=='' && $question>""){
 	...
 	$arr_attach=array("user_id"=>$manager_id,"mudoal_lock"=>$mudoal_lock,"cat_id"=>$cat_id,"site"=>$faq_site,'la'=>$faq_lang ,"status"=>$status,"ip"=>$custom_ip,"question"=>$question,"answer"=>$answer,"date"=>$now);
	$id=insert_to_data_base($arr_attach,'new_faq',$coms_conect);
	show_msg('اطلاعات با موفقیت اضافه گردید');
	
 	
	foreach($arr as $val){
		if($val>""){
 			$label_arr=array("id"=>$id,"type"=>100,"label_id"=>$val);
	...
	$arr_attach=array("edit_user_id"=>$manager_id,"mudoal_lock"=>$mudoal_lock,"cat_id"=>$cat_id,"site"=>$faq_site,'la'=>$faq_lang,"status"=>$status,"ip"=>$custom_ip,"question"=>$question,"answer"=>$answer,"edit_date"=>$now);
	update_data_base($arr_attach,'new_faq',$condition,$coms_conect);
	
     $condition1="id=$edit_mode and type=100";
    delete_from_data_base('new_mudoal_label',$condition1,$coms_conect);
	 foreach($arr as $val){
		if($val>""){
			$labe=array("id"=>$edit_mode,"type"=>100,"label_id"=>$val);
	...
	if($status_filter==0&&$status_filter!='') $status_str=' and status=0';
	...
	<div class="tabbable">
		...
		<div class="secfhead">
			...
			<div class="sectitle">  
				...
				<div class="title"><p class="titr">لیست سوالات متداول</p><p class="lead">توضیحات این بخش</p></div>
			</div>
			<!-- /section:tools/faq.head -->
			<div class="toolmenu">
				<ul>
			...
			<div class="tab-pane <?if($edit_id=='') echo 'active';?>" id="tab1">
								...
								<?}?>
									...
									<div class="form-group yepco">
										<form action="" method="get" class="navbar-form navbar-left pull-right yepco" role="search">
											<input type="hidden" name="yep"  value="new_faq">
				...
				<br><br><br>
					...
					<?		if($lang_filter>0&&!in_array($lang_filter,$_SESSION["manager_title_lang"])){
							...
							if($q>""){
								$str_q="  and(a.answer like '%$q%'  or a.question like '%$q%') ";
								 $manager_q="&q=$q";
							}
						    ...
						    if($publish_search>""){
								...
								$str_expire=" and a.date<='$today_expire' and a.date>='$yesterday_expire'";
							...
							if($search_status>""){
								$status_str=" and a.status='$search_status'";
								$status_cat="&search_status=$search_status";
							}else{
								$status_str='';		
								$status_cat="&search_status=$search_status"; 
							}	
							...
							if(isset($field)&&$field>""){
								...
								switch ($field) {
									 case 0:
										...
										$str_field=" and (a.answer like '%$search_txt%'  or a.question like '%$search_txt%')";
										break; 
									case 1:
							...
							$sql1 = "SELECT count(a.id)as cnt   from new_faq a  
							...
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=new_faq$expire_page$lang_page$site_page$manager_page$manager_q$field_page$expire_page$status_cat");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];						
								
						
				        	$sql12 = "SELECT a.user_id,a.site,a.la,a.mudoal_lock,a.date,a.answer,a.question,a.id,a.status   from new_faq a  
							where a.id>0    $str_site $str_manager $str_lang  $str_q $status_str $str_cat $str_field $str_expire
							order by  a.id desc limit $from,$to";
							 //  echo $sql12; // exit;
							$resultd1 = $coms_conect->query($sql12);$i=1;
							while($row = $resultd1->fetch_assoc()) {
								$id=$row['id'];
								$status=$row['status'];
								if($status==0) $status_str='پیش نویس';
								if($status==1) $status_str='منتشر شده';
                                    ...
                                    <div class="panel-heading container-fluid">
                                        <div class="panel-title col-md-12" style="display: inline-block">
											...
											<div class="col-md-5 col-sm-12 col-xs-12 pull-left" style="float:left;position: relative;padding-left: 0px;font-size: 12px; text-align: left;">
													...
													<?=get_result("select user_name from new_managers where id={$row['user_id']}",$coms_conect);?>
												</span>
												...
												<a class="blue" name="editfaq" id="<?=$id?>" href="newcoms.php?yep=new_faq&edit_id=<?=$id?>">
 ...
 <?=$pgsel?>
			...
			<div class="tab-pane <?if($edit_id) echo 'active';?>" id="add_templates">
					<form class="form-horizontal" id="new_question" name="new_question" action="" role="form" method="post" enctype="multipart/form-data">
					    ...
					    <?if($_SESSION["can_add"]==1){?>
							<a   href="#" data-toggle="tooltip" data-placement="bottom" title="" class="save submit2" data-original-title="ذخیره">
						...
						<?}?>
							<a href="newcoms.php?yep=new_faq" title=""  data-original-title="بازگشت">
...
if($edit_id>""){
	$sql12 = "SELECT mudoal_lock,cat_id,site,la, answer,question,id,status   from new_faq    
	...
	$sql121 = "SELECT b.id   from new_mudoal_label a , new_keyword b  where a.id='$edit_id' and a.label_id=b.id and type=100";
									...
									<div class="row-fluid"> 
										...
										<div class="col-md-12">
											<div class="form-group">
												...
												<div class="form-group col-md-10"> 
													<?$sql = "SELECT name,id FROM new_modules_cat where type='100' and la='$lang_id'";
													  	$result = $coms_conect->query($sql);
...
<?$_SESSION["del_item"]='del_new_faq';
$_SESSION["edit_item"]='edit_new_faq';
?>													
<script src="ajax_js/faq.js"></script>		