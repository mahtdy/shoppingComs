...
if($edit_id==''&&$name>""){ 
	$arr_slide=array('type'=>100,"meta_keyword"=>$meta_keyword,"meta_desciption"=>$meta_desciption,"status"=>$status,"la"=>$manage_lang_filter,"site"=>$manage_site_filter,"date"=>$now,"name"=>$name,"user_id"=>$_SESSION["manager_id"],"ip"=>$custom_ip);
...
<div class="tabbable">
	...
	<div class="msheet tab-content">
		<div class="secfhead">
			...
			<div class="title"><p class="titr">دسته بندی سوالات متداول</p><p class="lead">توضیحات این بخش</p></div>
			</div>
			<!-- /section:tools/q_a_cat.head -->
			<div class="toolmenu">
				<ul>
		...
		<div class="tab-pane active" id="tab1">
			...
			<div class="uploadbts"> 
				...
				<div class="col-md-12" style="padding:0;">
					<div class="col-md-6 text-right" style="padding:0;"><a style="font-size: 18px;">دسته بندی های ساخته شده</a></div>	
				...
				<div class="col-md-12" style="padding:0;"><hr style="margin:5px 0 10px 0"></div> 
					...
					if($_GET['q']>"")
						$sql_str=" and (name like '%$q%' or  meta_desciption like '%$q%' or  meta_keyword like '%$q%' )";
								...
								//$sql = "SELECT * FROM new_modules_cat WHERE   site='$site_filter' $sql_str and type=100 and la='$lang_filter' and user_id=$manager_filter";
											 	
								$sql = "SELECT * FROM new_modules_cat a,new_cat_permission b  WHERE    b.permission=1 $sql_str  and  a.id=b.menu_id and a.parent_id='0' and b.group_id={$_SESSION["manager_group_id"]} and a.type='100' and a.la='$lang_filter' ORDER BY a.rang";
								// echo $sql;exit;
											  ...
											  while($row = $result->fetch_assoc()) {
												...
												echo "<li class='dd-item dd-nodrag' data-id='{$row['id']}'>";
													echo "<div class='dd-handle'><a  class='sr-pull-right' target='_blank' href="."/faq/$lang_filter/cat_id/$id"."> {$row['name']}</a>";
													 ...
													 show_madules_cat($row['id'],$row['site'],$lang_filter, 100 ,$coms_conect,$modul_name);
												echo "</li>\n";
											}
										echo "</ol>\n\n";
									echo "</div>\n";
