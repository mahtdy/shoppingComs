<?$modual_type=7;
$report_url='new_article_reports';
$report_table='new_article';
$report_title=$new_sysmenu[48];
include 'newcoms/main/new_modual_report.php4';
?>
				<div class="toptenrep">
					<h3><span class="flaticon-view32"></span><span class="title">مقالات پربازدید</span></h3>
					...
					<table cellpadding="0" id="new_page_table" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						<tbody>
						 ...
						 <?
						################################################################
							$sql1 = "SELECT count(id) as cnt from new_article a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=7 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id";
							$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
							$page=injection_replace($_GET["page"]);
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=new_article_reports$label_page$is_importand_page$cat_page$expire_page$status_page");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
							$sql12 = "SELECT a.site,a.la,a.title,a.id,a.author,a.publish_date,a.mudoal_lock,a.view,a.status 
							from new_article a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=7 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id
						...
						<tr>
								...
								<td><a href="<?="$domain/article/{$row['title']}/{$row['la']}/".insert_dash($row['title'])?>" target="_blank"><?=$row['title']?></a></td>
			...
			datasets : [
				{
					...
					data : [<?=create_chart_moduale(7,$coms_conect,$mounth,$year,$day,$start,$sql)?>]
				}
			]
