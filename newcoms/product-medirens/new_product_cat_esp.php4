...
//                    $sql = "SELECT * FROM new_modules_cat a,new_cat_permission b  WHERE    b.permission=1 and a.id=b.menu_id and a.parent_id='0' and b.group_id={$_SESSION["manager_group_id"]} and a.type='$modual_type' and a.la='$manage_lang' ORDER BY a.rang";
                    $sql = "SELECT * FROM new_modules_cat   WHERE    type='$modual_type' and parent_id='0' AND  la='fa' ORDER BY rang";

//                    								echo $sql;

//                    								$sssql= $coms_conect->prepare("show databases;");
//                    								$sssql->execute();
//                    								while ($rrrow=$mysql_result->fetch_assoc()){
//                    								    $dbname=$rrrow['database'];
//                    								    echo  $dbname;
//                                                    }
//                                                    exit;
                    $result = $coms_conect->query($sql);
//                    print_r($coms_conect);exit;
