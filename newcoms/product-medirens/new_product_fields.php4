...
//    $first_catid=$catid;
//    $catid1 ='newcoms.php?yep=new_product_fields&catid='. $catid;
//    echo '<input type="hidden" name="catidchng" id="catidchng" value="'.$catid1.' ">';
//}
//
...
$condition_ox4="cat_id='$filter_cat_id'";
delete_from_data_base('new_product_delicated_cat_val',$condition_ox4,$coms_conect);}
...
//	echo $field_id;
    $exist_id = 0;
    $exist_id = get_result("select id from new_product_delicated_cat where id='$field_id'", $coms_conect);
    if ($exist_id > 0) {
        $condition = "id=$exist_id";
        $arr = array("status" => 1);
        update_data_base($arr, 'new_product_delicated_cat', $condition, $coms_conect);
        $field_id = $exist_id;
    }


    if ($filed_title > "") {
        $get_cat_id = get_result("select count(id) from new_product_delicated_cat where id='$field_id'", $coms_conect);
        if ($get_cat_id == 0 && $exist_id == 0) {### add mode
           if ($option_field==0) $field_value='';
            ...
            $id = insert_to_data_base($arr, 'new_product_delicated_cat', $coms_conect);
            if ($option_field == 1) {
            ...
            if ($option_field == 1) {
                ...
                $result11 = $coms_conect->query("select id,value from new_product_delicated_cat_val where cat_id=$cat_id and delicated_id=$field_id");
                ...
                if (count($exist_arr) > 0) {
                    foreach ($exist_arr as $val) {  ##item is delete
                        if (!in_array($val, $del_temp)) {
                            ...
                            update_data_base($arr_slide, 'new_product_delicated_cat_val', $condition, $coms_conect);
                ...
                if (count($temp) > 0) {
                    foreach ($temp as $val) {
                        $arr = array("delicated_id" => $field_id, "cat_id" => $cat_id, 'value' => $val, 'status' => 1);
                        insert_to_data_base($arr, 'new_product_delicated_cat_val', $coms_conect);
                    }
                }
...
<div class="tabbable">
    <div class="msheet tab-content">
        ...
        <div class="tab-pane <? if ($edit_id == "") echo 'active'; else echo ''; ?> " id="tab1">
            <form class="form-horizontal" action="" method="post" name="special" id="special" role="form"
                  enctype="multipart/form-data">
                <div class="panel-body">
                    <? $query = "select name,id from new_modules_cat where   type='4' and status=1 and la='$la' ORDER BY rang";
                    $result = $coms_conect->query($query);
                    $j = 0;
                    echo "<div class='row'><div class='col-md-3'><select name='cat_id' id='cat_id' class='form-control'>";
                    while ($RS = $result->fetch_assoc()) {
                        $str = '';
                        if (($filter_cat_id == $RS['id']) or ($catid == $RS['id']))
                            $str = 'selected';
                        if ($j == 0) $first_catid = $RS['id'];
                        echo "<option $str value='{$RS['id']}'>{$RS['name']}</option>";
                        $j++;
                    ...
                    $query = "select * from new_product_delicated_cat where   cat_id=$first_catid and status=1";
                    $i = 1;
                    $result = $coms_conect->query($query);
                    while ($RS = $result->fetch_assoc()) {
                        ?>
                        ...
                        <div class="panel panel-success" id="field<?= $i ?>">
                            <div class="panel-body">
                                <div class="form-group">
                                    <a data-filedid="<?= $RS['id'] ?>" class="col-md-1 control-label del_add_field"
                                       id="<?= $i ?>">
                                        <i class="fa fa-trash text-danger remove" title=""
                                           data-original-title="حذف"></i>
                                    </a><label class="col-sm-2 control-label"
                                               style="text-align: center;">عنوان <?= $i ?></label>
                                    <div class="form-group col-md-3">
                                        <input value="<?= $RS['name'] ?>" type="text" name="filed_title<?= $i ?>"
                                               id="filed_title<?= $i ?>" class="form-control">
                                    </div>
                                    <label class="col-sm-2 control-label" style="text-align: center;">نوع فیلد</label>
                                    <div class="form-group col-md-3">
                                        <select name="option_field<?= $i ?>" data-option="option_field<?= $i ?>"
                                                data-div="div_<?= $i ?>" class="form-control select<?= $i ?>">
                                            <option value="0" <? if ($RS['type'] == 0) echo 'selected' ?>> متنی</option>
                                            <option value="1" <? if ($RS['type'] == 1) echo 'selected' ?>> انتخابی
                                            </option>
                                        </select>
                                    </div>
                                </div>

                                    <div class="form-group div_<?= $i ?> xblue" <? if ($RS['type'] == 1) echo 'style="display: block"'; else echo 'style="display: none"'; ?>>
                                        <label class="col-md-3 control-label" style="text-align: center;">گزینه ها را
                                            وارد نمایید</label>
                                        <div class="form-group col-md-8">
                                            <input value="<?= $RS['value'] ?>" type="text" name="field_value<?= $i ?>"
                                                   id="field_value<?= $i ?>" class="form-control tagsfield"
                                                   data-role="tagsinput" style="width:100%"
                                                   placeholder="لطفا بعد از وارد کردن هر گزینه Enter بزنید">
                                        </div>
                                    </div>
<!--                                --><?// } ?>
                            </div>
                    ...
                    <script>
                        ...
                        // $('#cat_id').( function() {
                        var catchng = $('#catidchng').val();
                        // alert(catchng);
                        // if(catchng > ''){
                        //         // catchng ='&cat_id='+catchng;
                        //     window.location.href = catchng;
                        //     $('#catidchng').val('');
                        // }
                        // });
                        ...
                        $(document).on('ready', function (e) {
                            ...
                            $(document).on('click', '#add_field', function () {
                                var someText = '<div class="panel panel-success" id="field' + i + '">' +
                                    '<div class="panel-body"><div class="form-group"><a class="col-md-1 control-label del_add_field" id="' + i + '">' +
                                    '<i class="fa fa-trash text-danger remove" title="" data-original-title="حذف"></i></a>' +
                                    '<label class="col-sm-2 control-label" style="text-align: center;">عنوان ' + i + '</label><div class="form-group col-md-3">' +
                                    '<input type="text" name="filed_title' + i + '" id="filed_title' + i + '" class="form-control"></div>' +
                                    '<label class="col-sm-2 control-label" style="text-align: center;">نوع فیلد</label><div class="form-group col-md-3">' +
                                    '<select name="option_field' + i + '" data-option="option_field' + i + '"  data-div="div_' + i + '" class="form-control select' + i + '">' +
                                    '<option value="0"> متنی</option> <option value="1"> انتخابی</option> </select></div></div>' +
                                    '<div style="display:none" class="form-group div_' + i + ' xblue">' +
                                    '<label class="col-md-3 control-label" style="text-align: center;">گزینه ها را وارد نمایید</label><div  class="form-group col-md-8">' +
                                    '<input type="text" name="field_value' + i + '" id="field_value' + i + '" class="form-control tagsfield" data-role="tagsinput" style="width:100%" placeholder="لطفا بعد از وارد کردن هر گزینه Enter بزنید">' +
                                    '</div></div></div></div>';
                            ...
                            $('select').change(function () {  ////show hide select
                                ...
                                if ($(this).val() == 1) {
                                   ...
                                   // alert( $('.' +$(this).data('input')).val());
                                    $('.' + $(this).data('div')).hide();
                                    // $('.tagsfield').val('');
                                    // $('.' + $(this).data('div') ).find('input').attr('value','');
                                    // $('.' + $(this).data('div') ).find('span').remove();
                                }
                                $('.tagsfield').tagsinput('items');
                                // $('#field_value4').focus();
                            });
                        ...
                        // if ($('select').val() == 1) {
                        //     $('.' + $('select').data('div')).show();
                        // } else {
                        //
                        //     // $('#' +$(this).data('option')).val('');
                        //     $('.' + $('select').data('div')).hide();
                        // }
                        // $('.tagsfield').tagsinput('items');

                    </script>

                    <a class="btn btn-success block" id="add_field" style="width: 100%;"><span
                                class="fa fa-plus"></span> افزودن فیلد بیشتر </a>
                     
