	...
	<div class="tabbable">
		...
		<div class="msheet tab-content">
			...
			<div class="sectitle">
				...
				<div class="title"><p class="titr">آرشیو SMS ارسالی</p><p class="lead">توضیحات این بخش</p></div>
			</div>
			<!-- /section:sms/sendsms.head -->
			<div class="toolmenu">
				<ul>
			...
			<div class="tab-pane <?if($edit_id=="")echo 'active'?> " id="tab1">
				...
				<!-- #section:sms/sendsms.table -->
					...
					<table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						if($q>""){
						$str="and (b.sms_number like '%$q%' or b.mobile like '%$q%' or b.text like '%$q%' or a.user_name like '%$q%' or a.name like '%$q%')";	
							
						}
					  	$sql1 = "SELECT count(a.id) as cnt from new_sms_archive b,new_managers a where b.manager_id= $manager_id and a.id=b.manager_id  $str_lang $str_manager	  $str order by a.id desc";
						
							$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
							$page=injection_replace($_GET["page"]);
							
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=new_sendsms$lang_page$site_page$manager_page");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
							$query="select * from new_sms_archive  b,new_managers a where b.manager_id= $manager_id and a.id=b.manager_id  $str_lang	$str_manager   $str LIMIT $from,$to"; $i=1;
							...
							$result = $coms_conect->query($query);
						 	...
						 	if(count($tempp)>0)	
							$temp=get_devivery($coms_conect,$tempp);
						
						
						$query="select * from new_sms_archive b,new_managers a where b.manager_id= $manager_id  and a.id=b.manager_id  $str_lang	$str_manager $site_page $str LIMIT $from,$to"; $i=1;
						// echo $query;
							$result = $coms_conect->query($query);$j=0;
								while($RS1 = $result->fetch_assoc()) {
									$condition="id={$RS1['id']}";
									$arr_imag=array("status"=>$temp[$j]);
									update_data_base($arr_imag,'new_sms_archive',$condition,$coms_conect);
							...
							<tr>
								...
								<td><?=$RS1['sms_number']?></td>
								<td><?=$RS1['mobile']?></td>
								<td><?=miladitojalalitime(date('Y-m-d H:i:s',$RS1['date']))?></td>
								<td><?=$RS1['site_id']?></td>
								<td><?=$RS1['la']?></td>
								<td><?=resolve_sms_status($temp[$j])?></td>
								<td>
									<a id="<?=$RS1['text']?>" class="show_item green" data-target="#details" data-placement="top" rel="tooltip" data-toggle="modal" title="مشاهده متن پیام" style="font-size: 15px !important;">
