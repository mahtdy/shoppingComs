<?$modual_type=8;
$report_url='new_video_reports';
$report_table='new_video';
$report_title=$new_sysmenu[53];
include 'newcoms/main/new_modual_report.php4';
?>
				<div class="toptenrep">
					<h3><span class="flaticon-view32"></span><span class="title">ویدئوهای پربازدید</span></h3>
			...
			<div class="row" style="background-color: #fff;margin-right: 0;margin-left: 0;">
				<div class="col-md-12 vitems">
					<div class="row">
						 ...
						 <?
						################################################################
							$sql1 = "SELECT count(id) as cnt from new_video a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=8 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id";
							$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
							$page=injection_replace($_GET["page"]);
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=new_video_reports$label_page$is_importand_page$cat_page$expire_page$status_page");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
							$sql12 = "SELECT a.duration,a.site,a.la,a.title,a.id,a.publish_date ,a.mudoal_lock,a.view,a.status 
							from new_video a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=8 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id
							...
							<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12 item">
												<div id="<?if($row['status']==1)echo 'active';else echo 'deactive'?>" class="ic">
													<div class="vl">
														...
														<img src="<?=get_modual_address($row1['adress'])?>">
													</div>
													<div class="tools">
														<div class="vi">
														...
														<div class="ticons">
															<a target="_blank" href="/newcoms.php?yep=new_video&edit_id=<?=$row['id']?>" data-placement="bottom" title="ویرایش"><span class="edit flaticon-note32"></span></a>
			...
			datasets : [
				{
					...
					data : [<?=create_chart_moduale(8,$coms_conect,$mounth,$year,$day,$start,$sql)?>]
				}
			]
