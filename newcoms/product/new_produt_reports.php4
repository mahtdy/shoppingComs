<?$modual_type=4;
$report_url='news_produt_reports';
$report_table='new_product';
$report_title=$new_sysmenu[161];
include 'newcoms/main/new_modual_report.php4';
?>
			<div class="toptenrep">
					<h3><span class="flaticon-view32"></span><span class="title">اخبار پربازدید</span></h3>
					<table cellpadding="0" id="new_page_table" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						<tbody>
						 ...
						 <?
						################################################################
							$sql1 = "SELECT count(id) as cnt from new_product a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=4 
							...
							from new_product a ,new_modules_catogory b   where a.id>0 and a.id=b.page_id and b.type=4 
							$str_expire $str_cat $str_status $str_is_importand  
							group by b.page_id
				...
				<tr>
							<td class="center"><?=$RS1["id"]?></td>
								...
								<td>
									...
									<a id="<?=$id?>" class="edit_menu blue"  href="newcoms.php?yep=new_producttext&edit_id=<?=$id?>" style="font-size: 15px !important;">
									...
									<?}?>
									<?/*?>
									<a href="/newcoms.php?yep=new_product_comment" target="_blank" id="<?=$id?>" class="green new_scomments">
										<i class="ace-icon fa fa-comments bigger-120" title="نظرات"></i>
									</a>	
									<a href="/newcoms.php?yep=new_product_comment" target="_blank" id="<?=$id?>" class=" new_movie" data-title="show_video" data-toggle="modal" data-target="#show_video" data-placement="top" rel="tooltip" style="font-size: 15px !important;">
										<i class="ace-icon fa fa-film bigger-120" title="فیلم"></i>
									</a>
									<a href="/newcoms.php?yep=new_product_comment" target="_blank" id="<?=$id?>" class="green new_movie">
										<i class="ace-icon fa fa-picture-o bigger-120" title="البوم عکس"></i>
									</a>
									<a href="/newcoms.php?yep=new_product_comment" target="_blank" id="<?=$id?>" class="blue new_voice">
										<i class="ace-icon fa fa-volume-up bigger-120" title="صدا"></i>
									</a>
									<a href="/newcoms.php?yep=new_product_comment" target="_blank" id="<?=$id?>" class="red new_files">
