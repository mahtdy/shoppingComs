...
<!--<script src="/yep_theme/default/rtl/css/product_manager_anbar/jquery.min.js"></script>-->
...
<!--<script src="/yep_theme/default/rtl/css/product_manager_anbar/bootstrap.min.js"></script>-->
...
if ($edit_mode > "") {
    $temp_user = get_result("select user_id from new_product_manager_anbar where id= $edit_mode", $coms_conect);
...
if ($edit_id > "") {
    $temp_user = get_result("select user_id from new_product_manager_anbar where id= $edit_id", $coms_conect);
//    if (!in_array($temp_user, $_SESSION['manager_user_permisson']))
//        header('Location: new_manager_signout.php');
}
...
//$product_manager_anbar_type = $fori . $tasviri . $videoi . $soti . $erjaei;
...
//if ($upload_type == 2)
//    $product_manager_anbar_image = injection_replace($_POST['product_manager_anbar_image']);
//
//else if ($upload_type == 1) {
//    $product_manager_anbar_image_avatar = injection_replace($_POST['product_manager_anbar_image_avatar'][0]);
//    $product_manager_anbar_image_avatar = trim($product_manager_anbar_image_avatar);
//    if ($product_manager_anbar_image_avatar != '')
//        $product_manager_anbar_image = 'http://' . $_SERVER["HTTP_HOST"] . '/new_gallery/files/' . $product_manager_anbar_image_avatar;
//    else
//        $product_manager_anbar_image = '';
//}

//echo  $product_manager_anbar_image.'wew';
$product_manager_anbar_video_pic = injection_replace($_POST['product_manager_anbar_video_pic']);
$product_manager_anbar_attach = injection_replace($_POST['product_manager_anbar_attach']);
$product_manager_anbar_sound = injection_replace($_POST['product_manager_anbar_sound']);
$product_manager_anbar_video = injection_replace($_POST['product_manager_anbar_video']);

$album_type = injection_replace($_POST['album_type']);
if ($album_type == 2) {
    $imagelist = ($_POST['imagelist']);
    foreach ($_POST['product_manager_anbar_image_album'] as $val) {
        unlink('new_gallery/files/' . $val);
        unlink('new_gallery/files/tn' . $val);
    }

} else if ($album_type == 1 && count($_POST['product_manager_anbar_image_album']) > 0) {
    foreach ($_POST['product_manager_anbar_image_album'] as $val)
...
$totla_related_manager_anbar = injection_replace($_POST['totla_related_manager_anbar']);
$spesial_date = jdate('Y/m/d');

#اطلاعات مدیر انبار ها
$manager_name = injection_replace($_POST['product_manager_anbar_manager_name']);
$boss_name = injection_replace($_POST['product_manager_anbar_boss_name']);
$tech_name = injection_replace($_POST['product_manager_anbar_technical_name']);
$latin_name = injection_replace($_POST['product_manager_anbar_latin_name']);
$num_sabt = injection_replace($_POST['product_manager_anbar_num_sabt']);
$num_code = injection_replace($_POST['product_manager_anbar_num_code']);
$com_type = injection_replace($_POST['product_manager_anbar_type']);
$sabt_date = injection_replace($_POST['sabt_date']);
$city = injection_replace($_POST['product_manager_anbar_city']);
$zipcode = injection_replace($_POST['product_manager_anbar_zipcode']);
$com_adres = injection_replace($_POST['product_manager_anbar_address']);
$com_adres_site = injection_replace($_POST['product_manager_anbar_address_site']);

//$img_email = injection_replace($_POST['img_email']);

$com_fax = injection_replace($_POST['product_manager_anbar_fax']);
...
$check_checked_manager_anbar = injection_replace($_POST['check_checked_representation']);
$chkbx_link = injection_replace($_POST['chkbx_link']);
$state = injection_replace($_POST['product_manager_anbar_state']);
$com_social_link1 = injection_replace($_POST['product_manager_anbar_social_link1']);
$com_social_link2 = injection_replace($_POST['product_manager_anbar_social_link2']);
$com_social_link3 = injection_replace($_POST['product_manager_anbar_social_link3']);
$com_social_link4 = injection_replace($_POST['product_manager_anbar_social_link4']);



$manager_anbar_tel1 = injection_replace($_POST['manager_anbar_tel1']);
$manager_anbar_tel2 = injection_replace($_POST['manager_anbar_tel2']);
$manager_anbar_tel3 = injection_replace($_POST['manager_anbar_tel3']);
$product_manager_anbar_email = injection_replace($_POST['product_manager_anbar_email']);
$manager_anbar_unit_city = injection_replace($_POST['manager_anbar_unit_city']);
$manager_anbar_city = injection_replace($_POST['manager_anbar_city']);
$manager_anbar_address = injection_replace($_POST['manager_anbar_address']);
$manager_anbar_zip = injection_replace($_POST['manager_anbar_zip']);
//echo $check_checked;
//$phone_phone = injection_replace($_POST['product_manager_anbar_ph_links_title1']);
...
//====== تبدیل ایمیل به عکس invert email to jpg ========//
    if (isset($_POST['product_manager_anbar_email']) and $edit_mode == '') {
//        echo 'sdsd';
        $len = strlen($product_manager_anbar_email);
        $width = ($len + 2) * 7;
...
//    $font = 'http://' . $_SERVER["HTTP_HOST"] .'/yep_theme/default/rtl/images/email/arial.ttf';
        ...
        imagettftext($im, 10, 0, 5, 15, $black, $font, $product_manager_anbar_email);

// Create image in folder
        $rnd = rand(0, 9999);

        $product_manager_anbar_img_email = 'yep_theme/default/rtl/images/email/' . 'img-email' . $rnd . '.jpg';
//        echo 'rr='.$image_email.'rnd='.$rnd;exit;
        imagejpeg($im, $product_manager_anbar_img_email);
        imagedestroy($im);
...
//=======end invert email to jpg
    ...
    $arr = array("mngr_anbr_mob1" => $manager_anbar_tel1,"mngr_anbr_mob2" => $manager_anbar_tel2,"mngr_anbr_tel" => $manager_anbar_tel3,"mngr_anbr_email" => $product_manager_anbar_email,"mngr_anbr_img_email" => $product_manager_anbar_img_email,"mngr_anbr_unit_city" => $manager_anbar_unit_city,"mngr_anbr_city" => $manager_anbar_city,"mngr_anbr_address" => $manager_anbar_address,"mngr_anbr_zip" => $manager_anbar_zip,"name_url" => $name_url, "upload_type_nav" => $upload_type_nav, "navication_pic" => $navication_pic, "check_link" => $chkbx_link, "upload_type" => $upload_type, 'hashtag_key' => $meta_label1, "seo_title" => $seo_title, "source_link_product" => $source_link_product, "source_product" => $source_product, "name" => $name, "title" => $title, "text" => $text, "meta_keyword" => $meta_keyword, "meta_description" => $meta_desciption, "abstract" => $abstract, "user_id" => $user_id, "la" => $manage_lang, "site" => $manage_site, "status" => $status);
    $id = insert_to_data_base($arr, 'new_product_manager_anbar', $coms_conect);
//    print_r($arr);
    $id_com = $coms_conect->insert_id;
//echo 'idc='.$id_com;


//    echo  $product_manager_anbar_image;
//    $arr_imag = array("id" => $id, "type" => 4, "adress" => $product_manager_anbar_image, "name" => 'product_manager_anbar_image');
//    insert_to_data_base($arr_imag, 'new_file_path', $coms_conect);
////    echo  $product_manager_anbar_image.'qq';
//    $arr_attach = array("id" => $id, "type" => 4, "adress" => $product_manager_anbar_attach, "name" => 'product_manager_anbar_attach');
//    insert_to_data_base($arr_attach, 'new_file_path', $coms_conect);
//
//
//    $arr_sound = array("id" => $id, "type" => 4, "adress" => $product_manager_anbar_sound, "name" => 'product_manager_anbar_sound');
//    insert_to_data_base($arr_sound, 'new_file_path', $coms_conect);
//
//    $arr_video = array("id" => $id, "type" => 4, "adress" => $product_manager_anbar_video, "name" => 'product_manager_anbar_video');
//    insert_to_data_base($arr_video, 'new_file_path', $coms_conect);
//
//    $arr_video = array("id" => $id, "type" => 4, "adress" => $product_manager_anbar_video_pic, "name" => 'product_manager_anbar_video_pic');
//    insert_to_data_base($arr_video, 'new_file_path', $coms_conect);
...
//            if ($image != -1) {
//                $gallery_arr = array("id" => $id, "type" => 4, "adress" => $image, "name" => 'product_manager_anbar_gallery');
...
//    }
//
//    $related_manager_anbar = explode(",", $totla_related_manager_anbar);
//    foreach ($related_manager_anbar as $value_manager_anbar) {
//
//        $arr = array("id" => $value_manager_anbar, "page_id" => $id_com, "type" => 4);
//        insert_to_data_base($arr, 'new_product_manager_anbar_related', $coms_conect);
//    }
//
//    ### slide show add
//
//    if ($slide == 1) {
//        $slide_link = "/product_manager_anbar/$manage_lang/$id/" . insert_dash("$name");
...
//====== تبدیل ایمیل به عکس invert email to jpg ========//
    ...
    $query = "SELECT mngr_anbr_img_email FROM new_product_manager_anbar where id='$edit_id'";
...
//    $font = 'http://' . $_SERVER["HTTP_HOST"] .'/yep_theme/default/rtl/images/email/arial.ttf';
    $font = 'yep_theme/default/rtl/images/email/arial.ttf';
    imagettftext($im, 10, 0, 5, 15, $black, $font, $product_manager_anbar_email);
    $product_manager_anbar_img_email = $row['mngr_anbr_img_email'];
// Create image in folder
//    echo 'imemail = '.$img_email.'<br>';
    if ($product_manager_anbar_img_email == '') {
        $rnd = rand(0, 9999);
        $product_manager_anbar_img_email = 'yep_theme/default/rtl/images/email/' . 'img-email' . $rnd . '.jpg';
    }
...
//=======end invert email to jpg
    ...
    $arr = array("mngr_anbr_mob1" => $manager_anbar_tel1,"mngr_anbr_mob2" => $manager_anbar_tel2,"mngr_anbr_tel" => $manager_anbar_tel3,"mngr_anbr_email" => $product_manager_anbar_email,"mngr_anbr_img_email" => $product_manager_anbar_img_email,"mngr_anbr_unit_city" => $manager_anbar_unit_city,"mngr_anbr_city" => $manager_anbar_city,"mngr_anbr_address" => $manager_anbar_address,"mngr_anbr_zip" => $manager_anbar_zip,"name_url" => $name_url, "upload_type_nav" => $upload_type_nav, "navication_pic" => $navication_pic, "check_link" => $chkbx_link, "upload_type" => $upload_type, 'hashtag_key' => $meta_label1, "seo_title" => $seo_title, "source_link_product" => $source_link_product, "source_product" => $source_product, "name" => $name, "title" => $title, "text" => $text, "meta_keyword" => $meta_keyword, "meta_description" => $meta_desciption, "abstract" => $abstract, "user_id" => $user_id, "la" => $manage_lang, "site" => $manage_site, "status" => $status);
    update_data_base($arr, 'new_product_manager_anbar', $condition, $coms_conect);
    $id_pro = $edit_id;

//
    $condition = "id=$edit_mode && name='product_manager_anbar_image'";
    $arr_imag = array("id" => $edit_mode, "type" => 4, "adress" => $product_manager_anbar_image, "name" => 'product_manager_anbar_image');
    update_data_base($arr_imag, 'new_file_path', $condition, $coms_conect);
//    echo  $product_manager_anbar_image.'ee';
//
//    $condition = "id=$edit_mode && name='product_manager_anbar_attach'";
//    $arr_attach = array("id" => $edit_mode, "type" => 4, "adress" => $product_manager_anbar_attach, "name" => 'product_manager_anbar_attach');
//    update_data_base($arr_attach, 'new_file_path', $condition, $coms_conect);
//
//
//    $condition = "id=$edit_mode && name='product_manager_anbar_sound'";
//    $arr_sound = array("id" => $edit_mode, "type" => 4, "adress" => $product_manager_anbar_sound, "name" => 'product_manager_anbar_sound');
//    update_data_base($arr_sound, 'new_file_path', $condition, $coms_conect);
//
//    $condition = "id=$edit_mode && name='product_manager_anbar_video'";
//    $arr_video = array("id" => $edit_mode, "type" => 4, "adress" => $product_manager_anbar_video, "name" => 'product_manager_anbar_video');
//    update_data_base($arr_video, 'new_file_path', $condition, $coms_conect);
//
//    $condition = "id=$edit_mode && name='product_manager_anbar_video_pic'";
//    $arr_video = array("id" => $edit_mode, "type" => 4, "adress" => $product_manager_anbar_video_pic, "name" => 'product_manager_anbar_video_pic');
//    update_data_base($arr_video, 'new_file_path', $condition, $coms_conect);
//
...
//    ##edit gallery
//    $query1 = "delete from new_file_path where id='$edit_mode' && name='product_manager_anbar_gallery' and type=4";
//    $coms_conect->query($query1);
//    if (!empty($imagelist)) {
//        foreach ($imagelist as $image) {
//            if ($image != -1) {
//                $gallery_arr = array("id" => $edit_mode, "type" => 4, "adress" => $image, "name" => 'product_manager_anbar_gallery');
//                insert_to_data_base($gallery_arr, 'new_file_path', $coms_conect);
//            }
//        }
//    }
...
//        insert_to_data_base($arr, 'new_related_madual', $coms_conect);
//    }
//    $query111 = "delete from new_product_manager_anbar_related where page_id='$edit_mode' and type=4";
//    $coms_conect->query($query111);
//    $related_manager_anbar = explode(",", $totla_related_manager_anbar);
//    foreach ($related_manager_anbar as $value_manager_anbar) {
//        $arr = array("id" => $value_manager_anbar, "page_id" => $edit_mode, "type" => 4);
//        insert_to_data_base($arr, 'new_product_manager_anbar_related', $coms_conect);
//    }
//    ### slide show update
//    if ($slide == 1) {
//        $slide_link = "/product_manager_anbar/$manage_lang/$edit_mode/" . insert_dash("$name");
...
if ($edit_id > "") {
    $query = "SELECT * FROM new_product_manager_anbar where id='$edit_id'";
    $result = $coms_conect->query($query);
//    print_r($query);
...
//    $navication_pic = $row['navication_pic'];
    ...
    $manager_anbar_tel1 = $row['mngr_anbr_mob1'];
    $manager_anbar_tel2 = $row['mngr_anbr_mob2'];
    $manager_anbar_tel3 = $row['mngr_anbr_tel'];
    $product_manager_anbar_email = $row['mngr_anbr_email'];
    $product_manager_anbar_img_email = $row['mngr_anbr_img_email'];
    $manager_anbar_unit_city = $row['mngr_anbr_unit_city'];
    $manager_anbar_city = $row['mngr_anbr_city'];
    $manager_anbar_address = $row['mngr_anbr_address'];
    $manager_anbar_zip = $row['mngr_anbr_zip'];
//    $com_social_link4 = $row['social_link4'];
...
//    echo $phone_phone.$phone_name;
    ...
    $slide = $row['slide'];
//    $product_manager_anbar_type = $row['content_type'];
//    $temp_1 = str_split($product_manager_anbar_type);
//    $fori = $temp_1[0];
//    $tasviri = $temp_1[1];
//    $videoi = $temp_1[2];
//    $soti = $temp_1[3];
...
//    $text3 = $row['text3'];
    ...
    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_manager_anbar_image'  and type=4";
    $result = $coms_conect->query($query);
    $row = $result->fetch_assoc();
    $product_manager_anbar_image = $row['adress'];
//    echo $product_manager_anbar_image;
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_manager_anbar_attach'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_manager_anbar_attach = $row['adress'];
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_manager_anbar_sound'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_manager_anbar_sound = $row['adress'];
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_manager_anbar_video'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_manager_anbar_video = $row['adress'];
//
//    $query = "SELECT b.id,title FROM new_related_madual a ,new_product_manager_anbar b where page_id='$edit_id' and a.id=b.id and type=4";
//    $result = $coms_conect->query($query);
////    echo $query;exit;
//    $totla_related = "";
//    $i = 1;
...
//    }
//
//    $query1b = "SELECT b.id,name FROM new_product_manager_anbar_related a ,new_product_manager_anbar_cat_manager_anbar b where page_id='$edit_id' and a.id=b.id and a.type=4";
//    $resultb = $coms_conect->query($query1b);
//    $totla_related_manager_anbar = "";
////    echo $query1b;exit;
//    $i = 1;
//    while ($row = $resultb->fetch_assoc()) {
//        if ($i != 1) $str = ',';
//        $i++;
//        $totla_related_manager_anbar .= $str . $row['id'];
//    }
//
...
////echo 'val-i='.$val_i;
//for ($j = 1; $j <= $val_i; $j++) {
//    $phone_name = injection_replace($_POST["product_manager_anbar_na_links_title{$j}"]);
//    $phone_phone = injection_replace($_POST["product_manager_anbar_ph_links_title{$j}"]);
//    $phone_id = injection_replace($_POST["product_manager_anbar_ph_id{$j}"]);
//    $phone_id_com = injection_replace($_POST["product_manager_anbar_ph_id_com{$j}"]);
////echo 'pic='.$phone_id_com.'<br>';
//    if (empty($phone_id_com) && !empty($_POST["product_manager_anbar_ph_links_title{$j}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_phone = array("phone_name" => $phone_name, "phone_phone" => $phone_phone, "id_com" => $id_com);
////       print_r($arr_phone);
//        insert_to_data_base($arr_phone, 'new_product_manager_anbar_phone', $coms_conect);
//    } else {
//
//        $condition = "id=$phone_id";
//        $arr_phone = array("phone_name" => $phone_name, "phone_phone" => $phone_phone, "id_com" => $phone_id_com);
//        update_data_base($arr_phone, 'new_product_manager_anbar_phone', $condition, $coms_conect);
//
//    }
//}
...
//    $slide_esp_id_com = injection_replace($_POST["product_manager_anbar_slide_esp_id_com{$j}"]);
//
//
...
//        insert_to_data_base($arr_slide_esp, 'new_product_manager_anbar_slide_esp', $coms_conect);
//    } else {
//
...
//        update_data_base($arr_slide_esp, 'new_product_manager_anbar_slide_esp', $condition, $coms_conect);
//
//    }
...
////echo '$val_z='.$val_z;
//for ($z = 1; $z <= $val_z; $z++) {
//    $product_manager_anbar_adres_onvan = injection_replace($_POST["product_manager_anbar_adres_onvan{$z}"]);
//    $product_manager_anbar_adres_state = injection_replace($_POST["product_manager_anbar_adres_state{$z}"]);
//    $product_manager_anbar_adres_city = injection_replace($_POST["product_manager_anbar_adres_city{$z}"]);
//    $product_manager_anbar_adres_zip = injection_replace($_POST["product_manager_anbar_adres_zip{$z}"]);
//    $product_manager_anbar_adres_adres = injection_replace($_POST["product_manager_anbar_adres_adres{$z}"]);
//    $adres_id = injection_replace($_POST["product_manager_anbar_ad_id{$z}"]);
//    $adres_id_com = injection_replace($_POST["product_manager_anbar_ad_id_com{$z}"]);
////echo 'pic='.$adres_id_com.'<br>';
////echo 'pi='.$adres_id.'<br>';
//    if (empty($adres_id_com) && !empty($_POST["product_manager_anbar_adres_adres{$z}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_adres = array("onvan_adres" => $product_manager_anbar_adres_onvan, "state_adres" => $product_manager_anbar_adres_state, "city_adres" => $product_manager_anbar_adres_city, "adres_adres" => $product_manager_anbar_adres_adres, "zip_adres" => $product_manager_anbar_adres_zip, "id_com" => $id_com);
////       print_r($arr_phone);
//        insert_to_data_base($arr_adres, 'new_product_manager_anbar_adres', $coms_conect);
//    } else {
//
//        $condition = "id=$adres_id";
//        $arr_adres = array("onvan_adres" => $product_manager_anbar_adres_onvan, "state_adres" => $product_manager_anbar_adres_state, "city_adres" => $product_manager_anbar_adres_city, "adres_adres" => $product_manager_anbar_adres_adres, "zip_adres" => $product_manager_anbar_adres_zip, "id_com" => $adres_id_com);
////       print_r($arr_adres);
//        update_data_base($arr_adres, 'new_product_manager_anbar_adres', $condition, $coms_conect);
//
//    }
//}
...
//
//
//########################################################### شروع ثبت مدیر انبار ها START insert update manager_anbar   ########################################################
////
////echo 'val-w='.$val_w;
//for ($j = 1; $j <= $val_w; $j++) {
...
//    $manager_anbar_id = injection_replace($_POST["product_manager_anbar_dis_id{$j}"]);
//    $manager_anbar_id_com = injection_replace($_POST["product_manager_anbar_dis_id_com{$j}"]);
////echo 'pic='.$phone_id_com.'<br>';
//    if (empty($manager_anbar_id_com) && !empty($_POST["tab_distributors_links_title{$j}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_manager_anbar = array("com_tozi_name" => $com_tozi_name, "com_tozi_type" => $com_tozi_type, "id_com" => $id_com);
////       print_r($arr_manager_anbar);
//        insert_to_data_base($arr_manager_anbar, 'new_product_manager_anbar', $coms_conect);
//    } else {
//
//        $condition = "id=$manager_anbar_id";
//        $arr_manager_anbar = array("com_tozi_name" => $com_tozi_name, "com_tozi_type" => $com_tozi_type, "id_com" => $manager_anbar_id_com);
//        update_data_base($arr_manager_anbar, 'new_product_manager_anbar', $condition, $coms_conect);
//
//    }
//}
//########################################################### پایان ثبت مدیر انبار ها  END insert update manager_anbar  ####################################################


//print_r($totla_related_manager_anbar);
##################################
?>
<div class="modal fade" id="del_reated_product_manager_anbar" tabindex="-1" role="dialog" aria-labelledby="edit"
     aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="alert alert-warning"><span class="glyphicon glyphicon-warning-sign"></span> آیا از حذف این
                    مدیر انبار اطمینان دارید؟
                </div>
            </div>
            <div class="modal-footer ">
                <button type="button" id="btn_del_related_product_manager_anbar" data-dismiss="modal"
...
<div class="modal fade" id="del_reated_product_manager_anbar_representation" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer ">
                <button type="button" id="btn_del_related_product_manager_anbar_representation" data-dismiss="modal"
                        class="btn btn-warning"><span
                            class="glyphicon glyphicon-ok-sign"></span>&nbsp;بلی
                </button>
                <button type="button" data-dismiss="modal" class="btn btn-warning"><span
...
<form class="form-horizontal" id="content_search" name="content_search" action="" role="form" method="get"
    ...
    <input type="hidden" value="new_product_manager_anbar" name='yep'>
    <fieldset>
...
<fieldset>
    <div class="modal fade" id="show_product_manager_anbar" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer ">
                    <button type="button" id="btn_ok_product_manager_anbar00" value="" data-dismiss="modal"
            ...
            <div class="collapse navbar-collapse" id="nav-collapse">
                <ul class="nav navbar-nav navbar-left">
                    <? if ($_SESSION['can_add'] == 1) { ?>    <a data-toggle="tab" href="#add_templates"
                        ...
                        افزودن </a><? } ?>
                    <? if ($_GET['manager_filter'] > "") { ?>    <a href="/newcoms.php?yep=new_product_manager_anbar"
                                                                    style="background-color: #2a8bcb!important;border-color: #2a8bcb;color: white;padding: 4px;">
    ...
    <div class="msheet tab-content">
        <div class="secfhead">
            ...
            <div class="sectitle">
                ...
                <div class="title"><p class="titr">لیست مدیر انبار ها</p>
                    <p class="lead">امکان مدیریت و افزودن و ویرایش کردن انبار ها در این قسمت فراهم شده است.</p>
                </div>
            </div>
            <!-- /section:content/contenttext.head -->
            <div class="toolmenu">
        ...
        <div class="tab-pane <? if ($edit_id == "") echo 'active' ?>" id="" style="background-color: #EFF3F8;">
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="form-group yepco">
                            <form action="" method="get" class="navbar-form navbar-left pull-right yepco" role="search">
                                <input type="hidden" name="yep" value="new_product_manager_anbar">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered"
                ...
                <tr>
                    ...
                    <th>ردیف</th>
                    <th width="15%">نام مدیر انبار</th>

                    <!--                    <th>ایمیل</th>-->
                    <th>آدرس سایت</th>
                    <!--                    <th>نوع مدیر انبار </th>-->
                ...
                // $str_cat
                $sql1 = "select count(a)as cnt from (SELECT count(a.id) as a from new_product_manager_anbar a  where la='$la' )q";
                //                  echo $sql1; exit;
                $result = $coms_conect->query($sql1);
                $RS = $result->fetch_assoc();

                $page = injection_replace($_GET["page"]);
                $a = pgsel((int)$page, $RS["cnt"], 10, 10, "$root/newcoms.php?yep=new_product_manager_anbar$lang_page$site_page$manager_page$status_page$manager_q$field_page$expire_page$publish_page$cat_page$id_number_page");
                $from = $a[0];
                $to = $a[1];
                $pgsel = $a[2];
                $query = "SELECT a.site,a.id,a.la,a.title,a.name,a.status FROM new_product_manager_anbar a
                ...
                while ($RS1 = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><a target="_blank"
                               href="/<?= "$domain/product_manager_anbar/{$RS1['la']}/{$RS1['id']}/" . insert_dash($RS1['title']) ?>"><?= $RS1["title"] ?></a>
                        </td>
                        ...
                        <td>
                            ...
                            <? if ($_SESSION["can_edit"] == 1) {
                                ...
                                <a id="<?= $id ?>" class="edit_menu blue icon"
                                   href="newcoms.php?yep=new_product_manager_anbar&edit_id=<?= $id ?>">
                            ...
                            if ($_SESSION["can_delete"] == 1) {
                                ...
                                <a href="/<?= "$domain/product_manager_anbar/{$RS1['la']}/{$RS1['id']}/" . insert_dash($RS1['name']) ?>"
        ...
        <div class="tab-pane <? if ($edit_id != "" || $add_new == 1) echo 'active'; ?>" id="add_templates">
            ...
            <form id="contenttext" name="contenttext" class="" action="" role="form" method="post"
                ...
                <div id="id-message-new-navbar" class="message-navbar clearfix">
                    ...
                    <? } ?>
                    <a href="newcoms.php?yep=new_product_manager_anbar" data-toggle="tooltip" data-placement="bottom"
                       title=""
                        ...
                        <ul class="nav nav-tabs" id="myTab">
                            <li>
                                <a data-toggle="tab" href="#home">
                                    <p class="flaticon-file23">ثبت مدیر انبار </p>
                            ...
                            <li>
                                <a data-toggle="tab" href="#product_manager_anbar_info">
                                    <p class="flaticon-copy23"></p>
                                    اطلاعات مدیر انبار
                                </a>
                            </li>
                            <!--                            <li>-->
                            <!--                                <a data-toggle="tab" href="#product_manager_anbar_boss">-->
                            <!--                                    <p class="flaticon-copy23"></p>-->
                            <!--                                    اطلاعات مدیر مدیر انبار-->
                            <!--                                </a>-->
                            <!--                            </li>-->
                            ...
                            <!--                            </li>-->
                            <!--                            <li>-->
                            <!--                                <a data-toggle="tab" href="#tab_product_manager_anbars">-->
                            <!--                                    <p class="flaticon-copy23"></p>-->
                            <!--                                    محصولات-->
                            <!--                                </a>-->
                            <!--                            </li>-->
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-6">
                                                        <label class="control-label">نام مدیر انبار </label>
...
<!---->
<!--                                                </div>-->
<!--                                                <input type="button" id="colorsize">-->
<!--                                                <script>-->
<!--                                                    $('#colorsize').click(function () {-->
<!--                                                        // alert('hi');-->
<!--                                                        $.ajax({-->
<!--                                                            type: 'POST',-->
<!--                                                            url: 'New_ajax_product.php',-->
<!--                                                            data: "action=testcolorcolor",-->
...
<!--###################################################                                                -->
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-6">
                                                        <label class="control-label">آدرس سایت مدیر انبار </label>
                                                ...
                                                <div class="form-group" id="oracup">
                                                    ...
                                                    <div class="ui-sortable red_nav box_nav" id="upload_type1_nav"
                                                        ...
                                                        <? if ($edit_id > "" && $upload_type_nav == 1) {
                                                            $query_nav = "select navication_pic from new_product_manager_anbar where id='$edit_id' ";
...
//                                                       $pic_str=explode(" ",$pic_str);echo 'aa='.$product_image_avatar;
                                            ...
                                            <div class="form-group">
                                                <div class="form-group col-sm-12">
                                                    <label class="control-label">معرفی مدیر انبار *</label>
                            ...
                            <div id="product_manager_anbar_info" class="form-group row tab-pane fade">
                                        ...
                                        <div class="page-body" style="margin-top: 25px;">
                                            ...
                                            <div class="col-md-8">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="form-group row">
                                                        <div class="form-group col-sm-6">
                                                            <label class="control-label">موبایل و تلفن مدیر
                                                                انبار </label>
                                                            <input value="<?= $manager_anbar_tel1 ?>"
                                                                   name="manager_anbar_tel1" id="manager_anbar_tel1"
                                                                   placeholder="موبایل 1" class="form-control">
                                                            <input value="<?= $manager_anbar_tel2 ?>"
                                                                   name="manager_anbar_tel2" id="manager_anbar_tel2"
                                                                   placeholder="موبایل 2" class="form-control">
                                                            <input value="<?= $manager_anbar_tel3 ?>"
                                                                   name="manager_anbar_tel3" id="manager_anbar_tel3"
                                                                   placeholder="تلفن" class="form-control">
                                                        </div>
                                                    </div>
                                                    <label class="control-label" for="product_manager_anbar_email">ایمیل</label>
                                                    <div class="form-group row">
                                                        <div class="col-md-8">
                                                            <input class="form-control" type="email"
                                                                   name="product_manager_anbar_email"
                                                                   id="product_manager_anbar_email"
                                                                   value="<?= $product_manager_anbar_email ?>">
                                                        </div>
                                                        <? if ($edit_id > "") { ?>
                                                            <div class="col-md-4">
                                                                <img src="<?= $product_manager_anbar_img_email ?>" class="form-control"
                                                                     style="display: block;" alt="<?= $product_manager_anbar_img_email ?>">
                                                            </div>
                                                        <? } ?>
                                                    </div>
                                                    <div class="form-group row">
                                                        <div class="form-group col-sm-6">
                                                            <label class="control-label">آدرس مدیر انبار</label>
                                                            <input value="<?= $manager_anbar_unit_city ?>"
                                                                   name="manager_anbar_unit_city"
                                                                   placeholder="شهرستان" id="manager_anbar_unit_city"
                                                                   class="form-control">
                                                            <input value="<?= $manager_anbar_city ?>"
                                                                   name="manager_anbar_city"
                                                                   placeholder="شهر" id="manager_anbar_city"
                                                                   class="form-control">
                                                            <input value="<?= $manager_anbar_address ?>"
                                                                   name="manager_anbar_address"
                                                                   placeholder="آدرس" id="manager_anbar_address"
                                                                   class="form-control">
                                                            <input value="<?= $manager_anbar_zip ?>" name="manager_anbar_zip"
                                                                   placeholder="کدپستی" id="manager_anbar_zip"
                                                                   class="form-control">
                                                        </div>
                                                    </div>
<!---->
<!--                                                    <div class="form-group row">-->
<!--                                                        <div class="form-group col-sm-6">-->
<!--                                                            <label class="control-label">متراژ مدیر انبار </label>-->
...
<!--                                                        </div>-->
<!--                                                        <div class="form-group col-sm-6">-->
<!--                                                            <label class="control-label">آدرس گوگل </label>-->
<!--                                                            <input type="text" value="--><?//= $title ?><!--" name="title"-->
<!--                                                                   id="title"-->
<!--                                                                   class="form-control">-->
<!--                                                        </div>-->
<!--                                                    </div>-->
                            ...
                            <!--                            --><? // include 'newcoms/main/new_modual_slide.php4'; ?>
                            <!--                            <div id="product_manager_anbar_boss" class="form-group row tab-pane fade">-->
                            <!--                                <div class="page-content-area" id="more7">-->
                            <!--                                    <div class="page-body" style="margin-top: 25px;">-->
                            <!--                                        <div class="page-body" style="margin-top: 25px;">-->
                            <!--                                            <!-- #section:content/contenttext.content -->
                            ...
                            <!--                                                    <div class="form-group row">-->
                            <!--                                                        <div class="form-group col-sm-6">-->
                            <!--                                                            <label class="control-label">تلفن مدیر انبار </label>-->
                            <!--                                                            <input value="-->
                            <? //= $edit_name ?><!--" name="name" id="name"-->
                            <!--                                                                   class="form-control">-->
                            <!--                                                        </div>-->
                            <!--                                                    </div>-->
                            <!--                                                    <div class="form-group row">-->
                            ...
                            <fieldset>
                                <div class="modal fade" id="show_product_manager_anbar_representation" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header no-padding">
                                                <div class="table-header">
                                            ...
                                            <div class="modal-footer ">
                                                <button type="button" id="btn_ok_product_manager_anbar_representation00"
                                                        value=""
                                                        data-dismiss="modal"
                        ...
                        <div class="panel-footer bttools">
                            ...
                            <? if ($_SESSION["can_add"] == 1) { ?>
                                <a id="qazzaq" class="btn btn-success submit2"><span
                                            class="flaticon-verified9"></span><span>ثبت مدیر انبار </span></a>
                            <? } ?>
                        </div>
                    </div>
                </fieldset>
    ...
    <script>
        $('#product_manager_anbar_fax').click(function () {
            $('#product_manager_anbar_fax').mask("(+99) 9999-9999", {placeholder: ""});
        });
        // $('#product_manager_anbar_email').click(function () {
        //           $('#product_manager_anbar_email').mask("{a-z}@a-z.aaa",{placeholder:""});
        //       });

    </script>

    <? $_SESSION["del_item"] = 'del_product_manager_anbar';
    $_SESSION["edit_item"] = 'change_lock_product_manager_anbar';
    ?>
    ...
    <script>
        ...
        $("#relate_btn").click(function () {
            ...
            if (chkId == '') {
                ...
                // chkId_manager_anbar =  chkId_manager_anbar.slice(0,-1);
                // alert('chkchk==' + chkId_manager_anbar);
            }<?}?>
            $('.conchkNumber_product_manager_anbar:checked').each(function () {
                var vval = $(this).val();
                // alert('thisval'+vval);
            ...
            $('.conchkNumber_product_manager_anbar').not(':checked').each(function () {
                var unche = $(this).val();
                // alert('unche'+unche);
                chkIdArr = chkId.split(',');
                chkIdArr = chkIdArr.filter(function (val) {
            ...
            $.ajax({
                type: 'POST',
                url: 'New_ajax.php',
                data: "action=show_related_product_manager_anbar&id=" + $("#related_search").val() + "&value=" + $("#manage_lang").val() + "&checkid=" + chkId,
                success: function (result) {
                    //alert(result);
                    $("#show_waiting_search").hide();
        ...
        // if( chkId_manager_anbar>"")
        //        chkId_manager_anbar +=",";
        // else chkId_manager_anbar='';

        var chkId_manager_anbar = '';
        var chkIdArr_manager_anbar = '';
        $("#relate_btn_representation").click(function () {
            $("#show_waiting_search").show();
            <? if($edit_id > "") {?>
            if (chkId_manager_anbar == '') {
                chkId_manager_anbar = $('.check_checked_representation').val() + ",";
                // chkId_manager_anbar =  chkId_manager_anbar.slice(0,-1);
                // alert('chkchk==' + chkId_manager_anbar);
            }<?}?>
            $('.conchkNumber_product_manager_anbar_representation:checked').each(function () {
                var vval = $(this).val();
                // alert('thisval'+vval);
                chkIdArr_manager_anbar = chkId_manager_anbar.split(',');
                chkIdArr_manager_anbar = chkIdArr_manager_anbar.filter(function (val) {
                    return val !== vval;
                });
                chkId_manager_anbar = chkIdArr_manager_anbar.toString(',');
                chkId_manager_anbar += vval + ",";
                // alert('chkidchkid=='+chkId_manager_anbar);
            });
            $('.conchkNumber_product_manager_anbar_representation').not(':checked').each(function () {
                var unche = $(this).val();
                // alert('unche=' + unche);
                chkIdArr_manager_anbar = chkId_manager_anbar.split(',');
                // alert('charr='+chkIdArr_manager_anbar);
                chkIdArr_manager_anbar = chkIdArr_manager_anbar.filter(function (val) {
                    return val !== unche;
                });
                chkId_manager_anbar = chkIdArr_manager_anbar.toString(',');
            });
            // alert('chkid==' + chkId_manager_anbar);
            $.ajax({
                type: 'POST',
                url: 'New_ajax_product_manager_anbar_manager_anbar.php',
                data: "action=show_related_product_manager_anbar_representation&id_manager_anbar=" + $("#related_search_representation").val() + "&value_manager_anbar=" + $("#manage_lang").val() + "&checkid_manager_anbar=" + chkId_manager_anbar,
                success: function (result) {
                    //alert(result);
                    $("#show_waiting_search").hide();
        ...
        $(document).on('click', '#dropdelete', function (event) {
            $("#btn_del_related_product_manager_anbar").val($(this).val());

        });

        $(document).on('click', '#dropdelete_representation', function (event) {
            $("#btn_del_related_product_manager_anbar_representation").val($(this).val());
