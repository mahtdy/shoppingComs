...
<!--<script src="/yep_theme/default/rtl/css/product_anbar/jquery.min.js"></script>-->
...
<!--<script src="/yep_theme/default/rtl/css/product_anbar/bootstrap.min.js"></script>-->
...
if ($edit_mode > "") {
    $temp_user = get_result("select user_id from new_product_anbar where id= $edit_mode", $coms_conect);
...
if ($edit_id > "") {
    $temp_user = get_result("select user_id from new_product_anbar where id= $edit_id", $coms_conect);
//    if (!in_array($temp_user, $_SESSION['manager_user_permisson']))
//        header('Location: new_manager_signout.php');
}
...
//$product_anbar_type = $fori . $tasviri . $videoi . $soti . $erjaei;
...
//if ($upload_type == 2)
//    $product_anbar_image = injection_replace($_POST['product_anbar_image']);
//
//else if ($upload_type == 1) {
//    $product_anbar_image_avatar = injection_replace($_POST['product_anbar_image_avatar'][0]);
//    $product_anbar_image_avatar = trim($product_anbar_image_avatar);
//    if ($product_anbar_image_avatar != '')
//        $product_anbar_image = 'http://' . $_SERVER["HTTP_HOST"] . '/new_gallery/files/' . $product_anbar_image_avatar;
//    else
//        $product_anbar_image = '';
//}

//echo  $product_anbar_image.'wew';
$product_anbar_video_pic = injection_replace($_POST['product_anbar_video_pic']);
$product_anbar_attach = injection_replace($_POST['product_anbar_attach']);
$product_anbar_sound = injection_replace($_POST['product_anbar_sound']);
$product_anbar_video = injection_replace($_POST['product_anbar_video']);

$album_type = injection_replace($_POST['album_type']);
if ($album_type == 2) {
    $imagelist = ($_POST['imagelist']);
    foreach ($_POST['product_anbar_image_album'] as $val) {
        unlink('new_gallery/files/' . $val);
        unlink('new_gallery/files/tn' . $val);
    }

} else if ($album_type == 1 && count($_POST['product_anbar_image_album']) > 0) {
    foreach ($_POST['product_anbar_image_album'] as $val)
...
$totla_related_anbar = injection_replace($_POST['totla_related_anbar']);
$spesial_date = jdate('Y/m/d');

#اطلاعات انبار ها
$manager_name = injection_replace($_POST['product_anbar_manager_name']);
$boss_name = injection_replace($_POST['product_anbar_boss_name']);
$tech_name = injection_replace($_POST['product_anbar_technical_name']);
$latin_name = injection_replace($_POST['product_anbar_latin_name']);
$num_sabt = injection_replace($_POST['product_anbar_num_sabt']);
$num_code = injection_replace($_POST['product_anbar_num_code']);
$com_type = injection_replace($_POST['product_anbar_type']);
$sabt_date = injection_replace($_POST['sabt_date']);
$city = injection_replace($_POST['product_anbar_city']);
$zipcode = injection_replace($_POST['product_anbar_zipcode']);
$com_adres = injection_replace($_POST['product_anbar_address']);
$com_adres_site = injection_replace($_POST['product_anbar_address_site']);
$com_email = injection_replace($_POST['product_anbar_email']);
//$img_email = injection_replace($_POST['img_email']);

$com_fax = injection_replace($_POST['product_anbar_fax']);
...
$check_checked_anbar = injection_replace($_POST['check_checked_representation']);
$chkbx_link = injection_replace($_POST['chkbx_link']);
$state = injection_replace($_POST['product_anbar_state']);
$anbar_tel1 = injection_replace($_POST['anbar_tel1']);
$anbar_tel2 = injection_replace($_POST['anbar_tel2']);
$anbar_unit_city = injection_replace($_POST['anbar_unit_city']);
$anbar_city = injection_replace($_POST['anbar_city']);
$anbar_address = injection_replace($_POST['anbar_address']);
$anbar_zip = injection_replace($_POST['anbar_zip']);
$anbar_metraj = injection_replace($_POST['anbar_metraj']);
$adrs_google = injection_replace($_POST['adrs_google']);
//echo $check_checked;
//$phone_phone = injection_replace($_POST['product_anbar_ph_links_title1']);
...
//    if (isset($_POST['product_anbar_email']) and $edit_mode == '') {
//        echo 'sdsd';
...
//=======end invert email to jpg
    ...
    $arr = array("anbar_tel1" => $anbar_tel1,"anbar_tel2" => $anbar_tel2,"anbar_unit_city" => $anbar_unit_city,"anbar_city" => $anbar_city,"anbar_address" => $anbar_address,"anbar_zip" => $anbar_zip,"anbar_metraj" => $anbar_metraj,"adrs_google" => $adrs_google,"name_url" => $name_url, "upload_type_nav" => $upload_type_nav, "navication_pic" => $navication_pic, "check_link" => $chkbx_link, "upload_type" => $upload_type, 'hashtag_key' => $meta_label1, "seo_title" => $seo_title, "source_link_product" => $source_link_product, "source_product" => $source_product, "name" => $name, "title" => $title, "text" => $text, "meta_keyword" => $meta_keyword, "meta_description" => $meta_desciption, "abstract" => $abstract, "user_id" => $user_id, "la" => $manage_lang, "site" => $manage_site, "status" => $status);
    $id = insert_to_data_base($arr, 'new_product_anbar', $coms_conect);
//    print_r($arr);
    $id_com = $coms_conect->insert_id;
//echo 'idc='.$id_com;


//    echo  $product_anbar_image;
//    $arr_imag = array("id" => $id, "type" => 4, "adress" => $product_anbar_image, "name" => 'product_anbar_image');
//    insert_to_data_base($arr_imag, 'new_file_path', $coms_conect);
////    echo  $product_anbar_image.'qq';
//    $arr_attach = array("id" => $id, "type" => 4, "adress" => $product_anbar_attach, "name" => 'product_anbar_attach');
//    insert_to_data_base($arr_attach, 'new_file_path', $coms_conect);
//
//
//    $arr_sound = array("id" => $id, "type" => 4, "adress" => $product_anbar_sound, "name" => 'product_anbar_sound');
//    insert_to_data_base($arr_sound, 'new_file_path', $coms_conect);
//
//    $arr_video = array("id" => $id, "type" => 4, "adress" => $product_anbar_video, "name" => 'product_anbar_video');
//    insert_to_data_base($arr_video, 'new_file_path', $coms_conect);
//
//    $arr_video = array("id" => $id, "type" => 4, "adress" => $product_anbar_video_pic, "name" => 'product_anbar_video_pic');
//    insert_to_data_base($arr_video, 'new_file_path', $coms_conect);
...
//            if ($image != -1) {
//                $gallery_arr = array("id" => $id, "type" => 4, "adress" => $image, "name" => 'product_anbar_gallery');
...
//    }
//
//    $related_anbar = explode(",", $totla_related_anbar);
//    foreach ($related_anbar as $value_anbar) {
//
//        $arr = array("id" => $value_anbar, "page_id" => $id_com, "type" => 4);
//        insert_to_data_base($arr, 'new_product_anbar_related', $coms_conect);
//    }
//
//    ### slide show add
//
//    if ($slide == 1) {
//        $slide_link = "/product_anbar/$manage_lang/$id/" . insert_dash("$name");
...
//    }
    ...
    $arr = array("anbar_tel1" => $anbar_tel1,"anbar_tel2" => $anbar_tel2,"anbar_unit_city" => $anbar_unit_city,"anbar_city" => $anbar_city,"anbar_address" => $anbar_address,"anbar_zip" => $anbar_zip,"anbar_metraj" => $anbar_metraj,"adrs_google" => $adrs_google,"name_url" => $name_url, "upload_type_nav" => $upload_type_nav, "navication_pic" => $navication_pic, "check_link" => $chkbx_link, "upload_type" => $upload_type, 'hashtag_key' => $meta_label1, "seo_title" => $seo_title, "source_link_product" => $source_link_product, "source_product" => $source_product, "name" => $name, "title" => $title, "text" => $text, "meta_keyword" => $meta_keyword, "meta_description" => $meta_desciption, "abstract" => $abstract, "user_id" => $user_id, "la" => $manage_lang, "site" => $manage_site, "status" => $status);
    update_data_base($arr, 'new_product_anbar', $condition, $coms_conect);
    $id_pro = $edit_id;

//
    $condition = "id=$edit_mode && name='product_anbar_image'";
    $arr_imag = array("id" => $edit_mode, "type" => 4, "adress" => $product_anbar_image, "name" => 'product_anbar_image');
    update_data_base($arr_imag, 'new_file_path', $condition, $coms_conect);
//    echo  $product_anbar_image.'ee';
//
//    $condition = "id=$edit_mode && name='product_anbar_attach'";
//    $arr_attach = array("id" => $edit_mode, "type" => 4, "adress" => $product_anbar_attach, "name" => 'product_anbar_attach');
//    update_data_base($arr_attach, 'new_file_path', $condition, $coms_conect);
//
//
//    $condition = "id=$edit_mode && name='product_anbar_sound'";
//    $arr_sound = array("id" => $edit_mode, "type" => 4, "adress" => $product_anbar_sound, "name" => 'product_anbar_sound');
//    update_data_base($arr_sound, 'new_file_path', $condition, $coms_conect);
//
//    $condition = "id=$edit_mode && name='product_anbar_video'";
//    $arr_video = array("id" => $edit_mode, "type" => 4, "adress" => $product_anbar_video, "name" => 'product_anbar_video');
//    update_data_base($arr_video, 'new_file_path', $condition, $coms_conect);
//
//    $condition = "id=$edit_mode && name='product_anbar_video_pic'";
//    $arr_video = array("id" => $edit_mode, "type" => 4, "adress" => $product_anbar_video_pic, "name" => 'product_anbar_video_pic');
//    update_data_base($arr_video, 'new_file_path', $condition, $coms_conect);
//
...
//    ##edit gallery
//    $query1 = "delete from new_file_path where id='$edit_mode' && name='product_anbar_gallery' and type=4";
//    $coms_conect->query($query1);
//    if (!empty($imagelist)) {
//        foreach ($imagelist as $image) {
//            if ($image != -1) {
//                $gallery_arr = array("id" => $edit_mode, "type" => 4, "adress" => $image, "name" => 'product_anbar_gallery');
//                insert_to_data_base($gallery_arr, 'new_file_path', $coms_conect);
//            }
//        }
//    }
...
//        insert_to_data_base($arr, 'new_related_madual', $coms_conect);
//    }
//    $query111 = "delete from new_product_anbar_related where page_id='$edit_mode' and type=4";
//    $coms_conect->query($query111);
//    $related_anbar = explode(",", $totla_related_anbar);
//    foreach ($related_anbar as $value_anbar) {
//        $arr = array("id" => $value_anbar, "page_id" => $edit_mode, "type" => 4);
//        insert_to_data_base($arr, 'new_product_anbar_related', $coms_conect);
//    }
//    ### slide show update
//    if ($slide == 1) {
//        $slide_link = "/product_anbar/$manage_lang/$edit_mode/" . insert_dash("$name");
...
if ($edit_id > "") {
    $query = "SELECT * FROM new_product_anbar where id='$edit_id'";
...
//    $navication_pic = $row['navication_pic'];
    ...
    $anbar_tel1 = $row['anbar_tel1'];
    $anbar_tel2 = $row['anbar_tel2'];
    $anbar_unit_city = $row['anbar_unit_city'];
    $anbar_city = $row['anbar_city'];
    $anbar_address = $row['anbar_address'];
    $anbar_zip = $row['anbar_zip'];
    $anbar_metraj = $row['anbar_metraj'];
    $adrs_google = $row['adrs_google'];
//    $com_social_link4 = $row['social_link4'];
...
//    echo $phone_phone.$phone_name;
    ...
    $slide = $row['slide'];
//    $product_anbar_type = $row['content_type'];
//    $temp_1 = str_split($product_anbar_type);
//    $fori = $temp_1[0];
//    $tasviri = $temp_1[1];
//    $videoi = $temp_1[2];
//    $soti = $temp_1[3];
...
//    $text3 = $row['text3'];
    ...
    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_anbar_image'  and type=4";
    $result = $coms_conect->query($query);
    $row = $result->fetch_assoc();
    $product_anbar_image = $row['adress'];
//    echo $product_anbar_image;
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_anbar_attach'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_anbar_attach = $row['adress'];
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_anbar_sound'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_anbar_sound = $row['adress'];
//
//    $query = "SELECT adress FROM new_file_path where id='$edit_id' && name='product_anbar_video'  and type=4";
//    $result = $coms_conect->query($query);
//    $row = $result->fetch_assoc();
//    $product_anbar_video = $row['adress'];
//
//    $query = "SELECT b.id,title FROM new_related_madual a ,new_product_anbar b where page_id='$edit_id' and a.id=b.id and type=4";
//    $result = $coms_conect->query($query);
////    echo $query;exit;
//    $totla_related = "";
//    $i = 1;
...
//    }
//
//    $query1b = "SELECT b.id,name FROM new_product_anbar_related a ,new_product_anbar_cat_anbar b where page_id='$edit_id' and a.id=b.id and a.type=4";
...
//        if ($i != 1) $str = ',';
//        $i++;
//        $totla_related_anbar .= $str . $row['id'];
//    }
//
...
////echo 'val-i='.$val_i;
//for ($j = 1; $j <= $val_i; $j++) {
//    $phone_name = injection_replace($_POST["product_anbar_na_links_title{$j}"]);
//    $phone_phone = injection_replace($_POST["product_anbar_ph_links_title{$j}"]);
//    $phone_id = injection_replace($_POST["product_anbar_ph_id{$j}"]);
//    $phone_id_com = injection_replace($_POST["product_anbar_ph_id_com{$j}"]);
////echo 'pic='.$phone_id_com.'<br>';
//    if (empty($phone_id_com) && !empty($_POST["product_anbar_ph_links_title{$j}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_phone = array("phone_name" => $phone_name, "phone_phone" => $phone_phone, "id_com" => $id_com);
////       print_r($arr_phone);
//        insert_to_data_base($arr_phone, 'new_product_anbar_phone', $coms_conect);
//    } else {
//
//        $condition = "id=$phone_id";
//        $arr_phone = array("phone_name" => $phone_name, "phone_phone" => $phone_phone, "id_com" => $phone_id_com);
//        update_data_base($arr_phone, 'new_product_anbar_phone', $condition, $coms_conect);
//
//    }
//}
...
//    $slide_esp_id_com = injection_replace($_POST["product_anbar_slide_esp_id_com{$j}"]);
//
//
...
//        insert_to_data_base($arr_slide_esp, 'new_product_anbar_slide_esp', $coms_conect);
//    } else {
//
...
//        update_data_base($arr_slide_esp, 'new_product_anbar_slide_esp', $condition, $coms_conect);
//
//    }
...
////echo '$val_z='.$val_z;
//for ($z = 1; $z <= $val_z; $z++) {
//    $product_anbar_adres_onvan = injection_replace($_POST["product_anbar_adres_onvan{$z}"]);
//    $product_anbar_adres_state = injection_replace($_POST["product_anbar_adres_state{$z}"]);
//    $product_anbar_adres_city = injection_replace($_POST["product_anbar_adres_city{$z}"]);
//    $product_anbar_adres_zip = injection_replace($_POST["product_anbar_adres_zip{$z}"]);
//    $product_anbar_adres_adres = injection_replace($_POST["product_anbar_adres_adres{$z}"]);
//    $adres_id = injection_replace($_POST["product_anbar_ad_id{$z}"]);
//    $adres_id_com = injection_replace($_POST["product_anbar_ad_id_com{$z}"]);
////echo 'pic='.$adres_id_com.'<br>';
////echo 'pi='.$adres_id.'<br>';
//    if (empty($adres_id_com) && !empty($_POST["product_anbar_adres_adres{$z}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_adres = array("onvan_adres" => $product_anbar_adres_onvan, "state_adres" => $product_anbar_adres_state, "city_adres" => $product_anbar_adres_city, "adres_adres" => $product_anbar_adres_adres, "zip_adres" => $product_anbar_adres_zip, "id_com" => $id_com);
////       print_r($arr_phone);
//        insert_to_data_base($arr_adres, 'new_product_anbar_adres', $coms_conect);
//    } else {
//
//        $condition = "id=$adres_id";
//        $arr_adres = array("onvan_adres" => $product_anbar_adres_onvan, "state_adres" => $product_anbar_adres_state, "city_adres" => $product_anbar_adres_city, "adres_adres" => $product_anbar_adres_adres, "zip_adres" => $product_anbar_adres_zip, "id_com" => $adres_id_com);
////       print_r($arr_adres);
//        update_data_base($arr_adres, 'new_product_anbar_adres', $condition, $coms_conect);
//
//    }
//}
...
//
//
//########################################################### شروع ثبت انبار ها START insert update anbar   ########################################################
////
////echo 'val-w='.$val_w;
//for ($j = 1; $j <= $val_w; $j++) {
...
//    $anbar_id = injection_replace($_POST["product_anbar_dis_id{$j}"]);
//    $anbar_id_com = injection_replace($_POST["product_anbar_dis_id_com{$j}"]);
////echo 'pic='.$phone_id_com.'<br>';
//    if (empty($anbar_id_com) && !empty($_POST["tab_distributors_links_title{$j}"])) {
////        echo 'idcom='.$id_com.'pic='.$phone_id_com.'<br>';
//        $arr_anbar = array("com_tozi_name" => $com_tozi_name, "com_tozi_type" => $com_tozi_type, "id_com" => $id_com);
////       print_r($arr_anbar);
//        insert_to_data_base($arr_anbar, 'new_product_anbar', $coms_conect);
//    } else {
//
//        $condition = "id=$anbar_id";
//        $arr_anbar = array("com_tozi_name" => $com_tozi_name, "com_tozi_type" => $com_tozi_type, "id_com" => $anbar_id_com);
//        update_data_base($arr_anbar, 'new_product_anbar', $condition, $coms_conect);
//
//    }
//}
//########################################################### پایان ثبت انبار ها  END insert update anbar  ####################################################


//print_r($totla_related_anbar);
##################################
?>
<div class="modal fade" id="del_reated_product_anbar" tabindex="-1" role="dialog" aria-labelledby="edit"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer ">
                <button type="button" id="btn_del_related_product_anbar" data-dismiss="modal"
...
<div class="modal fade" id="del_reated_product_anbar_representation" tabindex="-1" role="dialog" aria-labelledby="edit"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-footer ">
                <button type="button" id="btn_del_related_product_anbar_representation" data-dismiss="modal"
                        class="btn btn-warning"><span
                            class="glyphicon glyphicon-ok-sign"></span>&nbsp;بلی
                </button>
                <button type="button" data-dismiss="modal" class="btn btn-warning"><span
...
<form class="form-horizontal" id="content_search" name="content_search" action="" role="form" method="get"
    ...
    <input type="hidden" value="new_product_anbar" name='yep'>
    <fieldset>
...
<fieldset>
    <div class="modal fade" id="show_product_anbar" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer ">
                    <button type="button" id="btn_ok_product_anbar00" value="" data-dismiss="modal"
            ...
            <div class="collapse navbar-collapse" id="nav-collapse">
                <ul class="nav navbar-nav navbar-left">
                    <? if ($_SESSION['can_add'] == 1) { ?>    <a data-toggle="tab" href="#add_templates"
                        ...
                        افزودن </a><? } ?>
                    <? if ($_GET['manager_filter'] > "") { ?>    <a href="/newcoms.php?yep=new_product_anbar"
                                                                    style="background-color: #2a8bcb!important;border-color: #2a8bcb;color: white;padding: 4px;">
    ...
    <div class="msheet tab-content">
        <div class="secfhead">
            ...
            <div class="sectitle">
                ...
                <div class="title"><p class="titr">لیست انبار ها</p>
        ...
        <div class="tab-pane <? if ($edit_id == "") echo 'active' ?>" id="" style="background-color: #EFF3F8;">
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="form-group yepco">
                            <form action="" method="get" class="navbar-form navbar-left pull-right yepco" role="search">
                                <input type="hidden" name="yep" value="new_product_anbar">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered"
                ...
                // $str_cat
                $sql1 = "select count(a)as cnt from (SELECT count(a.id) as a from new_product_anbar a  where la='$la' )q";
                //                  echo $sql1; exit;
                $result = $coms_conect->query($sql1);
                $RS = $result->fetch_assoc();

                $page = injection_replace($_GET["page"]);
                $a = pgsel((int)$page, $RS["cnt"], 10, 10, "$root/newcoms.php?yep=new_product_anbar$lang_page$site_page$manager_page$status_page$manager_q$field_page$expire_page$publish_page$cat_page$id_number_page");
                $from = $a[0];
                $to = $a[1];
                $pgsel = $a[2];
                $query = "SELECT a.site,a.id,a.la,a.title,a.name,a.status FROM new_product_anbar a
                ...
                while ($RS1 = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><a target="_blank"
                               href="/<?= "$domain/product_anbar/{$RS1['la']}/{$RS1['id']}/" . insert_dash($RS1['title']) ?>"><?= $RS1["title"] ?></a>
                        </td>
                        ...
                        <td>
                            ...
                            <? if ($_SESSION["can_edit"] == 1) {
                                ...
                                <a id="<?= $id ?>" class="edit_menu blue icon"
                                   href="newcoms.php?yep=new_product_anbar&edit_id=<?= $id ?>">
                            ...
                            if ($_SESSION["can_delete"] == 1) {
                                ...
                                <a href="/<?= "$domain/product_anbar/{$RS1['la']}/{$RS1['id']}/" . insert_dash($RS1['name']) ?>"
        ...
        <div class="tab-pane <? if ($edit_id != "" || $add_new == 1) echo 'active'; ?>" id="add_templates">
            ...
            <form id="contenttext" name="contenttext" class="" action="" role="form" method="post"
                ...
                <div id="id-message-new-navbar" class="message-navbar clearfix">
                    ...
                    <? } ?>
                    <a href="newcoms.php?yep=new_product_anbar" data-toggle="tooltip" data-placement="bottom"
                       title=""
                        ...
                        <ul class="nav nav-tabs" id="myTab">
                            <li>
                                <a data-toggle="tab" href="#home">
                                    <p class="flaticon-file23">ثبت انبار </p>
                            ...
                            <li>
                                <a data-toggle="tab" href="#product_anbar_info">
                                    <p class="flaticon-copy23"></p>
                                    اطلاعات انبار
                                </a>
                            </li>
<!--                            <li>-->
<!--                                <a data-toggle="tab" href="#product_anbar_boss">-->
<!--                                    <p class="flaticon-copy23"></p>-->
<!--                                    اطلاعات مدیر انبار-->
<!--                                </a>-->
<!--                            </li>-->
                            ...
                            <!--                            </li>-->
                            <!--                            <li>-->
                            <!--                                <a data-toggle="tab" href="#tab_product_anbars">-->
                            <!--                                    <p class="flaticon-copy23"></p>-->
                            <!--                                    محصولات-->
                            <!--                                </a>-->
                            <!--                            </li>-->
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-6">
                                                        <label class="control-label">نام انبار </label>
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-6">
                                                        <label class="control-label">آدرس سایت انبار </label>
                                                ...
                                                <div class="form-group" id="oracup">
                                                    ...
                                                    <div class="ui-sortable red_nav box_nav" id="upload_type1_nav"
                                                        ...
                                                        <? if ($edit_id > "" && $upload_type_nav == 1) {
                                                            $query_nav = "select navication_pic from new_product_anbar where id='$edit_id' ";
...
//                                                       $pic_str=explode(" ",$pic_str);echo 'aa='.$product_image_avatar;
                                            ...
                                            <div class="form-group">
                                                <div class="form-group col-sm-12">
                                                    <label class="control-label">معرفی انبار *</label>
                            ...
                            <div id="product_anbar_info" class="form-group row tab-pane fade">
                                        ...
                                        <div class="page-body" style="margin-top: 25px;">
                                            ...
                                            <div class="col-md-8">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="form-group row">
                                                        <div class="form-group col-sm-6">
                                                            <label class="control-label">تلفن انبار </label>
                                                            <input value="<?= $anbar_tel1 ?>" name="anbar_tel1" id="anbar_tel1"
                                                                   placeholder="تلفن 1" class="form-control">
                                                            <input value="<?= $anbar_tel2 ?>" name="anbar_tel2" id="anbar_tel2"
                                                                   placeholder="تلفن 2" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="form-group row">
                                                        <div class="form-group col-sm-6">
                                                            <label class="control-label">آدرس انبار</label>
                                                            <input value="<?= $anbar_unit_city ?>" name="anbar_unit_city"
                                                                   placeholder="شهرستان" id="anbar_unit_city" class="form-control">
                                                            <input value="<?= $anbar_city ?>" name="anbar_city"
                                                                   placeholder="شهر" id="anbar_city" class="form-control">
                                                            <input value="<?= $anbar_address ?>" name="anbar_address"
                                                                   placeholder="آدرس" id="anbar_address" class="form-control">
                                                            <input value="<?= $anbar_zip ?>" name="anbar_zip"
                                                                   placeholder="کدپستی" id="anbar_zip" class="form-control">
                                                        </div>
                                                    </div>

                                                    <div class="form-group row">
                                                        <div class="form-group col-sm-6">
                                                            <label class="control-label">متراژ انبار </label>
                                                            <input type="text" value="<?= $anbar_metraj ?>" name="anbar_metraj"
                                                                   id="anbar_metraj"
                                                                   class="form-control"
                                                                   >
<!--                                                            <input class="form-group" name="chkbx_link"-->
...
<!--                                                                        id="chklnk"></span></label>-->
                                                        ...
                                                        <div class="form-group col-sm-6">
                                                            ...
                                                            <input type="text" value="<?= $adrs_google ?>" name="adrs_google"
                                                                   id="adrs_google"
                                                                   class="form-control">
                                                    </div>
                                                    </div>
                            ...
                            <!--                            --><? // include 'newcoms/main/new_modual_slide.php4'; ?>
<!--                            <div id="product_anbar_boss" class="form-group row tab-pane fade">-->
<!--                                <div class="page-content-area" id="more7">-->
<!--                                    <div class="page-body" style="margin-top: 25px;">-->
<!--                                        <div class="page-body" style="margin-top: 25px;">-->
<!--                                            <!-- #section:content/contenttext.content -->
...
<!--                            </div>-->
                            ...
                            <fieldset>
                                <div class="modal fade" id="show_product_anbar_representation" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header no-padding">
                                                <div class="table-header">
                                            ...
                                            <div class="modal-footer ">
                                                <button type="button" id="btn_ok_product_anbar_representation00"
                                                        value=""
                                                        data-dismiss="modal"
                        ...
                        <div class="panel-footer bttools">
                            ...
                            <? if ($_SESSION["can_add"] == 1) { ?>
                                <a id="qazzaq" class="btn btn-success submit2"><span
                                            class="flaticon-verified9"></span><span>ثبت انبار </span></a>
                            <? } ?>
                        </div>
                    </div>
                </fieldset>
    ...
    <script>
        $('#product_anbar_fax').click(function () {
            $('#product_anbar_fax').mask("(+99) 9999-9999", {placeholder: ""});
        });
        // $('#product_anbar_email').click(function () {
        //           $('#product_anbar_email').mask("{a-z}@a-z.aaa",{placeholder:""});
        //       });

    </script>

    <? $_SESSION["del_item"] = 'del_product_anbar';
    $_SESSION["edit_item"] = 'change_lock_product_anbar';
    ?>
    ...
    <script>
        ...
        $("#relate_btn").click(function () {
            ...
            if (chkId == '') {
                ...
                // chkId_anbar =  chkId_anbar.slice(0,-1);
                // alert('chkchk==' + chkId_anbar);
            }<?}?>
            $('.conchkNumber_product_anbar:checked').each(function () {
                var vval = $(this).val();
                // alert('thisval'+vval);
            ...
            $('.conchkNumber_product_anbar').not(':checked').each(function () {
                var unche = $(this).val();
                // alert('unche'+unche);
                chkIdArr = chkId.split(',');
                chkIdArr = chkIdArr.filter(function (val) {
            ...
            $.ajax({
                type: 'POST',
                url: 'New_ajax.php',
                data: "action=show_related_product_anbar&id=" + $("#related_search").val() + "&value=" + $("#manage_lang").val() + "&checkid=" + chkId,
        ...
        $("#relate_btn_representation").click(function () {
            ...
            <? if($edit_id > "") {?>
            if (chkId_anbar == '') {
                chkId_anbar = $('.check_checked_representation').val() + ",";
                // chkId_anbar =  chkId_anbar.slice(0,-1);
                // alert('chkchk==' + chkId_anbar);
            }<?}?>
            $('.conchkNumber_product_anbar_representation:checked').each(function () {
                var vval = $(this).val();
                // alert('thisval'+vval);
                chkIdArr_anbar = chkId_anbar.split(',');
                chkIdArr_anbar = chkIdArr_anbar.filter(function (val) {
                    return val !== vval;
                });
                chkId_anbar = chkIdArr_anbar.toString(',');
                chkId_anbar += vval + ",";
                // alert('chkidchkid=='+chkId_anbar);
            });
            $('.conchkNumber_product_anbar_representation').not(':checked').each(function () {
                var unche = $(this).val();
                // alert('unche=' + unche);
                chkIdArr_anbar = chkId_anbar.split(',');
                // alert('charr='+chkIdArr_anbar);
                chkIdArr_anbar = chkIdArr_anbar.filter(function (val) {
                    return val !== unche;
                });
                chkId_anbar = chkIdArr_anbar.toString(',');
            });
            // alert('chkid==' + chkId_anbar);
            $.ajax({
                type: 'POST',
                url: 'New_ajax_product_anbar_anbar.php',
                data: "action=show_related_product_anbar_representation&id_anbar=" + $("#related_search_representation").val() + "&value_anbar=" + $("#manage_lang").val() + "&checkid_anbar=" + chkId_anbar,
                success: function (result) {
                    //alert(result);
                    $("#show_waiting_search").hide();
        ...
        $(document).on('click', '#dropdelete', function (event) {
            $("#btn_del_related_product_anbar").val($(this).val());

        });

        $(document).on('click', '#dropdelete_representation', function (event) {
            $("#btn_del_related_product_anbar_representation").val($(this).val());
