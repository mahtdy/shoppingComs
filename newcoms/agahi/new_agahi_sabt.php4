...
$can_comment = injection_replace($_POST['peygiri']);
...
if ($edit_id > "") {
    $query = "SELECT *,a.id as agahi_id,b.id as sabt_id,a.name as nameagahi FROM new_agahi a ,new_agahi_sabt b where  b.id='$id_agahi' AND a.id='$edit_id'";
//   echo 'query='.$query;
    $result = $coms_conect->query($query);
//    print_r($query);
//    print_r($result); echo $edit_id;exit;
    $row = $result->fetch_assoc();
    $agahi_id = $row['agahi_id'];
    $sabt_id = $row['sabt_id'];
    $nameagahi = $row['nameagahi'];
    $name = $row['name'];
    $family = $row['family'];
    $mobile = $row['mobile'];
    $view = $row['view'];
    ...
    $textagahi = $row['text'];
    $user_id = $row['user_id'];
//    $data_publish_sabt = $row['data_publish_sabt'];
...
//    $edit_source_company = $row['source_company'];
    $edit_name = $row['name'];
    $data_publish_sabt =     miladitojalaliuser(date('Y-m-d', $row['data_publish_sabt']));
//echo $data_publish_sabt;exit;
...
<!--                        افزودن </a>--><?// } ?>
<!--                    --><?// if ($_GET['manager_filter'] > "") { ?><!--    <a href="/newcoms.php?yep=new_agahi_sabt"-->
<!--                                                                    style="background-color: #2a8bcb!important;border-color: #2a8bcb;color: white;padding: 4px;">-->
<!---->
<!--                        بازگشت </a>--><?// } ?>
<!--                </ul>-->
    ...
    <div class="msheet tab-content">
        <div class="secfhead">
            ...
            <div class="sectitle">
                ...
                <div class="title"><p class="titr">لیست درخواست ها</p>
                    <p class="lead">امکان دیدن درخواست ها در این قسمت فراهم شده است.</p>
                </div>
            </div>
            <!-- /section:content/contenttext.head -->
            <div class="toolmenu">
        ...
        <div class="tab-pane <? if ($edit_id == "") echo 'active' ?>" id="" style="background-color: #EFF3F8;">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered"
                ...
                <tr>
                    ...
                    <th width="">نام</th>
                    <th>نام خانوادگی</th>
                    <th class="center">شماره همراه</th>
                    <th>عنوان آگهی</th>
                    <th class="center">توضیح آگهی</th>
                    <th>تاريخ درخواست</th>
                    <? if ($_SESSION['can_add'] && $_SESSION['can_edit']) { ?>
                        <th class="center">وضعیت درخواست</th>
                    <? } ?>
                    <th>امکانات</th>
                </tr>
                </thead>
                ...
                // $str_cat
                $sql1 = "select * from (SELECT count(c.id) as c from new_agahi a ,new_agahi_sabt c where  a.id=c.id_agahi)q";
//                                  echo $sql1; exit;
...
//                echo '=='.$page.$RS["cnt"];
                $a = pgsel((int)$page, $RS["cnt"], 10, 10, "$root/newcoms.php?yep=new_agahi_sabt$lang_page$site_page$manager_page$status_page$manager_q$field_page$expire_page$publish_page$cat_page$id_number_page");
                $from = $a[0];
                $to = $a[1];
                $pgsel = $a[2];
                $query = "SELECT *,a.id as agahi_id,b.id as sabt_id,a.name as nameagahi FROM new_agahi a ,new_agahi_sabt b where a.id = b.id_agahi  group by sabt_id order by sabt_id desc LIMIT $from,$to";
//                                   echo $query;exit;
                $result = $coms_conect->query($query);
//                print_r($result);exit;
                    $ii=1;
...
//                    $id += $ii++;
                    $id = $RS1["sabt_id"];
                    $id_agahi=$RS1["agahi_id"];
//                    echo 'id='.$id.'<br>'.'agahi-id='.$id_agahi;exit;
                    ?>
                    <tr>
                        ...
                        <td class="center"><?= $RS1["family"] ?></td>

                        <td class="center" style="direction: ltr"> <?= $RS1["mobile"] ?></td>
                        <td  > <?= $RS1["nameagahi"] ?></td>
                        <td  > <?= $RS1["title"] ?></td>
                        <? //= $RS1['site'] ?><!--</a></td>-->

<!--                                                <td>--><?//=miladitojalali( date($RS1["data_publish_sabt"])) ?><!--</td>-->

                        <td><?= miladitojalali( date('Y-m-d',$RS1["data_publish_sabt"])) ?></td>
                        <td class="center"><label></label>
                            <input id="<?= $RS1['id'] ?>"
                                   name="switch-field-1" <? if ($RS1['peygiri'] == 1) echo 'checked' ?>
                                   class="ace ace-switch ace-switch-5 can_comment" type="checkbox"/>
                            <span title="وضعیت پیگیری" class="lbl"></span></>
                        ...
                        <td>
                            <? if ($_SESSION["can_edit"] == 1) {
                                ...
                                <a id="<?= $id ?>" class="edit_menu blue icon"
                                   href="newcoms.php?yep=new_agahi_sabt&edit_id=<?= $id_agahi ?>&id=<?=$id?>">
                                    <i class="ace-icon fa fa-vimeo-square bigger-120" title="نمایش درخواست"></i>
                                </a>
                            <? }
                            if ($_SESSION["can_delete"] == 1) {
                                ?>
...
<!--                            <li id="newpag" class="addicon reset">-->
<!--                                                            <a href="#add_templates" id="--><?//=$RS1['id']?><!--"  data-toggle="tab" data-placement="bottom" title="نمایش">-->
<!--                                                                <span class="ace-icon fa fa-vimeo-square bigger-120"></span>-->
<!--                                                            </a>-->
<!--                                                        </li>-->
<!--                                <a href="/--><?//= "$domain/agahi/{$RS1['la']}/{$RS1['id']}/" . insert_dash($RS1['name']) ?><!--"-->
...
//                                ?>
<!---->
<!--                                <a href="/newcoms.php?yep=new_agahi_sabtt&id=--><?//= $RS1['id'] ?><!--" target="_blank"-->
        ...
        <div class="tab-pane <? if ($edit_id != "" || $add_new == 1) echo 'active'; ?>" id="add_templates">
            ...
            <form id="contenttext" name="contenttext" class="" action="" role="form" method="post"
                ...
                <div id="id-message-new-navbar" class="message-navbar clearfix">
                    ...
                    <input type="hidden" name="edit_mode" id="edit_mode" value="<?= $id_agahi ?>">
...
<!--                        </a>-->
<!--                    --><?// } ?>
                    <a href="newcoms.php?yep=new_agahi_sabt" data-toggle="tooltip" data-placement="bottom" title=""
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group row">
                                                    <?
                                                    if ($view==0) {
                                                        $query_sabt = "UPDATE new_agahi_sabt SET view=1 WHERE  id='$sabt_id'";
                                                        $coms_conect->query($query_sabt);
                                                    }
                                                    ?>
...
<!--                                                </div>-->
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-3">
                                                        ...
                                                        <br>
                                                        <br>
                                                        <span class="control-label"><?= $nameagahi ?></span>
                                                    </div>


                                                    <div class="form-group col-sm-3">
                                                        <label class="control-label">توضیح آگهی</label>
                                                        <br>
                                                        <br>
                                                    ...
                                                    <div class="form-group col-sm-4">
                                                        ...
                                                        <br>
                                                        <br>
                                                        <span class="control-label"><?=$textagahi  ?></span>
                                                    ...
                                                    <div class="form-group col-sm-2">
                                                        ...
                                                        <br>
                                                        <br>
                                                        <span class="control-label"><?=$publish_date  ?></span>
                                                ...
                                                <div class="form-group row">
                                                    <div class="form-group col-sm-4">
                                                        <label class="control-label">نام  و نام خانوادگی درخواست کننده</label>
                                                        <br>
                                                        <br>
                                                        <span class="control-label"><?= $name.' '.$family ?></span>

                                                    </div>


                                                    <div class="form-group col-sm-4" style="direction: ltr">
                                                        <label class="control-label">شماره همراه درخواست کننده</label>
                                                        <br>
                                                        <br>
                                                    ...
                                                    <div class="form-group col-sm-4">
                                                        ...
                                                        <br>
                                                        <br>
                                                        <span class="control-label"><?= $data_publish_sabt ?></span>
        ...
        $(function () {
            $('.can_comment').click( function() {
                ...
                $.ajax({
                    ...
                    data:"action=can_peygiri_agahi&valuepeygiri="+a+"&idpeygiri="+id,
    ...
    <script>
        ...
        $("#btn_ok").click(function () {
            ...
            $.ajax({
                type:'POST',
                url:'New_ajax_company.php',
                data:"action=del_agahi_sabt&idagahi_sabt="+id,
                success: function(result){
                    // console.log(result);
                    window.location.href = "newcoms.php?yep=new_agahi_sabt";
                    // alert(result);
                }
            });
        });
