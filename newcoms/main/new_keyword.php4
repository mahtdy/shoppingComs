...
$la=$manage_lang;
	if(get_result("select id from new_keyword where name='$name' and la='$manage_lang'",$coms_conect)==""){
		
		$arr_slide=array("name"=>$name,"la"=>$manage_lang);
		insert_to_data_base($arr_slide,'new_keyword',$coms_conect);
		show_msg($new_successfull);
	}else 
	show_msg_warninig($new_repaet_keyword);	
}if($name>""&&$_SESSION['can_edit']==1&&$edit_mode>0){
$la=$manage_lang;
	if(get_result("select id  from new_keyword where name='$name' and id <>$edit_mode and la='$manage_lang'",$coms_conect)==''){
		$condition="id=$edit_mode";
		$arr=array("name"=>$name);
		update_data_base($arr,'new_keyword',$condition,$coms_conect);
		show_msg($new_successfull);
	}else 
	show_msg_warninig($new_repaet_keyword);	
}
...
if($attach_file>""&&$_SESSION['can_add']==1&&end($pasvand)=='txt'){
	...
	while(!feof($myfile)) {
		...
		if($temp!=PHP_EOL&$temp>""&&get_result("select id from new_keyword where name='$temp' and la='$manage_lang_group'",$coms_conect)==""){
		...
		<div class="secfhead">
			...
			<div class="tab-pane active" id="tab1">
				...
				<div class="tt">
						...
						<div class="col-md-10">
							<div class="form-group yepco">
								<form action='' method='get' class="navbar-form navbar-left pull-right yepco" role="search">
									<input name="yep" value='new_keyword' type="hidden">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
						...
						################################################################
							$sql1 = "SELECT count(id) as cnt from new_keyword where la='$la'
							 $str_site  ";
							$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
							$page=injection_replace($_GET["page"]);
							$a=pgsel((int)$page,$RS["cnt"],10,10,"$root/newcoms.php?yep=new_keyword$site_page");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
							$sql1 = "SELECT * from new_keyword  where la='$la' $str_site order by id desc LIMIT $from,$to";
							$result = $coms_conect->query($sql1);
							while($RS1 = $result->fetch_assoc()){
								$id=$RS1["id"];
						?>
							<tr>
								<td class="center">
									<?if($pgsel>0){ echo $i*$pgsel;} else echo $i;?>
								</td>
								<td class="center">
									<label class="position-relative">
			...
			<div class="tab-pane" id="add_language">
				...
				<div id="id-message-new-navbar" class="message-navbar clearfix">
					...
					<a href="newcoms.php?yep=new_keyword" data-toggle="tooltip" data-placement="bottom" title="" class="cencel" data-original-title="بازگشت">
					...
					<!-- /section:main/keyword.enter -->	
						<div class="col-md-6">
							<div class="alert alert-warning ">یک فایل با فرمت txt و یونیکد utf8 را انتخاب نمایید.</br>
								<a href="newcoms.php?yep=new_keyword&download=ok">دانلود فایل نمونه</a>
							</div>	
						</div>
...
$("#show_lang").change(function () {
	window.location='newcoms.php?yep=new_keyword&show_lang='+$(this).val();
});
