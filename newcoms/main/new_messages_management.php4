...
if($draft_status==2){
	...
	$arr_attach=array("email"=>$recipient_email,"text"=>$recipient_text,"user_id"=>$_SESSION['manager_id'],"date"=>time(),"ip"=>$custom_ip,"type"=>2,"name"=>$recipient_subject);
...
<div class="tt">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="tabbable">
								...
								<div class="message-container">
									...
									<div id="id-message-list-navbar" class="message-navbar-sr clearfix">
										...
										<div>
											...
											<form action="" method="get" class="form-search" role="search">
												...
												<input type="hidden" name="yep" id="q" value="new_messages_management">
									...
									<div class="message-list-container">
										...
										<div class="message-list" id="message-list">
											<?if($site_filter>""&&$site_filter!=-1){
							$str_site=" and a.site='$site_filter'";
							$site_page="&site_filter=$site_filter";
						}
						if($q>""){
							$str_q="  and(a.text like '%$q%'  or a.name like '%$q%'  or a.email like '%$q%')";
							$manager_q="&q=$q";
						}
						if($lang_filter>""&&$lang_filter!=-1){
						...
						################################################################
							$sql1 = "SELECT count(a.id) as cnt from new_contactus_pm a  where a.id>0
							 $str_manager   $str_site $str_q  $str_lang and type=0  order by a.status";
						 //	 echo $sql1;
                 			$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();

							$a=pgsel((int)$_GET["page"],$RS["cnt"],10,10,"$root/newcoms.php?yep=new_messages_management$lang_page$site_page$manager_page$manager_q");
							$from=$a[0];
							$to=$a[1];
							$pgsel=$a[2];
						...
						$result1 = $coms_conect->query($sql1);
							...
							?>
									...
									<div class="message-footer clearfix">
										<div class="pull-left"> <?=get_result("select count(id) from new_contactus_pm where user_id={$_SESSION['manager_id']}",$coms_conect)?> پیام </div>
							...
							<div id="sent" class="tab-pane fade">
									...
									<div id="id-message-list-navbar" class="message-navbar-sr clearfix">
										<div class="message-bar">
											<div class="message-infobar" id="id-message-infobar">
												<span class="blue bigger-150">صندوق پیام</span> 
												<span class="grey-sr bigger-110">(<?=get_result("select count(id) from new_contactus_pm where user_id={$_SESSION["manager_id"]} and type=1 and status=1",$coms_conect)?> پیام خوانده نشده دارید)</span>
											</div>
									...
									<div id="id-message-item-navbar" class="hide message-navbar-sr clearfix">
										...
										<div>
											...
											<div class="messagebar-item-right">
												...
												<span class="grey-sr">امروز 7:15 بعدازظهر</span>
											</div>
										</div>
									</div>
						...
						################################################################
							$sql1 = "SELECT count(a.id) as cnt from new_contactus_pm a  where a.id>0
							 $str_manager   $str_site $str_q  $str_lang and type=1 order by a.status";
						 //	 echo $sql1;
                 			$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
						...
						$result1 = $coms_conect->query($sql1);
							...
							<div id="draft" class="tab-pane fade">
									...
									<div id="id-message-list-navbar" class="message-navbar-sr clearfix">
										<div class="message-bar">
											<div class="message-infobar" id="id-message-infobar">
												<span class="blue bigger-150">پیش نویس</span> 
						...
						################################################################
							$sql1 = "SELECT count(a.id) as cnt from new_contactus_pm a  where a.id>0
							 $str_manager   $str_site $str_q  $str_lang and type=2 order by a.status";
						 //	 echo $sql1;
                 			$result = $coms_conect->query($sql1);
							$RS = $result->fetch_assoc();
			...
			<form id="send_email_form" method="post" action="" class="hide form-horizontal message-form col-xs-12">
