...
}else if($album_type==1&&count($_POST['news_image_album'])>0){
    foreach($_POST['news_image_album'] as $val)
    ...
    <div class="msheet tab-content">
        <div class="secfhead">
            ...
            <div class="sectitle">
                ...
                <div class="title"><p class="titr">لیست اخبار</p><p class="lead">امکان مدیریت و افزودن و ویرایش کردن اخبار در این قسمت فراهم شده است.</p>
                </div>
            </div>
            <!-- /section:news/newstext.head -->
            <div class="toolmenu">
        ...
        <div class="tab-pane <?if($edit_id=="") echo 'active'?>" id="tab1" style="background-color: #EFF3F8;">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
                ...
                // $str_cat
                $sql1 = "select count(a)as cnt from (SELECT count(a.id) as a from new_managers b ,new_news a ,new_modules_catogory c where 
								 b.id = a.user_id and c.type=1 and a.id=c.page_id 
                ...
                $to=$a[1];
                $pgsel=$a[2];
                $query="SELECT a.can_comment,a.user_id,a.mudoal_lock,a.site,a.id,a.la,a.title,a.name,a.view,a.publish_date,a.status FROM new_managers b ,new_news a ,new_modules_catogory c where
							b.id = a.user_id and c.type=1 and a.id=c.page_id 
							$str_lang $str_site $str_q $str_manager $str_status $str_cat $str_id_number $str_field $str_expire
                ...
                while($RS1 = $result->fetch_assoc()){
                    ...
                    <tr>
                        ...
                        <td><a target="_blank" href="/<?="$domain/news/{$RS1['la']}/{$RS1['id']}/".insert_dash($RS1['title'])?>"><?=$RS1["title"]?></a></td>
					...
					<?if($RS1["status"]==1){echo 'منتشر شده';}else{echo 'پيش نويس';}?></span>
                        ...
                        <td>
                            <?if($_SESSION["can_edit"]==1){?>
                                <a id="<?=$id?>" class="edit_menu blue icon"  href="newcoms.php?yep=new_newstext&edit_id=<?=$id?>">
                            ...
                            <?}if($_SESSION["can_delete"]==1){?>
                                ...
                                <a href="/<?="$domain/news/{$RS1['la']}/{$RS1['id']}/".insert_dash($RS1['name'])?>" target="_blank"  class="del_menu blue icon">
                                    <i class="ace-icon fa fa-vimeo-square bigger-120" title="نمایش"></i>
                                </a>
                            <?}if(get_result("select count(id) from new_madules_comment where type=1 and status=0 and madul_id={$RS1['id']}",$coms_conect)){?>

                                <a href="/newcoms.php?yep=new_news_comment&id=<?=$RS1['id']?>" target="_blank" id="<?=$id?>" class="green new_scomments icon">
                                    <?=mudoal_comment_count($RS1['id'],1,$coms_conect)?><i class="ace-icon fa fa-comments bigger-120" title="نظرات"></i>
                                </a>
                            <?}$movie=get_result("SELECT adress FROM  new_file_path  WHERE type=1 and name='news_video' and id={$RS1['id']}",$coms_conect);
                            if($movie>""){?>
                            ...
                            <?}?>
                            <?$album=get_result("SELECT adress FROM  new_file_path  WHERE type=1 and name='news_gallery' and id={$RS1['id']}",$coms_conect);
                            if($album>""){?>
                            ...
                            <?}?>
                            <?$voice=get_result("SELECT adress FROM  new_file_path  WHERE type=1 and name='news_sound' and id={$RS1['id']}",$coms_conect);
                            if($voice>""){
                                ?>
                            ...
                            <?}?>
                            <?$attach=get_result("SELECT adress FROM  new_file_path  WHERE type=1 and name='news_attach' and id={$RS1['id']}",$coms_conect);
                            if($attach>""){
                                ?>
                                                ...
                                                <div class="form-group col-sm-12">
                                                    ...
                                                    <div class="form-group btn-group">
                                                        ...
                                                        <ul class="dropdown-menu" id="check_news" name="check_news" style="right:0px;">
                                                            <li><input type="checkbox" name="fori" <?echo 'checked';?> id="ID"><label for="ID" name="NAME" value="VALUE">خبر فوری</label></li>
                                                            <li><input type="checkbox" name="tasviri" <?if($tasviri=='1') echo 'checked';?> id="ID2"><label for="ID2" name="NAME" value="VALUE">خبر تصويری</label></li>
                                                            <li><input type="checkbox" name="videoi" <?if($videoi=='1') echo 'checked';?> id="ID3"><label for="ID3" name="NAME" value="VALUE">خبر ويدئويی</label></li>
                                                            <li><input type="checkbox" name="soti" <?if($soti=='1') echo 'checked';?> id="ID4"><label for="ID4" name="NAME" value="VALUE">خبر صوتی</label></li>
                                                            <li><input type="checkbox" name="erjaei" <?if($erjaei=='1') echo 'checked';?> id="ID5"><label for="ID5" name="NAME" value="VALUE">خبر ارجاعی</label></li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                                                ...
                                                                while($RS1 = $gallery_result->fetch_assoc()) {
                                                                    ...
                                                                    <script>
                                                                        $(document).ready(function(){
                                                                            ...
                                                                            var html_string = '<li><div><img width="105" height="105" alt="105x105" src="'+aks+'"/>'<div class="text"><div class="inner"><span>'+<?="'".get_pic_name($RS1['adress'])."'"?>+'</span><div class="tools"><a href="'+aks+'" //noinspection JSAnnotator
                                                                            data-rel="colorbox"><i class="ace-icon fa fa-search-plus"></i></a><a   data-id="<?=$edit_id?>" data-addres="'+aks+'" id="delete_image" href="#"><i class="ace-icon fa fa-times red"></i></a></div></div></div></div></li>';
...
<script>
    $("#relate_btn").click(function () {
        ...
        $.ajax({
            type:'POST',
            url:'New_ajax.php',
            data:"action=show_related&id="+$("#related_search").val()+"&value="+$("#manage_lang").val(),
            success: function(result){
                //alert(result);
