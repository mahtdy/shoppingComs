	 ...
	 <section class="breadcrumb-sec animated fadeIn">
            <div class="container">
                <ol class="breadcrumb rtl">
                    ...
                    <li class="active"><a href='<?="/content/$madual_lang"?>'><?=$view_content_list?></a></li>
                </ol>
            </div>
        </section>
        <!-- Start Page Title -->
        <div class="ptitle">
            <div class="container">
                ...
                <h1 class="title"><?=$view_content_list?></h1>
			...
			if($RS23['right_block']>0){
			   ...
			   <aside class="col-md-<?=$center?> col-sm-4 col-xs-12 pull-right animated fadeIn">
				...
				<article class="col-md-12 col-sm-8 pull-right animated fadeIn">
					<?$query1="select a.abstract,a.name,a.date,a.la,a.title,a.id,b.adress from new_content a ,new_file_path b 
						where b.name='content_image' and is_special=1 and spesial_start_date <='$now' and spesial_finish_date >='$now' and site='$site' and la='$madual_lang' and status=1 and b.type=11 and a.id=b.id  and publish_date<='$now'
                   ...
                   <section class="vige_news">
                      <div class="row">
                         <div class="title col-sm-6 col-xs-12 pull-right rtl">
                           <h4><?=$row1['name']?></h4>
                           <a href="<?="/content/{$row1['la']}/{$row1['id']}/".insert_dash($row1['title'])?>"><h3><?=$row1['title']?></h3></a>
                           <p><?=$row1['abstract']?></p>
                         </div>
                         <div class="photo col-sm-6 col-xs-12 pull-right rtl">
                             <a href="<?="/content/{$row1['la']}/{$row1['id']}/".insert_dash($row1['title'])?>" class="hvr-grow"><img src="<?=$row1['adress']?>"></a>
                        </div>
                      </div>
                 </section>
						<?}?>
...
<script>
 $("#content_type").change(function () {
			...
			$.ajax({
				...
				data:"action=content_type_change&id="+$(this).val()+"&value="+$("#content_order").val(),
 ...
 $("#content_order").change(function () {
			...
			$.ajax({
				type:'POST',
				url:'/New_members_ajax.php',
				data:"action=content_type_change&id="+$("#content_type").val()+"&value="+$("#content_order").val(),
				success: function(result){ 
					$("#waiting").hide();
					$("#show_result").html(result);
				}
