...
//			$arr=array("sms_value"=>0,"sms_active"=>$sms_active,"email_active"=>$email_active,"status"=>$status,"la"=>$madual_lang,"site"=>$site,"name"=>$name,"user_name"=>$user_name,"family"=>$family,"password"=>$password,"mobile"=>$mobile,"email"=>$email,"type"=>1,'date'=>$now,'ip'=>$custom_ip);
            $arr = array("sms_value" => 0, "sms_active" => $sms_active, "status" => $status, "la" => $madual_lang, "site" => $site, "password" => $password, "mobile" => $mobile, "user_name" => $mobile, "type" => 1, 'date' => $now, 'ip' => $custom_ip);
//			print_r($arr);
            ...
            if ($sms_active == 0) {
                ...
                $sendDate = array(time());
//				$sms=send_sms_new($sms_url,$sms_password,$sms_user,$senderNumbers,$recipientNumbers,$messageBodies,$sendDate,$coms_conect,$user_name);
                $sms = send_sms_new($sms_value, $mobile, $coms_conect);
//				echo $mobile.'repon sms='.$sms['result'];exit;
                if ($sms['result'] != 1) {
                    $condition1 = "id=$id";
...
//			}
                    ?>
                    <!--                    <div id="divcongig" style="display: none">-->
                    <!--                        <label for="ok_code">code sms ra vared konid</label>-->
                    <!--                        <input type="text" name="ok_code" id="ok_code">-->
                    <!--                        <button id="chck_code">ok</button>-->
                    <!--                    </div>-->
                    <!--                    <script>-->
                    <!--                    <script>-->
                    <!--                        $(document).ready(function () {-->
                    <!--                            $('.container').attr('style','display:none');-->
                    <!--                            $('#divcongig').attr('style','display:block');-->
                    <!--                            -->
                    <!--                        });-->
                    <!--                        $('#chck_code').click(function () {-->
                    <!--                            var okcode=$('#ok_code').val();-->
                    <!--                            $.ajax({-->
                    <!--                                type:'POST',-->
                    <!--                                url:'New_ajax_product.php',-->
                    <!--                                data:"action=ok_config_sms&value="+okcode+"&id="+--><?//=$id;
                    ?><!--,-->
        ...
        <div class="row register">
            ...
            <div class="col-md-6 col-xs-12 form margin_middle">
                ...
                <form method='post' id="member_registerForm"
                    ...
                    <? //=$user_name?><!--" class="form-control" placeholder="--><? //=$view_username?><!--"-->
                    ...
                    <? //=$view_username_point_num_letter?><!--"-->
                    <!--				 -->
                    <!--								data-fv-stringlength="true"-->
                    <!--								data-fv-stringlength-min="6"-->
                    <!--								data-fv-stringlength-max="30"-->
                    <!--								data-fv-stringlength-message="-->
                    <? //=$view_username_character?><!--">-->
                    <!--                            </div>-->
                    ...
                    <? //=$view_repeat_password_notsame?><!--">-->
                    <!--                            </div>-->

                    <!--                            <div class="col-md-6 col-sm-6 col-xs-12 form-group pull-right rtl">-->
                    <!--                                <input name="name" class="form-control" value="-->
                    <? //=$name?><!--" placeholder="--><? //=$view_name?><!--"-->
                    <!--								data-fv-notempty="true"-->
                    <!--								data-fv-notempty-message="-->
                    <? //=$view_field_required?><!--">-->
                    <!--                            </div>-->
                    <!--                            <div class="col-md-6 col-sm-6 col-xs-12 form-group pull-right rtl">-->
                    <!--                                <input name="family" class="form-control" value="-->
                    <? //=$family?><!--" placeholder="--><? //=$view_family?><!--"-->
                    <!--								data-fv-notempty="true"-->
                    <!--								data-fv-notempty-message="-->
                    <? //=$view_field_required?><!--">-->
                    <!--                            </div>-->
                    <div class="col-md-12 col-sm-12 col-xs-12 form-group pull-right">
                        <input name="mobile" id="mobile" class="form-control" value="<?= $mobile ?>" type="text"
                               placeholder="<?= $view_number_mobile ?>"
                               data-fv-notempty="true"
                    ...
                    <? //=$email?><!--" type="email" placeholder=" --><? //=$view_email?><!-- "-->
                    <!--								style="direction:ltr!important"-->
                    <!--								data-fv-notempty="true"-->
                    <!--								data-fv-notempty-message="-->
                    <? //=$view_field_required?><!--"-->
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-2 col-sm-1 col-xs-1 form-group pull-right rtls pr0">
                            <div><span><img src="<? crate_capcha_pic($madual_lang) ?>" style="height: 34px;"></span>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12 col-sm-12 col-xs-12 form-group pull-right rtl">
                        ...
                        <?= $view_me_all ?> <a data-toggle="modal" class="blue_link cs_hand"
                                               data-target="#reg_low"><?= $view_rules ?></a>
                        <?= $view_read_agree ?>
                        <img src="/waiting.gif" id="dynamic" style="width: 100px;display:none;">
                    </div>

                    <div class="col-md-12 col-sm-12 col-xs-12 form-group pull-right">
                        <button id='submit_page' type='submit'
                                class="btn btn-success fullwidth"><?= $view_register ?></button>
                    </div>
