	 ...
	 <section class="breadcrumb-sec animated fadeIn">
            <div class="container">
                <ol class="breadcrumb rtl">
                    ...
                    <li class="active"><a href='<?="/news/$madual_lang"?>'><?=$view_news_list?></a></li>
                </ol>
            </div>
        </section>
        <!-- Start Page Title -->
        <div class="ptitle">
            <div class="container">
                ...
                <h1 class="title"><?=$view_news_list?></h1>
        ...
        <div class="container">
            <main>
			 <?$query23="select id,left_block,right_block,center from new_blocks_sorts where type='m'  and tem_name='$tem_name' and file_name=1 and pages_id=1 and la='$defult_lang' and status=1";
			...
			if($RS23['right_block']>0){
			   ...
			   <aside class="col-md-<?=$center?> col-sm-4 col-xs-12 pull-right animated fadeIn">
				...
				<article class="col-md-12 col-sm-8 pull-right animated fadeIn">
					<?$query1="select a.abstract,a.name,a.date,a.la,a.title,a.id,b.adress from new_news a ,new_file_path b 
						where b.name='news_image' and is_special=1 and spesial_start_date <='$now' and spesial_finish_date >='$now' and site='$site' and la='$madual_lang' and status=1 and b.type=1 and a.id=b.id  and publish_date<='$now'
                   ...
                   <section class="vige_news">
                      <div class="row">
                         <div class="title col-sm-6 col-xs-12 pull-right rtl">
                           <h4><?=$row1['name']?></h4>
                           <a href="<?="/news/{$row1['la']}/{$row1['id']}/".insert_dash($row1['name'])?>"><h3><?=$row1['title']?></h3></a>
                           <p><?=$row1['abstract']?></p>
                         </div>
                         <div class="photo col-sm-6 col-xs-12 pull-right rtl">
                             <a href="<?="/news/{$row1['la']}/{$row1['id']}/".insert_dash($row1['name'])?>" class="hvr-grow"><img src="<?=$row1['adress']?>"></a>
                        </div>
                      </div>
                 </section>
						<?}?>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-10 col-sm-12 pull-right pr0">
                                                    <form class="row">
                                                        ...
                                                        <select id="news_order"  class="form-control input-sm" style="font-size: 9pt;">
                                                            <option <?if($_SESSION['news_order']==1) echo 'selected';?> value='1'><?=$view_new_prev?></option>
                                                            <option <?if($_SESSION['news_order']==0) echo 'selected';?> value='0'><?=$view_past_next?></option>
                                                        </select>
                                                        </div>
                                                        <div class="col-md-6 col-sm-6 col-xs-6 form-group">
                                                            <select id='news_type' class="form-control input-sm"  style="font-size: 9pt;">
                                                                <option value='0' <?if($_SESSION['news_type']==0) echo 'selected';?>><?=$sc_NewsTitle_All?></option>
                                                                <option value='1' <?if($_SESSION['news_type']==1) echo 'selected';?>><?=$view_content_last_month?></option>
                                                                <option value='2' <?if($_SESSION['news_type']==2) echo 'selected';?>><?=$view_content_last_week?></option>
                                                            </select>
                                                        </div>
                                                    </form>
                                                </div>
<script>
 $("#news_type").change(function () {
			...
			$.ajax({
				...
				data:"action=news_type_change&id="+$(this).val()+"&value="+$("#news_order").val(),
 ...
 $("#news_order").change(function () {
			...
			$.ajax({
				type:'POST',
				url:'/New_members_ajax.php',
				data:"action=news_type_change&id="+$("#news_type").val()+"&value="+$("#news_order").val(),
				success: function(result){ 
					$("#waiting").hide();
					$("#show_result").html(result);
				}
