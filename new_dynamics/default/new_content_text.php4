...
$query="select a.user_id,a.navication_pic,a.la,a.mudoal_lock,a.source_content,a.source_link_content,a.content_type,a.can_comment,a.date,a.abstract,a.name,title,la,a.id,publish_date,text,view from new_content a where status=1  and la='$madual_lang' and site='$site' and a.id='$madual_id' and publish_date<=$now";
	//echo $query1;
...
<section class="breadcrumb-sec animated fadeIn">
	<div class="container">
		<ol class="breadcrumb rtl">
			...
			<li  ><a href="/content/<?=$defult_lang?>"><?=$view_content?></a></li>
			<?$cat_id=get_result("select cat_id from new_modules_catogory where type=11 and page_id={$row['id']}",$coms_conect);?> 
			<li class="active"><a href="/content/<?=$defult_lang?>/category/<?=$cat_id?>"><?=get_result("select name from new_modules_cat where  id=$cat_id",$coms_conect);?></a></li>
...
<?if($row["navication_pic"]==''){
	$row["navication_pic"]=get_result("select address from new_default_navbar where name='content_nav_picture' and la='$defult_lang' and site='$defult_site'",$coms_conect);	
}
if($row["navication_pic"]>""){?>	
	<!-- Start Slider -->
...
<!--				<input class="inputh" type="text" id="rep_comment_id" name="rep_comment_id" >-->
			 ...
			 <section class="block-col">
				...
				<div class=""> 
					...
					<?if(time()>=$_SESSION['count']+2){
						$query1="update new_content set view=view+1 where id='$madual_id'"; 
							...
							<section class="content">
								<div class="">
									<section class="col-md-12 newsitempage">
										...
										<p class="cat_date ">
												<?$cat_content=get_modual_cat(11,$row['id'],$coms_conect); 
												foreach($cat_content as  $key=>$val){
													echo "  <span class='label'><a itemprop='keyword' target='blank' href='/content/$defult_lang/category/{$val['id']}'><span>{$val['name']}</span></a></span>  ";
												}
												?>
											...
											<div itemprop="author" itemscope="" itemtype="" class="author-details-row1">نویسنده: <span itemprop="name" class="color-red" id="manager"><?=$row330['name'];?></span>
											</div>
											...
											<div class="col-xs-12 pull-left prl0">
												<?$sql33 = "SELECT adress FROM new_file_path where name='content_image' and type=11 and id='$id'";
											...
											<div class="col-xs-12 tools rtl">
												<div class="col-md-6 col-xs-12 pull-right rtl pr0 social">
													...
													<div class="ccomment pull-right">
														...
														type=11 and madul_id=$madual_id",$coms_conect)?></span>
													</div>
										...
										<div class="row">
											<?if(get_result("SELECT adress from new_file_path where id='$id'  and type=11 and name='content_gallery'",$coms_conect)){?>
												<div id="myCarousel" class="carousel slide" data-ride="carousel">
													<!-- Indicators -->
													<ol class="carousel-indicators">
													<?$i=0;$sql14 = "SELECT adress from new_file_path where id='$id'  and type=11 and name='content_gallery'";
											...
											<?}?>
											<?$sql2330 = "SELECT adress FROM new_file_path where name='content_video' and type=11 and id='$id'";
											...
											<div class="col-xs-12 newsvideoitem prl0">
												...
												<?}?>
											 <?$sql233 = "SELECT adress FROM new_file_path where name='content_sound' and type=11 and id='$id'";
											...
											<div itemprop="articlebody" class="col-xs-12 matnkhabar prl0">
												...
												$row["text"]=str_replace('img src="source','img src="/source',$row["text"]);
													...
													$row["text"]=str_replace('<a href="content','<a href="/content',$row["text"]);
													...
													if($temp[1]==1)$erjaei="<a rel='nofollow' target='_blank' href='{$row['source_link_content']}'>{$row['source_content']}</a>";
													else $erjaei=$row['source_content']; 
													if($erjaei) echo  "$view_source_content ".$erjaei;
													 ?>
												</p>
											</div>
											<?$sql2343 = "SELECT adress FROM new_file_path where name='content_attach' and type=11 and id='$id'";
                                      ...
                                      <?  include ("/../../newcoms/new_like_dislike.php");?>
                                        <div class="row">
										<?if(get_result("select count(id) from new_mudoal_label where type=11 and id={$row['id']}",$coms_conect)>0){?>
											<div class="col-xs-12 tags prl0">
												<fieldset>
													<legend><?=$view_tags?></legend>
													<ul class="pr10">
														<?create_label_pishgaman($row['id'],'content',$madual_lang,$coms_conect,11);?>
													</ul>
												</fieldset>
											...
											<?}?>	
											 	<div class="col-xs-12 relatedposts prl0">
													<fieldset>
														...
														<?$query="SELECT b.name,b.id,b.title,b.date,content_type,b.la FROM new_related_madual a ,new_content b  where page_id='$madual_id' and a.id=b.id  and type=11";
															...
															while($mortab = $result->fetch_assoc()){
															 ?>
															<li>
																<a href="<?="/content/{$mortab['la']}/{$mortab['id']}/".insert_dash($mortab['name'])?>">
															...
															<?$i++;}?>
															<?if($i<6){
																$cat_id=get_result("select cat_id from new_modules_catogory    where page_id=$madual_id and type=11  ",$coms_conect);
																$j=6-$i;
																$sql120 = "SELECT a.date,a.content_type,a.la,a.id,a.title,name from new_content a ,new_modules_catogory c   
																where a.id>0  and c.type=11 and a.id<>$madual_id and c.page_id=a.id and c.cat_id=$cat_id    and status=1
																order by a.id desc limit 0,$j";
																// echo $sql120 ;//exit;
																$resultd10 = $coms_conect->query($sql120);
																while($row10 = $resultd10->fetch_assoc()) {?>
																<li>
																		<a href="<?="/content/{$row10['la']}/{$row10['id']}/".insert_dash($row10['name'])?>">
