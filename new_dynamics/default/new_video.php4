<?/*$query="select * from new_file_path where type=8 and name='video_videos'";$i=0;
				$result = $coms_conect->query($query);
					while($RS1 = $result->fetch_assoc()) {
						$temp=explode(".",basename($RS1['adress']));
						$adress="/source/comsroot/video_pic/".$temp[0].'.jpg';
						$arr_attach=array("type"=>8,"name"=>'video_pic',"id"=>$RS1['id'],"adress"=>$adress);
						 insert_to_data_base($arr_attach,'new_file_path',$coms_conect);
						//echo $adress.$RS1['id'].'<br>';
					} 
*/
...
$nav_src=get_result("select address from new_default_navbar where name='video_nav_picture' and la='$defult_lang' and site='$defult_site'",$coms_conect);	
        ...
        <div class="ptitle">
            <div class="container"> 
                ...
                <span class="pdesc"> <?get_result("select address from new_default_navbar where name='video_show_file_str'  and la='fa' and site='main'",$coms_conect);?></span>
            </div>
        </div>
        <!-- End Page Title -->
        <div class="container">
            <main>
                <!-- Start Main -->
			<?$query23="select id,left_block,right_block,center from new_blocks_sorts where type='m'  and tem_name='$tem_name' and file_name=1 and pages_id=8  and la='$defult_lang' and status=1";
			   ...
			   <aside class="col-md-<?=$center?> col-xs-12 pull-right animated fadeIn">
                ...
                <div class="row gridgallery">
                    ...
                    <div class="col-xs-12 cat_select_search_show row rtl prl0">
                        ...
                        <div class="col-md-8 col-sm-6 col-xs-12 form-group pull-right cat_search row pr0">
					...
					<?$sql12 = "SELECT a.duration,a.mudoal_lock,a.site,a.la,a.title,a.id  ,a.view,a.status   from new_video a,new_modules_catogory b   
							where id>0 and a.id=b.page_id and b.type=8 and site='$defult_site' and la='$defult_lang'
							group by b.page_id
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12 pull-right oh">
                        <div class="item">
                            <a href="/video/<?=$row['la'].'/'.$row['id'].'/'.insert_dash($row['title'])?>"> 
                                <figure>
                                    <img class = "hvr-glow" src="<?=get_modual_address($row['id'],$coms_conect)?>" alt="<?=$row['title']?>">
...
$("#ajax_pagin").click(function (e) {
		...
		$.ajax({
			...
			data:"action=show_videos_pagin_ajax&secend_value="+$("#page_num").val()+"&id=<?=$defult_site?>",
