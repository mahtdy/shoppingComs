...
<?$madual_tag_id=	delete_dash(urldecode($madual_tag_id));?>
            <div class="container">
                <ol class="breadcrumb rtl">
                    ...
                    <li> <span class="active"><?=$view_content_list_keyword?> </span> 
                    <a  href="<?=$url?>"><span class="active"><?=urldecode($madual_tag_id)?></span></a>
					</li>
                </ol>
            </div>
        </section>
        ...
        <div class="container">
            <main>
			 <?$query23="select id,left_block,right_block,center from new_blocks_sorts where type='m'  and tem_name='$tem_name' and file_name=4 and pages_id=11 and la='$defult_lang' and status=1";
			$result23 = $coms_conect->query($query23);
			$RS23 = $result23->fetch_assoc();
			$center=$RS23['center'];
			if($center=='')$center=12;
			 if(get_modual_setting_result($defult_site,$defult_lang,'content_sesrch_have_ads',$coms_conect))
				 $center=$center-2;
			   ...
			   <aside class="col-md-<?=$center?> col-sm-4 col-xs-12 pull-right animated fadeIn">
				...
				<article class="col-md-12 col-sm-8 pull-right animated fadeIn">
					...
					if($url_temp[3]=='keyword'){ 
						...
						$sql1 = "SELECT a.id as cnt from new_content a ,new_file_path b,new_mudoal_label c where 
						 c.type=11 and a.id=c.id and c.label_id='$madual_tag_id' and b.name='content_image' 
						 and a.status=1 and b.type=11 and a.id=b.id
						 and a.la='$madual_lang' and a.site='$site' and publish_date<='$now'
						 	group by b.id";
						 
						$result = $coms_conect->query($sql1);
						$RS = $result->fetch_assoc();
						$a=new_pgsel((int)$madual_page_id,$result->num_rows,9,9,"$root/content/$madual_lang/$madual_tag_id");
						$from=$a[0];
						$to=$a[1];
						$pgsel=$a[2];
						$limit=" limit $from,$to";
					...
					if($madual_search_str&&$url_temp[3]=='keyword');
						$str ="and (a.title like '%$madual_search_str%' or a.text like '%$madual_search_str%' or a.abstract like '%$madual_search_str%')";
						$sql12 = "SELECT a.mudoal_lock,a.site,a.la,a.title,a.id  ,a.view,a.status,name,abstract   from new_content a,new_modules_catogory b ,new_mudoal_label c  
							where   a.id=b.page_id and b.type=11 and
							c.type=11 and  a.id=c.id	and label_id='$madual_tag_id'
					...
					if($madual_search_str&&$url_temp[3]=='search');
						$str ="and (a.name like '%$madual_search_str%' or a.title like '%$madual_search_str%' or a.text like '%$madual_search_str%' or a.abstract like '%$madual_search_str%')";
						 $sql1 = "SELECT id from new_content a,new_modules_catogory b   
							where id>0 and a.id=b.page_id and a.status=1 and  b.type=11
							and site='$defult_site' $str $cat_str $finish_str $start_str
								group by b.page_id";
						$result = $coms_conect->query($sql1);
					 //	echo $sql1;
						$RS = $result->fetch_assoc();
						$a=new_pgsel((int)$madual_page_id,$result->num_rows  ,9,9,"$root/content/$madual_lang/page");
						$from=$a[0];
						$to=$a[1];
						$pgsel=$a[2];
						$limit=" limit $from,$to";
						
						$sql12 = "SELECT user_id,a.mudoal_lock,a.site,a.la,a.title,a.id,name,abstract  ,a.view,a.status   from new_content a,new_modules_catogory b   
							where id>0 and a.id=b.page_id and a.status=1 and b.type=11 and site='$defult_site' $str $cat_str $finish_str $start_str
							group by b.page_id
							order by a.id desc ";
					}	
							...
							while($row11 = $resultd1->fetch_assoc()) {
								$sql11="select adress from new_file_path where type=11 and name='content_image' and id ={$row11['id']}";	
									...
									<div class="col-lg-3 col-md-3 col-sm-4 col-xs-4 pull-right rtl sr_no_padd_1">
										<div class="photo">
											<figure>
												...
												<img src="<?=$row1['adress']?>" alt="<?=$row11['title']?>"></a>
											</figure>
										</div>
									</div> 
									<div class="col-lg-9 col-md-9 col-sm-8 col-xs-8 pull-right rtl">
										...
										<a href="<?="/content/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>"><h3><?=$row11['title']?></h3></a>
											<h6 class="news_item_author">
												<span class="sr-news-name">
                                                    <a href="<?="/content/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>">
                        ...
                        <section class="block-col">
                            <div class="block">
                                <div class="content ads-col">
									<?$query="select title,link,pic_adress from new_tem_setting where name='content_sesrch_have_ads'  and la='$defult_lang' and site='$defult_site'"; 
