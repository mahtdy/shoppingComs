...
$query="select a.tech_name,a.num_sabt,a.com_type,a.city,a.com_adres,a.com_adres_site,a.com_email,a.com_fax,a.user_id,a.navication_pic,a.la,a.source_company,a.source_link_company,a.can_comment,a.date,a.abstract,a.name,title,la,a.id,publish_date,text,view from new_company a where status=1  and la='$madual_lang' and site='$site' and a.id='$madual_id' and publish_date<=$now";
//echo $query;exit;
...
<section class="breadcrumb-sec animated fadeIn">
    <div class="container">
        <ol class="breadcrumb rtl">
            ...
            <li  ><a href="/company/<?=$defult_lang?>"><?=$view_company?></a></li>


            <?$cat_id=get_result("select cat_id from new_modules_catogory where type=19 and page_id={$row['id']}",$coms_conect);?>
            <li class="active"><a href="/company/<?=$defult_lang?>/category/<?=$cat_id?>"><?=get_result("select name from new_modules_cat where  id=$cat_id",$coms_conect);?></a></li>
...
<?if($row["navication_pic"]==''){
    $row["navication_pic"]=get_result("select address from new_default_navbar where name='company_nav_picture' and la='$defult_lang' and site='$defult_site'",$coms_conect);
}
if($row["navication_pic"]>""){?>
    <!-- Start Slider -->
...
<div class="container">
    <main>
        <?$query23="select id,left_block,right_block,center from new_blocks_sorts where type='m'  and tem_name='$tem_name' and file_name=2 and pages_id=11  and la='$defult_lang'  and status=1";

        $result23 = $coms_conect->query($query23);
        $RS23 = $result23->fetch_assoc();
        $center=$RS23['center'];
        if($center=='')$center=12;
        if(get_modual_setting_result($defult_site,$defult_lang,'company_show_have_ads',$coms_conect))
            $center=$center-2;
        if($RS23['right_block']>0){
            echo ' <aside class="col-md-'.$RS23['right_block'].' right_side col-xs-12 pull-right animated fadeIn">
        ...
        <aside class="col-md-<?=$center?> col-xs-12 pull-right animated fadeIn main_side">
            <section class="block-col">
                <?if($RS23['center']>0){?>
                ...
                <div class="">
                    ...
                    <?if(time()>=$_SESSION['count']+2){
                        $query1="update new_company set view=view+1 where id='$madual_id'";
                            ...
                            <section class="content">
                                <div class="">
                                    <section class="col-md-12 newsitempage">
                                        <div class="row">
                                            <div class="col-xs-12 pull-left rtl prl0">
                                                <p class="cat_date ">
                                                    <?$cat_company=get_modual_cat(19,$row['id'],$coms_conect);
                                                    foreach($cat_company as  $key=>$val){
                                                        echo "  <span class='label'><a itemprop='keyword' target='blank' href='/company/$defult_lang/category/{$val['id']}'><span>{$val['name']}</span></a></span>  ";
                                                    }
                                                    ?>
                                            ...
                                            <div class="author-avatar">
                                                <?$sql303 = "SELECT name,avatar FROM new_managers where id={$row['user_id']}";
                                                $sql306 = "SELECT name,manager_name FROM new_company where id={$row['id']}";
//                                                echo $sql306;
                                                //echo $sql303;
                                                $result303 = $coms_conect->query($sql303);
                                                $row330 = $result303->fetch_assoc();
                                                $result306 = $coms_conect->query($sql306);
                                                $row306 = $result306->fetch_assoc();?>
<!--                                                <img style="width:50px; border-radius:50%;"  src="--><?//=get_user_avatar($row330['avatar']);?><!--">-->

                                            </div>
                                            <div class="author-details">
                                                <div itemprop="author" itemscope="" itemtype="" class="author-details-row1">مدیرکل شرکت: <span itemprop="name" class="color-red" id="manager"><?=$row306['name'];?></span>
                                                </div>
                                                <div class="author-details-row2"><!--span><?=miladitojalalitime(date("Y-m-d ",$row["date"]),$row["la"])?></span-->
                                                    <span><?=jdate('d  F  Y | H:m:s',$row['date']);?></span></div>
                                            ...
                                            <div class="col-xs-12 pull-left prl0">
                                                <?$sql33 = "SELECT adress FROM new_file_path where name='company_image' and type=19 and id='$id'";
                                            ...
                                            <div class="col-xs-12 tools rtl">
                                                <div class="col-md-6 col-xs-12 pull-right rtl pr0 social">
                                                    ...
                                                    <div class="ccomment pull-right">
														...
														type=19 and madul_id=$madual_id",$coms_conect)?></span>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <?if(get_result("SELECT adress from new_file_path where id='$id'  and type=19 and name='company_gallery'",$coms_conect)){?>
                                                <div id="myCarousel" class="carousel slide" data-ride="carousel">
                                                    <!-- Indicators -->
                                                    <ol class="carousel-indicators">
                                                        <?$i=0;$sql14 = "SELECT adress from new_file_path where id='$id'  and type=19 and name='company_gallery'";
                                            ...
                                            <?}?>
                                            <?$sql2330 = "SELECT adress FROM new_file_path where name='company_video' and type=19 and id='$id'";
                                            ...
                                            if($row2330['adress']){?>
                                                <div class="col-xs-12 newsvideoitem prl0">
                                                    <video id="example_video_1" class="video-js vjs-default-skin" controls preload="none" width="100%" height="100%"
                                                           poster="/<?=get_content_modual_address($row['id'],$coms_conect)?>"
                                                           data-setup="{}">
                                            ...
                                            <?}?>
                                            <?$sql233 = "SELECT adress FROM new_file_path where name='company_sound' and type=19 and id='$id'";
                                            ...
                                            <div itemprop="articlebody" class="col-xs-12 matnkhabar prl0">
                                                <p><?$row["text"]=str_replace('src="source','src="/source',$row["text"]);
                                                    ...
                                                    $row["text"]=str_replace('<a href="company','<a href="/company',$row["text"]);
                                                    ...
                                                    echo $row["text"].'<br>';
                                                    $temp= str_split($row['content_type'],4);
                                                    if($temp[1]==1)$erjaei="<a rel='nofollow' target='_blank' href='{$row['source_link_company']}'>{$row['source_company']}</a>";
                                                    else $erjaei=$row['source_company'];
                                                    if($erjaei) echo  "$view_source_content ".$erjaei;
                                                    ?>
                                                </p>
                                            </div>
                                            <?$sql2343 = "SELECT adress FROM new_file_path where name='company_attach' and type=19 and id='$id'";
                                            $result2343 = $coms_conect->query($sql2343);
                                            $row2333 = $result2343->fetch_assoc();
                                            if($row2333['adress']){?>
                                                <div class="col-xs-12 newsatacheitem prl0">
                                                    <fieldset>
                                                        <legend><?=$view_company_mortabet?></legend>
                                                        <ul class="pr10">
                                                            <li><?$row2333['adress']=str_replace('%20',' ', $row2333['adress']);?>
                                                                ...
                                                                <a href="<?=$row2333['adress']?>"><span><?=$view_dl_attach_files?></span>
                                                                    <?$headers =get_headers($row2333['adress'],1);?>
                                                                    <span class="filesize"><?=format_size($headers["Content-Length"]);?></span>
                                        ...
                                        <div class="row">
                                            <?if(get_result("select count(id) from new_mudoal_label where type=19 and id={$row['id']}",$coms_conect)>0){?>
                                                <div class="col-xs-12 tags prl0">
                                                    <fieldset>
                                                        <legend><?=$view_tags?></legend>
                                                        <ul class="pr10">
                                                            <?create_label_pishgaman($row['id'],'company',$madual_lang,$coms_conect,19);?>
                                                        </ul>
                                                    </fieldset>
                                            ...
                                            <div class="col-xs-12 relatedposts prl0">
                                                <fieldset>
                                                    ...
                                                    <ul class="newsrelated pr10">
                                                        <?$query="SELECT b.name,b.id,b.title,b.date,b.la FROM new_related_madual a ,new_company b  where page_id='$madual_id' and a.id=b.id  and type=19";
                                                        ...
                                                        while($mortab = $result->fetch_assoc()){
                                                            ?>
                                                            <li>
                                                                <a href="<?="/company/{$mortab['la']}/{$mortab['id']}/".insert_dash($mortab['name'])?>">
                                                            ...
                                                            <?$i++;}?>
                                                        <?if($i<6){
                                                            $cat_id=get_result("select cat_id from new_modules_catogory    where page_id=$madual_id and type=19  ",$coms_conect);
                                                            $j=6-$i;
                                                            $sql120 = "SELECT a.date,a.la,a.id,a.title,name from new_company a ,new_modules_catogory c   
																where a.id>0  and c.type=19 and a.id<>$madual_id and c.page_id=a.id and c.cat_id=$cat_id    and status=1
...
//                                                             echo $sql120 ;exit;
                                                            ...
                                                            while($row10 = $resultd10->fetch_assoc()) {?>
                                                                ...
                                                                <li>
                                                                    <a href="<?="/company/{$row10['la']}/{$row10['id']}/".insert_dash($row10['name'])?>">
                                                                        <span class="fa <?=$icon_array[array_search($row10['content_type'],$content_type_array)]?>"></span>
                                                                        <span><?=$row10['title']?></span>
                                                                        <span class="datepublish"><?=miladitojalaliuser(date('Y-m-d',$row10["date"]),$row10["la"])?></span>
                                                                    </a>
                                                                </li>
                                                            <?}
                                                        } ?>
        ...
        <?if($RS23['left_block']>0){
            echo ' <aside class="col-md-'.$RS23['left_block'].' col-xs-12 left_side pull-right animated fadeIn">
				<section class="block-col">';
            create_location($subdomian_add,$defult_dir,$defult_site,$defult_lang,$tem_name,$RS23['id'],'l',$coms_conect,$cat_id);
            echo '	</aside>
				</section>';
        }?>
        <?if(get_modual_setting_result($defult_site,$defult_lang,'content_show_have_ads',$coms_conect)){?>
            <aside class="col-md-2 hidden-sm hidden-xs pull-left animated fadeIn">
                <section class="block-col">
                    <div class="block">
                        <div class="content ads-col">
                            <?$query="select title,link,pic_adress from new_tem_setting where name='content_show_have_ads'  and la='$defult_lang' and site='$defult_site'";
                            $result = $coms_conect->query($query);
