...
<section class="breadcrumb-sec animated fadeIn">
    <div class="container">
        <ol class="breadcrumb rtl">
            ...
            <li><span class=" active"><?=$view_company_list_sub?>
                    <a href="<?=$url?>"> <span class=" active"><?= get_result("select name from new_modules_cat where id='$madual_cat_id'",$coms_conect)?></span></a>
            </li>
            </li>
<!--         --><?// $erer="select name from new_modules_cat where id='$madual_cat_id'";  echo $erer;exit;    ?>
            ...
            <section class="block-col">
                ...
                <div class="row">
                    <article class="col-md-12 col-sm-8 pull-right animated fadeIn">
                        ...
                        <?$condition=get_result("select count(a.id) as count from new_company a ,new_file_path b, new_modules_catogory c
								where c.type=19 and a.id=c.page_id and c.cat_id='$madual_cat_id' and  b.name='content_image' and is_special=1 and spesial_start_date <='$now' and spesial_finish_date >='$now' and site='$site' and la='$madual_lang' and status=1 and b.type=19 and a.id=b.id  and publish_date<='$now'
								",$coms_conect);

//echo $condition.'asas'.$ewewe;exit;

                        if($condition>0){?>
                            <section class="vige">
                                <div class="row">
                                    <?$query1="select a.view,a.name,a.publish_date,a.la,a.title,a.id,b.adress from new_company a ,new_file_path b, new_modules_catogory c
								where c.type=19 and a.id=c.page_id and c.cat_id='$madual_cat_id' and  b.name='content_image' and is_special=1 and spesial_start_date <='$now' and spesial_finish_date >='$now' and site='$site' and la='$madual_lang' and status=1 and b.type=19 and a.id=b.id  and publish_date<='$now'
								group by b.id order by a.id desc limit 0,3";
                                    ...
                                    while($row1 = $result1->fetch_assoc()) {
                                        ...
                                        <div class="col-md-4 col-sm-4 col-xs-6 pull-right">
                                            <div class="item">
                                                <a href="<?="/company/{$row1['la']}/{$row1['id']}/".insert_dash($row1['name'])?>" class="hvr-grow"><img height="200" src="<?=$row1['adress']?>"></a>
                                                <h4><?=$row1['name']?></h4>
                                                <a href="<?="/company/{$row1['la']}/{$row1['id']}/".insert_dash($row1['name'])?>"><h3><?=$row1['title']?></h3>
                                                </a>
                                    ...
                                    <div id="switchshow">
                                        ...
                                        <div class="row">
                                            <ul class="pl0 gridshow">
                                                <?################################################################
                                                $sql1 = "SELECT count(a.id) as cnt from new_company a ,new_file_path b,new_modules_catogory c where 
												c.type=19 and a.id=c.page_id and c.cat_id='$madual_cat_id' and b.name='content_image'  and a.status=1 and b.type=19 and a.id=b.id and a.la='$madual_lang' and a.site='$site' and publish_date<='$now'";
//                                               echo $sql1;exit;
                                                $result = $coms_conect->query($sql1);
                                                $RS = $result->fetch_assoc();
                                                $a=new_pgsel((int)$madual_page_id,$RS["cnt"],9,9,"$root/company/$madual_lang/category/$madual_cat_id/page");
                                                $from=$a[0];
                                                $to=$a[1];
                                                $pgsel=$a[2];
                                                ?>
                                                <?$query="select view,adress,abstract,a.name,title,la,a.id,publish_date from new_company a ,new_file_path b ,new_modules_catogory c where
										c.type=19 and a.id=c.page_id and c.cat_id='$madual_cat_id' and b.name='content_image'  and status=1 and b.type=19 and a.id=b.id and la='$madual_lang' and site='$site' and publish_date<='$now'
                                                    ...
                                                    <div class="news-media">
                                                        <div class="row news_item">
                                                            <div class="col-lg-3 col-md-3 col-sm-4 col-xs-4 pull-right rtl sr_no_padd_1">
                                                                <div class="photo">
                                                                    <figure>
                                                                        <a href="<?="/company/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>" class="hvr-grow">
<!--                                                                            <img src="--><?//=$row11['adress']?><!--" alt="--><?//=$row11['title']?><!--"></a>-->
												...
												a.id=b.cat_id and b.type=19 and a.type=19 and b.page_id={$row11['id']} order by rang";
//                                                                       echo $queryp;exit;
                                                                        $resultp = $coms_conect->query($queryp);
                                                                        $p=array();$i=50;
                                                                        while($rowp = $resultp->fetch_assoc()) {?>
                                                                            <a href="/company/<?=$row11['la']?>/category/<?=$rowp['cat_id']?>" style=" background: rgb(<?=150+$i?>, <?=27+$i?>, <?=124+$i?>);"  class="news_item_tag">
                                                                                <?=$rowp['name']?>
                                                                            </a>
                                                                            <?$i=$i+50;}?>

                                                                        <a href="<?="/company/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>" class="pull-left">
													<span class="comment-number">
													<?=get_result("select count(id) from new_madules_comment where madul_id={$row11['id']} and type=19",$coms_conect);?>
													</span>
                                                                            <i class="fa fa-comment-o" aria-hidden="true"></i>
                                                                        </a>
                                                                    </h6>
<!--                                                                    --><?php //echo 'asddssdfdfhjkhkhlkkhj';?>
                                                                    <a href="<?="/company/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>"><h3><?=$row11['title']?></h3></a>
                                                                    <h6 class="news_item_author">
												<span class="sr-news-name"><a href="<?="/company/{$row11['la']}/{$row11['id']}/".insert_dash($row11['name'])?>">
