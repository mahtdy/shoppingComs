...
<?$keyword_id=	delete_dash(urldecode($keyword_id));?>
            <div class="container">
                <ol class="breadcrumb rtl">
                    ...
                    <li> <span class="active">لیست صفحه ها با جستجو :  </span> 
        ...
        <div class="container">
            <main>
			 <?$query23="select id,left_block,right_block,center from new_blocks_sorts where type='m'  and tem_name='$tem_name' and file_name=4 and pages_id=5 and la='$defult_lang' and status=1";
			$result23 = $coms_conect->query($query23);
			$RS23 = $result23->fetch_assoc();
			$center=$RS23['center'];
			if($center=='')$center=12;
			 if(get_modual_setting_result($defult_site,$defult_lang,'page_sesrch_have_ads',$coms_conect))
				 $center=$center-2;
							...
							<section class="content">
								<div class="row">
									...
									<?if($url_temp[3]=='search'){ 
										...
										c.type=5 and a.id=c.page_id    
										and a.status=1   
										and( a.text  like '%$keyword_id%' or a.name  like '%$keyword_id%' or a.title  like '%$keyword_id%'  or a.meta_description  like '%$keyword_id%' or a.meta_keyword  like '%$keyword_id%' ) 
										and a.la='$madual_lang' and a.site='$site' and publish_date<='$now'";
										//echo $sql1;exit;
										$result = $coms_conect->query($sql1);   
										$RS = $result->fetch_assoc();
										$a=new_pgsel((int)$madual_page_id,$result->num_rows,9,9,"$root/page/$madual_lang/search/$keyword_id/page"); 
										$from=$a[0];
										$to=$a[1];
										$pgsel=$a[2];
										$limit=" limit $from,$to";
										...
										where c.type=5 and  a.id=c.page_id	 
										and( a.text  like '%$keyword_id%' or a.name  like '%$keyword_id%' or a.title  like '%$keyword_id%'  or a.meta_description  like '%$keyword_id%' or a.meta_keyword  like '%$keyword_id%' ) 
                        ...
                        <section class="block-col">
                            <div class="block">
                                <div class="content ads-col">
									<?$query="select title,link,pic_adress from new_tem_setting where name='page_sesrch_have_ads'  and la='$defult_lang' and site='$defult_site'"; 
