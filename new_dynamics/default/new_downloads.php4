...
//     $conn = mysqli_connect('localhost', 'root', '', 'test');
    ...
    //اتصال برقرار است
        ...
        //ایمن سازی
        $token = mysqli_real_escape_string($coms_conect, $token);
        $file = mysqli_real_escape_string($coms_conect, $file);
//        حذف ردیف هایی که تاریخ اعتبار آنها گذشته است
        $sql ="DELETE FROM file_storage WHERE date_time < '$time'";
//        echo $token.$file.$ip.'42424242';
        //بررسی اطلاعات فایل درخواستی کاربر با سرور
        $sql = "SELECT temp_name FROM file_storage WHERE original_name = '$file' AND temp_name = '$token'";
//        $count = mysqli_num_rows($sql);
//        $count = 1;
        $sql =  "SELECT COUNT(id)AS countid FROM file_storage WHERE temp_name = '$token' AND original_name = '$file' ";
...
//        اطلاعات فایل معتبر است
        if($row['countid'] > 0){
            ...
            $file_name1 =$_SERVER['DOCUMENT_ROOT'] .trim('/new_image/file/ ').  $file;
...
//          $file_name =__dir__ .'/file/'.  $file;
            $file_name = __DIR__.'/new_image/file/'.$file.'';
 //             $file_name =   $file;
            $size=filesize("$file_name1");
//            echo $file_name1.  $size.'eeee'.$file_name;
            ...
            header('Content-Length: '.filesize("$file_name1"));
