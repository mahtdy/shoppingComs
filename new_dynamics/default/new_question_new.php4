<?$question_new_nave= get_tem_result($defult_site,$defult_lang,"question_new_nave",'default',$coms_conect); 
 $question_base_setting= get_tem_result($defult_site,$defult_lang,"question_base_setting",'default',$coms_conect);
if ($question_base_setting['title']==1&&$_SESSION["new_okusername"]=='') include 'new_lock_page.php';else{ 

 $question_text=injection_replace($_POST['question_text']);
$question_cat=injection_replace($_POST['question_cat']);
$sender_email=injection_replace($_POST['sender_email']);
$sender_name=injection_replace($_POST['sender_name']);
$question_mobile=injection_replace($_POST['question_mobile']);
$tag_question=injection_replace($_POST['tag_question']);
if(isset($_POST["comment_captcha"])&&$_POST["comment_captcha"]!=""&&$_SESSION["code"]==$_POST["comment_captcha"]&&$question_text>""){
	$arr=array("member_id"=>$_SESSION["new_okusername"],"mobile"=>$question_mobile,"sender_name"=>$sender_name,"sender_email"=>$sender_email,"cat_id"=>$question_cat,"question"=>$question_text,"la"=>$madual_lang,"site"=>$defult_site,'date'=>$now,'ip'=>$custom_ip);
	$id=insert_to_data_base($arr,'new_question',$coms_conect);
	$arr=explode(",",$tag_question); 
	foreach($arr as $val){
		if($val>""){
...
?>
 ...
 <!-- Start Slider -->
            <section class="slider pimg animated fadeIn hidden-xs">
                 <a href='<?=$question_new_nave['link']?>' target='_blank'><img src="<?=$question_new_nave['pic_adress']?>"></a>
            </section>
            <!-- Start breadcrumb -->
            <section class="breadcrumb-sec animated fadeIn">
                <div class="container">
                    <ol class="breadcrumb rtl">
						...
						<li class="active"><a href="/question/<?=$madual_lang?>"><?=$view_new_question?></a></li>
        ...
        <div class="ptitle">
            <div class="container">
                <span class="fa fa-lightbulb-o"></span>
                  <h1 class="title"><?=$question_new_nave['title']?></h1>
                <span class="pdesc"><?=$question_new_nave['text']?></span>
            </div>
        </div>
		 
        <!-- End Page Title -->
	<form method='post' id="questionnewForm" action='' data-fv-framework="bootstrap">
        <div class="container">
            <main>  
                <!-- Start Main -->
				<?$col_md=6;$col_md_str='col-md-offset-3';
					if(get_modual_setting_result($defult_site,$defult_lang,'question_new_have_ads',$coms_conect)){?>
						<aside class="col-md-2 hidden-sm hidden-xs pull-left animated fadeIn">
							<section class="block-col">
								<div class="block">
									<div class="content ads-col">
										<?$query="select title,link,pic_adress from new_tem_setting where name='question_new_have_ads'  and la='$defult_lang' and site='$defult_site'"; 
                ...
                <div class="row faq col-md-<?=$col_md .' '.$col_md_str?>  rtl" style="background-color: #EEEEEE;padding: 10px;">
                    ...
                    <h4 class="rtl"><?=$question_new_nave['pic']?></h4>
					<br>
					<div class="row">
						<div class="form-group col-md-8 pull-right">
							<input type="text" value="<?=$_SESSION["new_name"] . ' '.$_SESSION["new_family"]?>" name="question_name" placeholder="نام" class="form-control" 
					...
					<div class="row">
						<div class="form-group col-md-8 pull-right">
							<input type="text" value="<?=$_SESSION["new_usermobile"]?>" name="question_mobile" placeholder="تلفن همراه" class="form-control" 
							data-fv-notempty="true"  
							data-fv-notempty-message="پر کردن اين فيلد الزامي است">
						</div>
                    </div>
                    ...
                    <div class="row">
						<div class="form-group col-md-8 pull-right">
							<select class="form-control" name='question_cat' id="search_btn2">
                    ...
                    <div class="form-group">
                        <textarea name="question_text" placeholder="<?=$view_message_question?> ..." class="form-control" rows="6"
						data-fv-notempty="true"  
						data-fv-notempty-message="پر کردن اين فيلد الزامي است"></textarea>
                    </div>
					<div class="form-group">
                        <input type="text" name="tag_question" id="tag_question" value="" placeholder="برچسب خود را بنویسید سپس Enter بزنید" class="form-control" style="width: 100%; "/>
                    </div>
					<div class="form-group">
                    ...
                    <div class="col-md-4 form-group pull-right pl0">
                        <button type="text" onclick='window.location="/question/<?=$defult_lang?>"' class="btn btn-warning fullwidth"><?=$view_back_list_question?></button>
                    </div>
                </div>
                <!-- end main row -->
            </main>
...
<script>
	$(document).ready(function() {
		$('#tag_question').tagsinput({
			maxTags: <?=$question_base_setting['pic']?>
		});
	
		$('#questionnewForm').formValidation();
	});
</script>	