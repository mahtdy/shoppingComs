...
//echo 'fctr='.$factorId.'  idget='.$id_get.'  trns='.$trans_id.' modid='.$modID.' rs='.$rs.' ordid='.$orderID;
//exit;
...
$factorId=injection_replace($_POST['factorId']);
$id_get=injection_replace($_POST['id_get']);
$trans_id=injection_replace($_POST['trans_id']);
$result=injection_replace($_POST['result']);

$sql="select * form new_cart where ";

if (isset($list_list)) {
    $cart_arr = array("id_cart" => $list_id,"price" => $list_list, "rahgiri" => $trans_id, "peygiri" => $id_get,
        "not_trakonesh" => $mobile, "id_user" => $text, "id_off" => $text);
    insert_to_data_base($cart_arr, 'new_pay', $coms_conect);
}

echo 'fctr='.$factorId.'  idget='.$id_get.'  trns='.$trans_id.' result'.$result;
...
//$amount = 1000;
//$redirect = 'http://localhost/pay_pay/fa';
//$name = '';
//$email = '';
//$description = '';
//$factorId = 1;
...
?>
<script>
    var lis_lis=$.session.get("list_val");
    alert('lis='+lis_lis);
</script>
