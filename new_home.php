...
$defult_site=$site;
 	...
 	$query5="select  la  from new_first_page_setting where site='$site' and first_site=1";
	$result5 = $coms_conect->query($query5);
	$RS15 = $result5->fetch_assoc();
	$la=$RS15['la'];
if($active_lang=='')	
$active_lang=$la;	
####check lang in active
if(get_result("select status from new_language where title='$active_lang'",$coms_conect)==0)
...
if($madual_lang==''){
	...
	if($type==1&&$contraction==0){ 
		...
		echo $view_template_notexist;	
	}
	

	if($type==3){
		$page_title=get_result("select title from new_static_page where id={$RS1['value']}",$coms_conect);
		$page_name=get_result("select name from new_static_page where id={$RS1['value']}",$coms_conect);
		$page_site=get_result("select site from new_static_page where id={$RS1['value']}",$coms_conect);
		$page_la=get_result("select la from new_static_page where id={$RS1['value']}",$coms_conect);
		
		echo "<script>window.location='$page_name/$page_la"."'"."</script>";
	}
	if($type==4){
	...
	<div id="coming-soon">
	<h1><img src="/UnderConstruction.png" style="width: 100%;" class="center"></h1>
	<h1><?=$RS1['value']?></h1>
	</div>
...
}else	if($type==1&&$contraction==1&&$madual_lang==''){ 
	?>
	<title><?=$RS1['value']?></title>
	...
	<div id="coming-soon">
	<h1><img src="/undercounstraction1.png" class="center"></h1>
	<h1><?=$RS1['value']?></h1>
	</div>
...
//echo $manage_lang.'<br>'.$manage_site;
$tem_name=get_template($manage_site,$manage_lang,$coms_conect);
